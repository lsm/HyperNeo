...
- **Excludes**: any caller.
- **Tests**: CAS unit rows: win, lose, dead-ID clear-and-claim, token-scoped release, a row proving a LIVE reservation token blocks even while `postApprovalSessionId` is still NULL (the reserved-but-not-yet-spawned window — review correction PR #2983 round 6), an EXPIRED-RESERVATION TAKEOVER row (a token past its expiry is atomically replaced by the new claim with no concurrent-spawn window — review correction PR #2983 round 3), and TTL-FENCING INVARIANTS at the primitive level: the expiry strictly exceeds the shared spawn-bound constant (rounds 5/29 — the RENEW-until-settled integration behavior itself is exercised in P46 where the bounded spawn holder lives; this slice lands no caller). The ADMISSION PREDICATE BLOCKS EVERY UNRESOLVED PREPARED or ENQUEUED dispatch attempt — not only unfinalized delivered-kickoff records: P44 owns the claim primitive and the dispatch-record schema, so the crash-between-enqueue-and-stamp claim test (a PREPARED record whose kickoff is queued must reject a new claim until startup reconciliation clears it) lands HERE, before TAM-F1/F2 can expose PREPARED records.
- **Depends on**: P43.
...
- **Scope**: Wires the finalizer into the IDLE-COMPLETION callback (round 30, the fence's lifter) AND the `session.error` subscription (task-agent-manager.ts:3124–3143 — the completion callback fires only from the idle branch, so the error-terminal path is separate), with the error-before-idle retry pin. VERIFIED/CANCELLED TERMINATION wiring is NOT here (P46-W3 owns it). The branches are EXPLICITLY SPLIT for DEFERRED attempts: an ordinary IDLE event from the reused worker's current turn IGNORES DEFERRED attempts (the deferred kickoff is still valid and unconsumed — removing it there would lose the post-approval work and contradict the unrelated-completion ineligibility rule), while an actual `session.error` performs ONLY the message-specific fail/remove-and-correlate settlement via P46-W's primitive — it does NOT lift a DELIVERED fence by itself: `ErrorManager.broadcastError` publishes `session.error` for NONTERMINAL, recoverable errors (error-manager.ts:457–470) and message-delivery dead-lettering publishes the same event (app.ts:970–981), so an unrelated recoverable error before idle must not let P44 admit a retry while the original worker still processes the kickoff; a DELIVERED fence lifts only on VERIFIED TERMINATION or attempt-correlated completion (the error event alone drives message-specific failure settlement where applicable). Finalization is RESTRICTED TO `DELIVERED` attempts otherwise (a DEFERRED outcome's unconsumed kickoff survives the worker stop; finalizing its fence would admit a new dispatch after reapproval while the old deferred kickoff can still replay on session restore — deferred-then-cancelled-then-reapproved row).
- **Depends on**: P46-W.
