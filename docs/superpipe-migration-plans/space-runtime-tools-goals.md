...
- **Scope**: Pin `PostApprovalRouter.route` branch behavior: `multiRoutes` WARNS and continues with the first route (never a terminal branch); interpolation with unknown keys but NON-EMPTY produced instructions still spawns while logging `kickoff referenced unknown keys` (review correction PR #2983 round 6); `no-route` terminalization via `goalService.handleTaskTerminal`; dead-previous-session fall-through that spawns a replacement overwriting the stale ID; expected spawn failures (`isSpawnSupersededError`/`isTransientSpawnError`) clearing pending-completion fields and recording `postApprovalBlockedReason`. Parallel-safe leaf.
