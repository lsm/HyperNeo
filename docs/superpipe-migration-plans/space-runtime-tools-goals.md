...
- **Scope**: Wires the finalizer into the IDLE-COMPLETION callback (round 30, the fence's lifter) AND the `session.error` subscription (task-agent-manager.ts:3124–3143 — the completion callback fires only from the idle branch, so the error-terminal path is separate), with the error-before-idle retry pin. RESTRICTED TO `DELIVERED` attempts (a DEFERRED outcome's unconsumed kickoff survives the worker stop; finalizing its fence on termination would admit a new dispatch after reapproval while the old deferred kickoff can still replay on session restore — for DEFERRED attempts, atomically fail/remove the exact message and settle its correlation via P46-W's primitive BEFORE finalizing, deferred-then-cancelled-then-reapproved row).
- **Depends on**: P46-W.
