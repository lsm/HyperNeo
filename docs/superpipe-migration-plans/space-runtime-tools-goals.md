...
- **Scope**: Wires `finalizePostApprovalDispatch` into the worker-completion path so the reused-delivery fence always has a lifter (round 30) — the completion-path integration split OUT of P46 (which stays additive/unwired construction).
- **Depends on**: P46.
...
- **Depends on**: P46, P46-W; TAM-F2 from `task-agent-manager.md` (the wired
