...
- **Step-by-step migration**:
  ...
  4. Branch `noRoutes` → `effect` `terminalize-no-route`: the TERMINAL
     ...
     AND the atomic clears of `pendingCheckpointType`,
     `pendingCompletionSubmittedByNodeId`, `pendingCompletionSubmittedAt`,
     `pendingCompletionReason`, `postApprovalSessionId`,
     `postApprovalStartedAt`, `postApprovalBlockedReason`, and
     `postApprovalSourceNodeId` (review correction PR #2983 round 16:
     ...
     row). When the task
     belongs to a goal, PASS those terminal updates to
...
- **Step-by-step migration**:
  ...
  2. The target parse and execution resolution run in the pipeline's snapshot stages — with a MALFORMED-TARGET terminal branch (review correction PR #2983 round 15: `parseAddress` throws on inputs like `coder`; the current handler catches it, audits `reason: 'malformed_target'`, and returns the parser message as a normal unsuccessful tool result — the branch preserves that response and audit entry instead of an error outcome escaping the pipeline).
...
- **Shell/effect wiring**: The pipeline's effect stages call `boundTaskManager.archiveTask` / `publishTask`, then emit `space.task.updated`. Review correction PR #2983 round 16: each archive caller keeps its own lookup normalization — the tool's direct repo lookup reaches the routing gates and returns the explicit `does not belong to this space` response, while `onArchiveTask`'s space-bound manager masks a foreign task to `Task not found`; per-caller mapping is pinned with a foreign-task row for BOTH callers (never unified). The tool shell keeps its success audit (`logAudit('archive_task', { previousStatus }, task_id)` — review correction PR #2983 round 6).
- **Step-by-step migration**:
...
- **Step-by-step migration**:
  ...
  7. Stage `persist-subscription` (when `dynamic`) calls
     ...
     boundary: on throw, the shell's rollback removes the trie entry and
     re-inserts `displaced`, and the failure maps to the RESULT
...
- 🔧 apply — prod Δ ≲50, test Δ ≲100
- **Scope**: The `publish_task` tool in `space-agent-tools.ts` runs the P15 pipeline; keep CALLER-SPECIFIC lookup normalization/result mapping around the shared stages rather than literally "picking one" lookup — review correction PR #2983 round 14: the tool's direct repo lookup reaches `routePublishTask` and returns the explicit `does not belong to this space` response, while the RPC's space-bound `taskManager.getTask` normalizes a foreign task to `null` and returns `Task not found`; forcing one lookup changes a public response or exposes foreign-task existence, so each shell keeps its masking discipline (with foreign-task rows per shell). The tool shell RETAINS its success audit — `logAudit('publish_task', { previousStatus }, task_id)` from the admission snapshot — after the pipeline halts; only the response mapping and this audit stay in the shell.
...
- **Scope**: `space-runtime.ts:registerSubscription` executes the P30 pipeline — including its final in-stage-caught `redispatch-retained-events` effect — inside its shell's explicit in-memory rollback, with the `persist-subscription` stage's catch mapping a persistence failure to the `{ success: false, error: 'Failed to persist subscription.' }` RESULT after the rollback (review correction PR #2983 round 9) and the redispatch as the pipeline's own final guarded effect (review correction PR #2983 round 5).
...
- ➕ additive core — prod Δ ≲150 (types/stage-dominated cap), test Δ ≲350
- **Scope**: Remaining stages plus the SINGLE `stagedRun('post-approval-route')` composition (one pipeline per business path — it lands whole here): the interpolation stage appending its `kickoff referenced unknown keys` warning to the `warnings` annotation when `missingKeys` is non-empty with non-empty text, with a guarded logging effect before the spawn emitting it (review correction PR #2983 round 6); `spawn-post-approval-sub-session` wrapped in a DEADLINE SHORTER THAN THE RESERVATION TTL with lease RENEWAL until the spawn actually settles, or acknowledged cancellation plus LATE-SESSION CLEANUP on the deadline (review corrections PR #2983 rounds 5/6: without a bound, a slow spawn outlives the token and another router's expiry takeover double-spawns — and a timeout race alone leaves the still-pending create/attach/inject work running in the background, so a session that eventually appears after the race rejected must be terminated/recorded, never left to run kickoff alongside a takeover spawn), catching EXPECTED failures in-stage (diverting to the clear/block/skip branch) and compensating UNEXPECTED failures by terminating the spawned session (the claim is non-releasable post-spawn); the spawned session ID/start time written to a `SpawnAttemptBox`-style EXTERNAL mutable box EXTENDED WITH AN OWNERSHIP FLAG (review correction PR #2983 round 12: when the target agent already has a live sub-session, `spawnPostApprovalSubSession` INJECTS INTO IT and returns its pre-existing ID — `task-agent-manager.ts:4759-4784` — so the box records whether this attempt CREATED the session or REUSED one; the termination compensation and the deadline path's late-session cleanup run ONLY for sessions created by this attempt, never for a reused workflow worker this call does not own); `resnapshot` copying them into ctx; `record-dispatched-session` converting the reservation via the same CAS AND ATOMICALLY CLEARING the pending-completion state — `pendingCheckpointType`, `pendingCompletionSubmittedByNodeId`, `pendingCompletionSubmittedAt`, `pendingCompletionReason`, and the stale `postApprovalBlockedReason` — alongside `postApprovalSessionId`/`postApprovalStartedAt` (review correction PR #2983 round 14: matching post-approval-router.ts:277-286; leaving them set keeps an approved, dispatched task rendering as awaiting-completion review in `PendingTaskCompletionBanner`/`TaskStatusActions`; pin the successful-spawn task shape), whose loss terminates THIS caller's CREATED session only (round 12) and CONDITIONALLY RELEASES the claim after verified termination — with a FENCED COMPENSATION PROTOCOL for the REUSED-SESSION arm (review correction PR #2983 round 15: the reused worker has ALREADY received the kickoff injection before returning, so on CAS loss it must neither be terminated nor left silently re-dispatchable: the loser records the acknowledged dispatch outcome for that worker — e.g. persisting the delivered-kickoff fact/blocked reason scoped to this attempt — whose loss terminates THIS caller's CREATED session only (round 12) and CONDITIONALLY RELEASES the claim after verified termination — with a FENCED COMPENSATION PROTOCOL for the REUSED-SESSION arm (review corrections PR #2983 rounds 15/16: the reused worker has ALREADY received the kickoff injection before returning, so on CAS loss it must neither be terminated nor left silently re-dispatchable — a scoped `postApprovalBlockedReason` alone does NOT block re-reservation because P44's admission CAS consults only the token and `postApprovalSessionId`; the acknowledged-delivery fact must PARTICIPATE IN THE ADMISSION PREDICATE itself, e.g. a dispatch-record entry keyed to this task/attempt that `claimPostApprovalDispatch` treats as claim-blocking until acknowledged/expired, so the next caller cannot reserve and inject the kickoff again while the delivered one runs unacknowledged), pinned by a reused-session CAS-loss test row plus an admission row proving the recorded delivery blocks re-reservation.
