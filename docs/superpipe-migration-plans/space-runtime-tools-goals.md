...
- **Scope**: Wires the finalizer into the IDLE-COMPLETION callback (round 30, the fence's lifter) AND the `session.error` subscription (task-agent-manager.ts:3124–3143 — the completion callback fires only from the idle branch, so the error-terminal path is separate), with the error-before-idle retry pin.
- **Depends on**: P46-W.
