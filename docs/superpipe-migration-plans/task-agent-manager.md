...
cut child issues from that table, not from any earlier summary of it. The 43
...
| TAM-SW-W | WIRE the nonterminal `approved`→`in_progress` transitions — `SpaceTaskManager.setTaskStatus` and its runtime entry points — to honor the TAM-SW fence; F1 gates on this WIRED slice; the approved-to-in_progress race test lands here | wire | ≲30 | ≲40 | TAM-SW |
| TAM-DS | DAEMON-STARTUP RECONCILIATION for deferred settlement tracking: restore the attempt→message correlation and late-settlement subscriptions for EVERY unresolved durable DEFERRED attempt — including CAS-loss rows whose dispatch stamp did NOT survive (P46 permits a DEFERRED attempt to lose `record-dispatched-session` and retain only its durable unfinalized fence; an unstamped attempt is never rearmed by a stamp-scoped query, its later consumption/dead-letter cannot finalize the fence, and P44 then blocks every future reservation indefinitely) — F1 is an unwired build and F2 wires only the spawner, so this startup wiring is its own slice, and the router must not be exposed before it (deferred-consumption, deferred-failure, and unstamped-CAS-loss restart rows; SINGLE STARTUP OWNER — TAM-DS is the one daemon-startup reconciliation wiring seam: cross-plan P46-R's reconciler call is consolidated INTO TAM-DS's startup entry (P46-R supplies the reconciler operation only, no separate startup wiring; TAM-DS depends on it), so no two slices can claim the same call-site seam) | wire | ≲40 | ≲60 | TAM-F1, P44 (cross-plan: the dispatch-record schema and finalizePostApprovalDispatch operation this slice invokes are owned by P44), P46-W (cross-plan: the atomic deferred-message settlement primitive this slice's reconciliation invokes is owned by P46-W), P46-R (cross-plan: supplies the prepared-fence reconciler operation TAM-DS's startup seam invokes — no separate startup wiring in P46-R) |
| TAM-F1 | `spawn-post-approval-worker` stagedRun, unwired, returning the P46 delivery outcome with a THREE-WAY ownership result (CREATED / LIVE-REUSED / COLD-RESTORED) carried from the create-stage result AND a DELIVERED/SKIPPED/DEFERRED delivery state — a reused worker that is busy/rate-limited/fails defer admission persists the kickoff as DEFERRED, and TAM-F1 OWNS the message-specific consumption tracking (so P46 cannot finalize the reused-delivery fence when an unrelated current-turn completion lands; finalization ties to an ATTEMPT-CORRELATED IDLE/completion signal or verified termination — consumption alone transitions the attempt to DELIVERED semantics but NEVER finalizes the fence (handleMessageYielded marks consumed when the message is yielded to the SDK, before processing runs; a fence lifted at consumption would admit a retry enqueueing the same kickoff concurrently), or await actual delivery before DELIVERED is reported) — the attempt→message correlation is DURABLE and REARMED/reconciled at startup, because the completion callbacks and late-settlement subscriptions are process-local while a DEFERRED kickoff plus its dispatch stamp survive a daemon restart (a post-restart dead-letter must still conditionally clear the matching stamp, and P45's alreadyRouted gate must not block every retry while the worker stays live; deferred-consumption and deferred-failure restart rows); COMPOSES the B1 create stage group and the E1 delivery stage group directly (no runner nesting); the reuse-arm terminal gate runs BEFORE the reuse mutation sequence under the same lock (fenced by TAM-F0); the fresh arm's post-create recheck covers every create outcome (CREATED skip terminates the just-created session AND deletes its durable row, COLD-RESTORED skip unwinds this attempt's streaming/bookkeeping WITHOUT deleting the durable row — the restored session was newly registered and started by this attempt, so treating it as untouched REUSED leaves a worker running for a terminal task, with its own test row — REUSED skip leaves the worker untouched). After the final create/attach await, REVALIDATE task ownership: the fence does not block a nonterminal reassignment, so re-read and match `workflowRunId` (a task reassigned while workspace resolution or the composed B1 stages await would otherwise use init/workflow/MCP/kickoff data prepared for the old run) AND RE-RESOLVE AND MATCH THE WORKSPACE: a task with no recorded post-approval session is not treated as active (`hasActiveTaskSession`, space-task-manager.ts:127–134), so a concurrent `workspacePath` update can change the target checkout while F1 awaits creation/attachment — deliver only when the workspace still matches (or fence workspace mutation through commit), with a workspace-change-during-create test; apply ownership-specific cleanup on refusal. The LIVE-REUSE arm needs the same post-awaits ownership reread: it awaits workspace reinjection after its ownership snapshot (:4879–4912), and `updateTask` permits `workflowRunId` changes even with an active session — a reassignment during `reinjectNodeAgentMcpServer` leaves the pre-await guard satisfied and delivers the kickoff to the old run's worker; add a post-reinjection ownership reread under the reuse protocol, with a reassignment-during-reinjection row. The LIVE-REUSE arm must also REARM message/attempt-specific COMPLETION TRACKING before each injection: `registerCompletionCallback` sets a one-shot `fired` flag and the first idle event unsubscribes/deletes the callbacks (:3086–3111), while the live-reuse arm (:4860–4917) does not re-register — an already-idle-then-reused worker would otherwise never lift the later kickoff's fence (already-idle-then-reused row). When the OWNERSHIP REREAD REFUSES after reuse mutations, the rollback scope follows the ARM: the full B1 mutation rollback (execution bindings, prompt/config, guards, skills, workspace/MCP state, completion callbacks, :1148–1277) applies ONLY to COLD-RESTORED outcomes (the create group runs there), while the LIVE-REUSE arm — which never calls `createSubSession` (:4860–4912) and must not have B1 composed into it — rolls back only its ACTUAL mutations (workspace/MCP reinjection and the completion-tracking rearm); fence nonterminal ownership through the mutation sequence to commit, or restore the captured state and attempt-owned bindings on the late refusal, with live- and cold-reuse reassignment rows. OWNERSHIP MUST STAY STABLE THROUGH DISPATCH COMMIT: the protected interval ends before the composed E1 delivery and P46's `record-dispatched-session` (E1 awaits context clearing/backlog replay/status publication/consumption, and F1 returns before the CAS), and TAM-F0 does not block nonterminal `workflowRunId`/workspace changes — fence ownership through ACTUAL DELIVERY and the dispatch CAS, or make ownership validation AND ENQUEUE ATOMIC rather than relying on the later CAS alone: revalidating before enqueue while releasing F1's fences before returning to P46 leaves a window where `workflowRunId`/workspace changes after E1 enqueues but before `record-dispatched-session` — the guarded CAS then rejects the stale snapshot, but the old worker already received the kickoff (the failure fence prevents a duplicate retry, not delivery to the wrong run/repository); pin races in BOTH windows (create-to-delivery and delivery-to-commit); lands the atomic-ownership/rollback protocol TOGETHER WITH its tests (terminal-skip rows for both outcomes, after-recheck-before-delivery transition, post-create attach/inject failure compensation per ownership — CREATED stopped/unregistered + durable-row deleted + status-and-session-guarded EXECUTION UNBIND (B1's fresh arm CAS-bound the execution; stopping alone leaves it `in_progress` pointing at the deleted worker, so both the terminal-skip and attach/inject-failure cleanups include the unbind with execution-row assertions), COLD-RESTORED unwound without deleting the durable session AND only while no successful concurrent joiner retains it, LIVE-REUSED intact — cold-cache unwind, the owner-fails-joiner-succeeds race, the terminal races) | build | ≲220 | ≲250 | TAM-B0, TAM-B1 (create stage group composed directly — F1 never calls the public B2 wrapper, so B2 stays on its caller-side track), TAM-E1, TAM-F0, P44 (cross-plan: F1's durable attempt/message correlation and crash-safe delivery fence use P44's dispatch-record schema and persistence API), TAM-F0-W1, TAM-F0-W2, TAM-F0-W3, TAM-F0-W4, TAM-F0-W5, TAM-F0-W6, TAM-OW, TAM-OW-W1, TAM-OW-W2, TAM-SW, TAM-SW-W, TAM-LH, TAM-PF (P46 outcome contract). F1 must RELEASE the TAM-LH injection handle before EVERY completed outcome (completed halts do not unwind compensations, and a skip before the E1 delivery group would otherwise leave it held and block later injections/resets); pin a subsequent lock waiter. The TAM-F0 terminal fence does NOT release at the F1 return on the success path — instead F1 returns RELEASABLE FENCE / COMMIT CONTINUATIONS FOR the TAM-F0 terminal fence, the TAM-OW nonterminal ownership fence, AND the TAM-SW approved-status-departure fence (matching P46's settlement and S2's propagation; under the fence-based implementation P46 has no handle to retain if F1 returns fewer, and a writer can acquire the missing fence before `record-dispatched-session`), which P46 settles on EVERY path (success, halt, error, timeout), OR ownership validation + kickoff enqueue + dispatch stamping move into ONE atomic operation: the fences stay held through P46's `record-dispatched-session` CAS (which runs after `spawnPostApprovalSubSession` returns), so a task reassignment cannot strand the kickoff with the old worker. EVERY POST-ADMISSION task-status DEPARTURE is fenced too: a task changing from `approved` back to `in_progress` after P44 admits it but before delivery/commit is a valid transition (:39) that neither TAM-F0 (terminal) nor TAM-OW (run/workspace) blocks — the worker would receive a post-approval kickoff after the approval was withdrawn — fence ALL departures from the admitted `approved` state through enqueue and commit, or make status validation + enqueue + stamp atomic, with an approved-to-in-progress race row. F1 must ALSO RELEASE THE MODEL-POOL RESERVATION on every skip: the fresh arm reserves the slot before creation (:4961) and a staged terminal-skip halt completes normally rather than entering the catch (:5049–5051), so each skipped outcome permanently consumes pool capacity until restart — include the reservation release and a capacity-reuse assertion. F1's exported result must CARRY an owner-provided, SESSION-SCOPED CLEANUP OPERATION for the ACTIVATED assignment, exposed for EVERY activated ownership outcome: F1 activates the model-pool reservation before returning, while P46 owns the later dispatch-CAS/route-failure cleanup — so P46 can invoke the operation whenever a session is terminated AFTER activation, for CREATED termination AND for the VERIFIED COLD-RESTORED unwind (a stopped cold-restored worker retaining its active assignment permanently consumes capacity), with a capacity-reuse test covering CAS loss, another post-spawn failure, AND the cold-unwind case. F1 EXPOSES the post-spawn FLUSH CONTINUATION WITHOUT STARTING it — P46 invokes the flush ONLY after `record-dispatched-session` wins the CAS (a CAS loss or later route failure otherwise terminates the CREATED session while the detached flush is still settling pending rows); the continuation/race row covers CAS loss or later-route-failure with pending rows present |
| TAM-F2 | Wire `spawnPostApprovalSubSession` AND own the missing-MCP FENCE-PROPAGATION integration test IN THE SAME SLICE (the acyclic order is F1 → S2 → F2; S2 does NOT depend on F1; the separate TAM-F2-INT is REMOVED — keeping it created an F2↔F2-INT cycle and an unverified intermediate state, so the propagation test is a single atomically-landable owner in F2). ALSO gates on TAM-DS AND P46-R (the crash-safe PREPARED/ENQUEUED fence construction + startup reconciliation must exist before F2's adapter — a delivered kickoff enqueued and daemon-exit before the guarded stamp would otherwise leave a retry able to re-inject after reservation expiry). NO UNSETTLED-CONTINUATION INTERMEDIATE STATE — and NO EARLY-RELEASING ADAPTER: a wrapper at the spawner-return seam CANNOT settle the continuations correctly, because the legacy router (post-approval-router.ts:252–285) awaits `spawnPostApprovalSubSession` and only THEN writes `postApprovalSessionId` — a wrapper releasing at return would drop the fences before the stamp, letting a reassignment/status-departure send the kickoff to stale work. The ONE compliant transition: F2 INCLUDES the router call/write ADAPTATION (and the adapter consumes the COMPLETE F1 result — the three fence continuations after its guarded stamp, the POST-SPAWN FLUSH CONTINUATION on the success branch, and the ACTIVATED-ASSIGNMENT CLEANUP operation on CAS-loss/post-spawn failure branches, exactly as P46 consumes them; settling only the fences would leave pending rows undrained after an F2 stamp win and leak model-pool capacity after a CAS loss) (a spawner/adapter at the spawner seam CANNOT settle the continuations or prevent the router's second write — the legacy router (:252–257) receives only `{ sessionId }` and unconditionally stamps at :277–285 with no post-return control; a spawner-only F2 state is unimplementable). F2 therefore has exactly ONE wiring seam — the ROUTER CALL SITE — plus one METHOD-BODY DELEGATION that is not a call-site seam: the `spawnPostApprovalSubSession` method body delegates to the F1 pipeline (the same pattern TAM-B2 uses for `createSubSession`, with both in-file callers' signatures unchanged). The router call site CLAIMS P44's reservation, spawns through F1, settles the three continuations AFTER its guarded stamp (the legacy second write is replaced by this guarded stamp). No combined-swap contradiction remains: the withdrawn "combined-swap" option referred to folding the P47 router-PIPELINE swap into F2 — THAT stays withdrawn (P47 later replaces F2's temporary guarded consumer seam with the complete pipeline); the method-body delegation is part of F2's single seam by the B2 precedent. The seam OWNS THE P44 LEASE LIFECYCLE while F1 is exposed before P46/P47: a BOUNDED SPAWN SHORTER THAN THE RESERVATION TTL with RENEWAL until the underlying F1 operation settles, or acknowledged cancellation plus ownership-aware late cleanup (merely calling `claimPostApprovalDispatch` does not satisfy P44's caller-side fencing — a spawn outliving the TTL lets a takeover dispatch a second worker); timeout/takeover race coverage included. The pre-P47 router (:209–257) otherwise performs only a liveness check then spawns WITHOUT claiming the reservation, so two concurrent route calls could both deliver and stamp afterward; include a race test for reassignment or status departure between spawner return and dispatch stamping | wire | ≲60 | ≲70 | TAM-F1, TAM-S2, TAM-DS, TAM-C2 (the message-handler fallback at space-task-message-handlers.ts:233–238 can otherwise still invoke the legacy restoration path while F2's adapter holds a P44 reservation and creates replacement worker B — the dead-A-versus-replacement-B race), P44 (dispatch-reservation primitive), P45, P46-R (crash-safe fence construction + startup reconciliation), P46-W2/W3 (the only valid fence lifters: idle/attempt-correlated completion, message-correlated failure, and verified/cancelled termination — a generic `session.error` never lifts one; a CAS loss after delivery otherwise creates an unfinalized fence no completion path can finalize, blocking every later reservation for the task) |
