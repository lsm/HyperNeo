...
| TAM-F2 | SPAWNER method-body delegation ONLY: the `spawnPostApprovalSubSession` method body delegates to the F1a pipeline (the B2 pattern — both in-file callers' signatures unchanged, no call-site change) AND owns the missing-MCP FENCE-PROPAGATION integration test (single atomically-landable owner). The ROUTER CALL-SITE SWAP is NOT this slice's — it is solely P47's in `space-runtime-tools-goals.md` (P47 depends on TAM-F2); the two docs are consistent on that ownership. Ordering expressed as dependencies, not combination: TAM-F2 may not merge before P47's prerequisites exist (it depends on the full P46 family and P53), and between TAM-F2 and P47 the delegated method holds its F1b fence continuations unsettleable-by-the-legacy-router — the legacy router path is NOT exercised with F1a live until P47's swap lands, because the delegated method is invoked by that legacy path; the acyclic order is therefore recorded as: TAM-F2 lands, P47's router swap lands immediately after in dependency order, and the Fence-PROPAGATION test gates the exposure (order F1a → S2 → TAM-F2 → P47) | wire | ≲40 | ≲60 | TAM-F1a, TAM-F1b, TAM-F1c, TAM-F1d, TAM-S2, TAM-DS, TAM-C2 (the message-handler fallback at space-task-message-handlers.ts:233–238 can otherwise still invoke the legacy restoration path while a reservation is held — the dead-A-versus-replacement-B race), P44 (dispatch-reservation primitive), P45, P46-R, P46-W2/W3 (the only valid fence lifters: idle/attempt-correlated completion, message-correlated failure, and verified/cancelled termination — a generic `session.error` never lifts one), P53 (P47's router swap requires the CAS-capable goal-terminal handler first) |
