...
The ladder below is the current 47-row table (pin / build / wire / delete /
extract) — 46 mandatory slices plus the optional TAM-H non-superpipe
extraction, which does not count toward the mandatory ladder; coordinators
cut child issues from that table, not from any earlier summary of it. The 46
...
| TAM-F1d | DURABLE OUTCOME ARTIFACTS + RESOURCE CLEANUP: the attempt→message correlation is DURABLE and REARMED/reconciled at startup (completion callbacks and late-settlement subscriptions are process-local while a DEFERRED kickoff plus its dispatch stamp survive a restart — a post-restart dead-letter must still conditionally clear the matching stamp, and P45's alreadyRouted gate must not block every retry while the worker stays live; deferred-consumption and deferred-failure restart rows, consumed by TAM-DS). RELEASE THE MODEL-POOL RESERVATION on every skip (the fresh arm reserves the slot before creation :4961 and a staged terminal-skip halt completes normally rather than entering the catch :5049–5051 — capacity-reuse assertion). The exported result CARRIES an owner-provided, SESSION-SCOPED CLEANUP OPERATION for the ACTIVATED assignment, exposed for EVERY activated ownership outcome (P46 invokes it for CREATED termination AND the VERIFIED COLD-RESTORED unwind; capacity-reuse tests covering CAS loss, another post-spawn failure, and the cold-unwind case). EXPOSES the post-spawn FLUSH CONTINUATION WITHOUT STARTING it — P46 invokes the flush ONLY after `record-dispatched-session` wins the CAS (CAS-loss/later-route-failure race row with pending rows present) | build | ≲40 | ≲60 | TAM-F1a, P44 (cross-plan: durable correlation and the crash-safe delivery fence use P44's dispatch-record schema and persistence API), TAM-LH |
| TAM-F2 | Wire `spawnPostApprovalSubSession` AND own the missing-MCP FENCE-PROPAGATION integration test IN THE SAME SLICE (the acyclic order is F1 → S2 → F2; S2 does NOT depend on F1; the separate TAM-F2-INT is REMOVED — keeping it created an F2↔F2-INT cycle and an unverified intermediate state, so the propagation test is a single atomically-landable owner in F2). ALSO gates on TAM-DS AND P46-R (the crash-safe PREPARED/ENQUEUED fence construction + startup reconciliation must exist before F2's adapter — a delivered kickoff enqueued and daemon-exit before the guarded stamp would otherwise leave a retry able to re-inject after reservation expiry). NO UNSETTLED-CONTINUATION INTERMEDIATE STATE — and NO EARLY-RELEASING ADAPTER: a wrapper at the spawner-return seam CANNOT settle the continuations correctly, because the legacy router (post-approval-router.ts:252–285) awaits `spawnPostApprovalSubSession` and only THEN writes `postApprovalSessionId` — a wrapper releasing at return would drop the fences before the stamp, letting a reassignment/status-departure send the kickoff to stale work. The ONE compliant transition: the router call site runs the COMPLETE P46 `post-approval-route` RUNNER — never a hybrid that invokes P46's internal spawn/dispatch stage group from the legacy imperative flow (a partial second composition for the same business operation that bypasses the already-built full pipeline). Because the router-side integration and P47's router swap target the same seam, they are COMBINED into ONE wiring PR (explicitly permitted here — the alternatives are worse: a temporary duplicate router, or a hybrid bypass); F2's spawner METHOD-BODY DELEGATION (the `spawnPostApprovalSubSession` body delegates to the F1a pipeline, the B2 pattern, both in-file callers' signatures unchanged) may land in the same PR or the immediately preceding one, since it changes no call site. The complete runner consumes the FULL F1a result (the three fence continuations after the guarded stamp, the POST-SPAWN FLUSH CONTINUATION on the success branch, and the ACTIVATED-ASSIGNMENT CLEANUP operation on CAS-loss/post-spawn failure branches; settling only the fences would leave pending rows undrained after a stamp win and leak model-pool capacity after a CAS loss); P47's residual scope after the combination is only deleting the legacy imperative route arms the runner replaced. The seam OWNS THE P44 LEASE LIFECYCLE while F1 is exposed before P46/P47: a BOUNDED SPAWN SHORTER THAN THE RESERVATION TTL with RENEWAL until the underlying F1 operation settles, or acknowledged cancellation plus ownership-aware late cleanup (merely calling `claimPostApprovalDispatch` does not satisfy P44's caller-side fencing — a spawn outliving the TTL lets a takeover dispatch a second worker); timeout/takeover race coverage included. The pre-P47 router (:209–257) otherwise performs only a liveness check then spawns WITHOUT claiming the reservation, so two concurrent route calls could both deliver and stamp afterward; include a race test for reassignment or status departure between spawner return and dispatch stamping | wire | ≲60 | ≲70 | TAM-F1a, TAM-F1b, TAM-F1c, TAM-F1d, TAM-S2, TAM-DS, TAM-C2 (the message-handler fallback at space-task-message-handlers.ts:233–238 can otherwise still invoke the legacy restoration path while F2's adapter holds a P44 reservation and creates replacement worker B — the dead-A-versus-replacement-B race), P44 (dispatch-reservation primitive), P45, P46-R (crash-safe fence construction + startup reconciliation), P46-W2/W3 (the only valid fence lifters: idle/attempt-correlated completion, message-correlated failure, and verified/cancelled termination — a generic `session.error` never lifts one; a CAS loss after delivery otherwise creates an unfinalized fence no completion path can finalize, blocking every later reservation for the task), P53 (cross-plan: the combined complete-router wiring this slice owns is exactly P47's router swap, and P47 requires P53's CAS-capable goal-terminal handler first — without it a concurrent cancellation can be overwritten as `done`; the router swap cannot go live before P53) |
...
encoded in BOTH ladders: P46 depends on TAM-F1a–F1d (outcome/fence/compensation/artifact contracts) and P47 on
