...
| TAM-F2W | SPAWNER-METHOD SWAP wire slice: the live `spawnPostApprovalSubSession` method body switches from the legacy implementation to the TAM-F2 dead-code delegated implementation (the single production enablement moment; with it, the F1b fence continuations become reachable and are settled by the caller P47's guarded stamp) | wire | ≲30 | ≲40 | TAM-F2, TAM-F2T |
