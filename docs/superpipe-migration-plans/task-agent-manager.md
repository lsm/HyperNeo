...
The ladder below is the current 51-row table (pin / build / wire / delete /
extract) — 50 mandatory slices plus the optional TAM-H non-superpipe
extraction, which does not count toward the mandatory ladder; coordinators
cut child issues from that table, not from any earlier summary of it. The 50
