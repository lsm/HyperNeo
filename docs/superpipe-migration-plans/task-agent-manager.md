...
| TAM-S2 | Wire `mcpSelfHeal` to the S1 pipeline; index/callback/displaced-session lifecycle effects stay class-owned; MUST NOT wire before S1's terminal-status fence (TAM-F0 + writers) is available; the S2→F1 dependency is DROPPED (F1 → B0 → B1 → LH → S2 would otherwise cycle B1→S2→F1→B0→B1), and the missing-MCP startup propagation test is OWNED BY TAM-F2 (S2 carries NO fence-propagation test); OWNS the post-S2 FENCE-PROPAGATION integration + test — the startup self-heal path reusing F1's held fences (the TAM-F0 terminal fence AND the TAM-OW nonterminal ownership fence — when F1 uses the fence-based implementation it holds BOTH while B1's startStreamingQuery invokes the missing-MCP self-heal callback, and S1 must not wait forever trying to acquire the same non-reentrant ownership fence; alternatively the reversible-adoption alternative is explicitly tested) is verified HERE, so TAM-LH/TAM-OW's stated contracts are completable and S2 cannot be wired without proving self-heal reuses the held fences, and F2 cannot expose the startup path before this (a separate wiring slice may be cut if S2 grows past budget) | wire | ≲40 | ≲50 | TAM-S1 (no TAM-F1 dependency — the F1→B0→B1→LH→S2 path would otherwise cycle B1→S2→F1→B0→B1) |
