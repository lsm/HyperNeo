...
| TAM-DS | DAEMON-STARTUP RECONCILIATION for deferred settlement tracking: restore the attempt→message correlation and late-settlement subscriptions for DEFERRED kickoffs whose dispatch stamp survived a restart (the process-local tracking is gone, so a later consumption/dead-letter cannot clear the matching stamp and retries stay blocked at alreadyRouted) — F1 is an unwired build and F2 wires only the spawner, so this startup wiring is its own slice, and the router must not be exposed before it (deferred-consumption and deferred-failure restart rows) | wire | ≲40 | ≲60 | TAM-F1 |
