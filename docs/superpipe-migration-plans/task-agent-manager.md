...
| TAM-F2 | Wire `spawnPostApprovalSubSession`; MUST NOT wire the F1 spawner before TAM-S2 lands the fence-propagation (a missing-MCP startup enters self-heal while F1 holds the TAM-F0 fence, and the same-handle propagation preventing the nested-acquisition deadlock is S2's) — sequence F1 → S2 → F2, with S2 depending on F1 so its F1-integration test can run; ALSO gates on TAM-DS (the router/spawner path must not be exposed before daemon-startup reconciliation of deferred settlement tracking). NO UNSETTLED-CONTINUATION INTERMEDIATE STATE: F2 swaps the live method to F1 while the PRE-P47 router (post-approval-router.ts:252–285) still destructures only `sessionId` and stamps on its own — that router never settles the three fence continuations F1 deliberately leaves held, so any successful dispatch in the intermediate state permanently blocks later cancellation/reassignment/workspace/approved-status writers; F2 therefore ships a COMPATIBILITY WRAPPER (the legacy-returning seam settles the continuations itself), or the spawner and router call-site swaps are COMBINED/ORDERED so the intermediate deployment never exists | wire | ≲40 | ≲50 | TAM-F1, TAM-S2, TAM-DS |
