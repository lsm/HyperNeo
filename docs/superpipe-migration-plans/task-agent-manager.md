...
| TAM-LH | Held INJECTION-LOCK-HANDLE handoff API for startup self-heal (additive): `mcpSelfHeal`/`withSessionInjectLock` accept an already-held handle so `startStreamingQuery`'s `onMissingWorkflowMcpServers` path (query-runner.ts:793–796 → mcpSelfHeal → withSessionInjectLock :4252) does not deadlock against an owner's held reset-coordination lock — with the missing-MCP startup test; lands BEFORE any create pipeline wires the locked `startStreamingQuery` path | build | ≲50 | ≲80 | — |
