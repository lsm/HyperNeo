...
| TAM-B3 | Spawn-flow stage-list swap ONLY: split the `createSpawnedSession` dependency (spawn-flow.ts :281's monolithic `Promise<string>` seam) into preparation inputs + the create stage group and compose those stages into the owning stage list, consuming the TAM-B3C compensation adapter — the interface change plus the isolated swap; no compensation logic is authored here | wire | ≲50 | ≲60 | TAM-B1, TAM-B0, TAM-B3C |
...
| TAM-F2 | ADDITIVE dead-code construction (kind = build, not wire): the F1a-delegated spawner implementation merges as a SEPARATE, UNREACHABLE-FROM-PRODUCTION function beside the legacy method — no production path changes in this slice; the LIVE method keeps the legacy body. The production wiring lands as TWO independently shippable wire slices — the SPAWNER-METHOD swap is TAM-F2W's (its SELF-SETTLING adapter settles every continuation itself, guarded stamp + release, when no consumer has registered; P44's durable fence/lease is the backstop, so landing F2W alone strands nothing) and the ROUTER call-site swap is P47's (it registers as the continuation consumer before invoking, deactivating the adapter fallback in the same registration check). No atomic combination, no stranded-fence window. (The fence-propagation test is TAM-F2T's, not this slice's.) | build | ≲40 | ≲60 | TAM-F1a, TAM-F1b, TAM-F1c, TAM-F1d, TAM-F1e, TAM-F1f, TAM-DS, TAM-C2 (the message-handler fallback at space-task-message-handlers.ts:233–238 can otherwise still invoke the legacy restoration path while a reservation is held — the dead-A-versus-replacement-B race), P44 (dispatch-reservation primitive), P45, P46-R, P46-W2/W3 (the only valid fence lifters: idle/attempt-correlated completion, message-correlated failure, and verified/cancelled termination — a generic `session.error` never lifts one), P53 (P47's router swap requires the CAS-capable goal-terminal handler first) |
...
| TAM-F2W | SPAWNER-METHOD SWAP wire slice: the live `spawnPostApprovalSubSession` body switches to the delegated implementation whose SELF-SETTLING adapter settles every continuation (guarded stamp + release) when no consumer has registered — independently shippable, no stranded fence (the adapter settles; P44's durable fence is the backstop) | wire | ≲40 | ≲50 | TAM-F2, TAM-F2T |
