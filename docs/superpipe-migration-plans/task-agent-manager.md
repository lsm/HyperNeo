...
The ladder below is the current 43-row table (pin / build / wire / delete /
extract) — 42 mandatory slices plus the optional TAM-H non-superpipe
extraction, which does not count toward the mandatory ladder; coordinators
cut child issues from that table, not from any earlier summary of it. The 42
...
| TAM-F2 | Wire `spawnPostApprovalSubSession`; MUST NOT wire the F1 spawner before TAM-S2 lands the fence-propagation (a missing-MCP startup enters self-heal while F1 holds the TAM-F0 fence, and the same-handle propagation preventing the nested-acquisition deadlock is S2's) — sequence F1 → S2 → F2, with S2 depending on F1 so its F1-integration test can run; ALSO gates on TAM-DS (the router/spawner path must not be exposed before daemon-startup reconciliation of deferred settlement tracking). NO UNSETTLED-CONTINUATION INTERMEDIATE STATE — and NO EARLY-RELEASING ADAPTER: a wrapper at the spawner-return seam CANNOT settle the continuations correctly, because the legacy router (post-approval-router.ts:252–285) awaits `spawnPostApprovalSubSession` and only THEN writes `postApprovalSessionId` — a wrapper releasing at return would drop the fences before the stamp, letting a reassignment/status-departure send the kickoff to stale work. The only viable transitions: F2 ALSO updates the router call site to consume and settle the continuations AFTER its dispatch stamp (one combined spawner+router-consumer swap), the spawner and P45/P46/P47 RESERVATION + router integration deploy ATOMICALLY, or the compatibility seam CLAIMS P44's RESERVATION and settles it before exposing F1 — the pre-P47 router (:209–257) performs only a liveness check then spawns WITHOUT claiming the reservation, so two concurrent route calls can both observe no session, both deliver a kickoff, and only stamp afterward even with correctly-retained continuations; the seam must not merely own the post-spawn stamp. Include a race test for reassignment or status departure between spawner return and dispatch stamping | wire | ≲40 | ≲50 | TAM-F1, TAM-S2, TAM-DS, P44/P45 (dispatch-reservation path) |
