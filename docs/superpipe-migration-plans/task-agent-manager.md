...
| TAM-B0 | Add RESTORATION-OWNER provenance WITHOUT changing the stable `rehydrateSubSession` return type (it stays `AgentSession | null` — after D2 it is live at five call sites and `createSubSession`/`getSubSessionByAgentName` consume it directly): a provenance-returning companion helper/stage wraps the existing call; extend the B1 create-stage result to carry the THREE-WAY ownership (CREATED / LIVE-REUSED / COLD-RESTORED) — landed as its own slice (after the rehydration rewire so it doesn't conflict, before B2/B3 and F1 which consume it), NOT hidden inside TAM-F1. TAM-B0 ALSO OWNS the PRE-RETURN COLD-RESTORATION FAILURE compensation — the provenance-aware unwind inside the create group when a cold restoration succeeds but a later create-stage operation rejects before `createSubSession` returns: roll back ONLY bindings won by that attempt, stop/unregister the restored worker ONLY when no successful concurrent caller retains it, and PRESERVE the durable session (tests land with B0, so this path is implemented before any wiring slice can expose it) | build | ≲110 | ≲160 | TAM-B1, TAM-D2 |
