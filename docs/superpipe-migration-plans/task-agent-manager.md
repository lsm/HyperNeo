...
| TAM-S1 | `self-heal-node-agent` stagedRun, unwired (prologue gates via the TAM-FH fenced repair helper + adoption admission composed in front of the existing heal stages; heal workspace routed through `resolveTaskWorkspace`; adoption at the manager-owned pre-heal boundary) | build | ≲140 | ≲120 | TAM-PS, TAM-FH, TAM-E1 (acquire/release lock-handle protocol via TAM-LH) |
