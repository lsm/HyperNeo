...
The ladder below is the current 52-row table (pin / build / wire / delete /
extract) — 51 mandatory slices plus the optional TAM-H non-superpipe
extraction, which does not count toward the mandatory ladder; coordinators
cut child issues from that table, not from any earlier summary of it. The 51
...
and healing: cancellation/archive/stop/run-cancel do NOT change
...
workflow-run check passes too) — revalidate terminal status under the fence
spanning adoption + healing, or make adoption reversible, with a
cancellation-during-space-load/adoption race test. Three fences: (a) the prologue's
...
| TAM-LH | The COMPLETE shared INJECTION-LOCK-HANDLE contract (additive): the class-owned acquire/release handle over `withSessionInjectLock`/`withSessionResetCoordination` (which today expose only the callback-form lock, task-agent-manager.ts:1876–1877) PLUS the held-handle acceptance so `mcpSelfHeal` and `startStreamingQuery`'s `onMissingWorkflowMcpServers` path (query-runner.ts:793–796 → mcpSelfHeal :4252) do not deadlock against an owner's held handle — TAM-LH lands ONLY primitive-level tests (acquire/release, stale-handle detection/reacquisition, held-handle acceptance); the missing-MCP STARTUP INTEGRATION test belongs to TAM-F2T post-S2 (TAM-F2 is delegation-only dead code), not here. TAM-LH OWNS this contract so E1 (acquire/release), S1 (self-heal), and the create pipelines (held startStreamingQuery) all consume the SAME primitive rather than reworking each other's API; E1 depends on it. The held handle must be RETIRED safely: `onMissingWorkflowMcpServers` stays stored on the AgentSession and `QueryRunner` invokes it on later starts/restarts — if that persistent callback captures the owner's held handle, a post-release self-heal would bypass coordination with a stale capability; the handle must DETECT release and reacquire normally, or the callback is replaced before release, and a second post-start invocation is tested. The held-handle protocol EXTENDS TO THE TAM-F0 TERMINAL FENCE: when F1 holds the task/run fence through B1 creation and `startStreamingQuery` invokes the missing-MCP self-heal callback, the wired S1 flow must acquire THAT SAME fence across adoption+healing — TAM-LH alone makes only the injection lock held-aware, so the fence handle is propagated into S1 (or a release/reacquire sequence preserves the terminal race guarantee), and the missing-MCP startup integration row is owned by TAM-F2 (post-S2) — TAM-LH lands only PRIMITIVE-LEVEL coverage (acquire/release, stale-handle detection, held-handle acceptance), never the post-S2 startup integration | build | ≲120 | ≲160 | TAM-F0 (the terminal-fence handle propagation is part of this contract, so the TAM-F0 primitive must exist first). Budget raised to match the slice's true scope — coordination acquisition, stale-handle detection, self-heal propagation, AND terminal fencing can't safely fit a 70-line build) |
...
| TAM-F2 | ADDITIVE dead-code construction (kind = build, not wire): the F1a-delegated spawner implementation merges as a SEPARATE, UNREACHABLE-FROM-PRODUCTION function beside the legacy method — no production path changes in this slice; the LIVE method keeps the legacy body, and the production wiring lands as TWO independently shippable wire slices (the SPAWNER-METHOD swap is TAM-F2W's; the ROUTER call-site swap is P47's — ownership is unambiguous) with NO stranded-fence window, via a SELF-SETTLING compatibility boundary:
  - **TAM-F2W (spawner-method swap)**: the live `spawnPostApprovalSubSession` body switches to the delegated implementation, which exposes the full F1a result to continuation CONSUMERS but SETTLES EVERY CONTINUATION ITSELF when no consumer has registered (method-level settlement holds the fences through its own P44-reservation-scoped guarded stamp, then releases — the same settlement P46 performs; the durable P44 fence/lease is the backstop, so the in-process continuations are never the only representation). Landing this alone is safe: the legacy router observes the legacy `{ sessionId }` shape and the fences are still settled, by the adapter.
  - **P47 (router call-site swap)**: the router call site registers itself as the continuation consumer BEFORE invoking, then performs its guarded stamp and settles the continuations itself; the method-level settlement is bypassed only when a consumer is registered, so activation of the consumer and deactivation of the adapter fallback happen in the same registration check.
  The two seams never conflict and no intermediate deployment strands a fence: before P47 the adapter settles; after P47 the router settles. The ROUTER CALL-SITE SWAP is NOT this slice's — it is solely P47's in `space-runtime-tools-goals.md` (P47 depends on TAM-F2); the two docs are consistent on that ownership. Ordering expressed as dependencies, not combination: TAM-F2 may not merge before P47's prerequisites exist (it depends on the full P46 family and P53), and NO STRANDED-CONTINUATION INTERMEDIATE STATE — ENFORCEABLE WITHOUT FLAG SEMANTICS: the live `spawnPostApprovalSubSession` method KEEPS the legacy implementation; TAM-F2 merges the F1a-delegated implementation as a SEPARATE, UNREACHABLE-FROM-PRODUCTION function (dead code beside it, invoked only by TAM-F2's tests). No router path, flag state, or intermediate merge can reach the delegated body; "not exercised" is enforced structurally, not by convention. P47's call-site swap replaces the method body AND the router call site in the SAME change (the single moment the delegated path goes live with a caller that settles the complete result). The fence-propagation test exercises the dead-code delegated path directly; exposure is gated by P47 alone (order F1a → S2 → TAM-F2 → P47) | build | ≲40 | ≲60 | TAM-F1a, TAM-F1b, TAM-F1c, TAM-F1d, TAM-F1e, TAM-F1f, TAM-S2, TAM-DS, TAM-C2 (the message-handler fallback at space-task-message-handlers.ts:233–238 can otherwise still invoke the legacy restoration path while a reservation is held — the dead-A-versus-replacement-B race), P44 (dispatch-reservation primitive), P45, P46-R, P46-W2/W3 (the only valid fence lifters: idle/attempt-correlated completion, message-correlated failure, and verified/cancelled termination — a generic `session.error` never lifts one), P53 (P47's router swap requires the CAS-capable goal-terminal handler first) |
...
| TAM-S1 | `self-heal-node-agent` stagedRun, unwired (prologue gates via the TAM-FH fenced repair helper + adoption admission composed in front of the existing heal stages; heal workspace routed through `resolveTaskWorkspace`; adoption at the manager-owned pre-heal boundary) | build | ≲140 | ≲120 | TAM-PS, TAM-FH, TAM-LH (shared lock-handle contract — S1 consumes no E1 delivery behavior, so it depends on the primitive directly, not the delivery build), TAM-F0 + the terminal-status writers (TAM-F0-W1..W6), TAM-OW + TAM-OW-W1/W2 (nonterminal ownership fenced across adoption + healing — a workflowRunId/workspace reassignment after adoption while the heal tail awaits MCP reinjection/restart must not leave the adopted session canonical for the old run; held-handle propagation for the nested F1 startup-self-heal path, or reversible adoption + final ownership check). S1 carries the SELF-HEAL LOCK BOUNDARY: displaced-session adoption finishes BEFORE acquisition, only the heal tail runs while the handle is held, and successful completion EXPLICITLY releases (compensations do not unwind on success) — with a waiter test; wiring the whole runner under the callback-form lock or omitting the success release would move adoption under coordination or permanently block later injection/reset callers. S1 CARRIES THE TERMINAL-STATUS RACE CONTRACT: cancellation/archive/stop/run-cancel must be FENCED across adoption and healing (via TAM-F0 + TAM-F0-W1..W6 dependencies), OR adoption is fully reversible — otherwise a terminal transition completing during the space load or after the ownership check lets adoption replace the manager indexes, cache registration, and completion callback after terminal cleanup has run (the heal tail's workflow-run check passes too), reviving a worker for terminal work; pin the cancellation-during-space-load/adoption race test, and TAM-S2 MUST NOT wire the pipeline until this protocol is available |
| TAM-S2 | Wire `mcpSelfHeal` to the S1 pipeline; index/callback/displaced-session lifecycle effects stay class-owned; MUST NOT wire before S1's terminal-status fence (TAM-F0 + writers) is available; the S2→F1 dependency is DROPPED (F1 → B0 → B1 → LH → S2 would otherwise cycle B1→S2→F1→B0→B1), and the missing-MCP startup propagation test is OWNED BY TAM-F2T alone (S2 carries NO fence-propagation test — the startup self-heal path reusing F1's held fences is verified in F2, which owns that integration; S1's contract to not wait forever on the non-reentrant fence is met by TAM-LH's held-handle propagation, which F2's test exercises) | wire | ≲40 | ≲50 | TAM-S1 (no TAM-F1 dependency — the F1→B0→B1→LH→S2 path would otherwise cycle B1→S2→F1a→B0→B1) |
