...
| TAM-F2 | SPAWNER method-body delegation ONLY: the `spawnPostApprovalSubSession` method body delegates to the F1a pipeline (the B2 pattern — both in-file callers' signatures unchanged, no call-site change) AND owns the missing-MCP FENCE-PROPAGATION integration test (single atomically-landable owner). The ROUTER CALL-SITE SWAP is NOT this slice's — it is solely P47's in `space-runtime-tools-goals.md` (P47 depends on TAM-F2); the two docs are consistent on that ownership. Ordering expressed as dependencies, not combination: TAM-F2 may not merge before P47's prerequisites exist (it depends on the full P46 family and P53), and NO STRANDED-CONTINUATION INTERMEDIATE STATE: the delegation lands INERT — the F1a path exists behind a default-off rollout flag (or equivalent mechanism) while the OLD `spawnPostApprovalSubSession` implementation stays active, so the legacy router never consumes the F1 outcome contract until P47's call-site swap (which both swaps the call site AND enables the delegation) turns it live in the SAME change; no merged state delegates to F1 while the legacy router still consumes the old sessionId-only contract. The fence-propagation test in TAM-F2 exercises the delegated path directly while it is inert (never through the live router), so exposure is gated by P47 alone (order F1a → S2 → TAM-F2 → P47, with the activation inside P47) | wire | ≲40 | ≲60 | TAM-F1a, TAM-F1b, TAM-F1c, TAM-F1d, TAM-S2, TAM-DS, TAM-C2 (the message-handler fallback at space-task-message-handlers.ts:233–238 can otherwise still invoke the legacy restoration path while a reservation is held — the dead-A-versus-replacement-B race), P44 (dispatch-reservation primitive), P45, P46-R, P46-W2/W3 (the only valid fence lifters: idle/attempt-correlated completion, message-correlated failure, and verified/cancelled termination — a generic `session.error` never lifts one), P53 (P47's router swap requires the CAS-capable goal-terminal handler first) |
