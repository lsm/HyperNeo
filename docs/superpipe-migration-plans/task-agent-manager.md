...
The ladder below is the current 41-row table (pin / build / wire / delete /
extract) — 40 mandatory slices plus the optional TAM-H non-superpipe
extraction, which does not count toward the mandatory ladder; coordinators
cut child issues from that table, not from any earlier summary of it. The 40
