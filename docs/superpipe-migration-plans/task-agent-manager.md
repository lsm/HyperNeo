...
(5,065 lines; 365 `if`/`else if` arms + 74 ternaries ≈ 439 measured arms,
...
The ladder below is the current 54-row table (pin / build / wire / delete /
extract) — 53 mandatory slices plus the optional TAM-H non-superpipe
extraction, which does not count toward the mandatory ladder; coordinators
cut child issues from that table, not from any earlier summary of it. The 53
...
ladder before the coordinator cuts child issues. The build slices form a
serial dependency graph (pins, shared primitives, and wired fence/writer
slices precede the pipelines that consume them) — build slices are NOT
independent of one another, and the dependency cells below are authoritative
for scheduling.
...
restart would double-spawn the worker. A FAILED cold restoration falls through to the
fresh arm, but the failure is TYPED before the stale-binding repair:
`rehydrateSubSession` returns null not only for a missing/archived durable
session but also for a missing execution, missing parent task, terminal task
status, missing space, and a cancelled run (:3486–3585), and treating every
null as a stale binding clears the old session ID so the fresh arm binds and
streams a replacement during those REFUSAL cases (the task-cancel race) —
the repair applies only to the proven missing/archived case (or the relevant
admissions re-run first), with terminal/missing-space rows proving no fresh
worker is created. Where the failure IS a proven stale binding, the fresh
bind (:1343–1376) sees the OLD non-null binding
...
directly — `spawn-post-approval-worker` via TAM-F1a, and the spawn flow's
...
seam from TAM-E2), and TAM-F1a/F2
...
so TAM-F1a composes B1's create stages in the NOMINAL FRESH ARM ONLY — running
...
the P46 box is explicitly THREE-STATE — DELIVERED/SKIPPED/DEFERRED; collapsing DEFERRED into delivered lets an unrelated completion lift its fence, collapsing it into skipped takes terminal-release handling instead of message-specific tracking) and KICKOFF DELIVERED vs SKIPPED vs DEFERRED (the terminal-gate
skip) — its CAS-loss compensation and deadline cleanup run for sessions the attempt created AND apply the COLD-RESTORED unwind-or-fence protocol (a timed-out spawn that later settles as COLD-RESTORED+DELIVERED must not keep processing the kickoff unfenced after lease expiry), and a skipped reuse routes to release/terminal handling.
...
post-delivery failure, not just CAS loss: when TAM-F1a returns REUSED or
...
is left untouched. Because TAM-F1a composes the create stages directly, the
...
reuse would still yield a skip after mutation. TAM-F1a/F1c must close that race
...
reuse finishes; terminal during rehydration) land TOGETHER in TAM-F1a —
...
failure; the concurrent-join row lands with TAM-F1c). An owner token alone is
...
owner-fails-joiner-succeeds race row lands with TAM-F1c). The reference
tracker SPANS concurrent TAM-F1a runs, so it is MANAGER-OWNED RESOURCE STATE
...
three outcomes and the execution-row land with TAM-F1c). The protocol
...
TAM-F1a's protocol (TAM-PF keeps only surviving behavior). Pins exist
...
with TAM-G2's primitive (the atomic claim), not the later combined TAM-G
pipeline slice. Fit:
...
sequence — `getById(...).status === 'pending'` followed by `markDelivered`
(:1553–1555, whose SQL is already `WHERE id = ? AND status = 'pending'`,
pending-agent-message-repository.ts:230–242, so a concurrent failure makes the
write a no-op rather than overwriting) — has the REAL defect that the watcher
UNCONDITIONALLY emits `emitPendingDelivered` after that no-op: the watcher
must be REWIRED so the settlement reports whether its conditional write WON
and the event fires ONLY on a win (no repository-semantics change). A dedicated wiring
...
and test it explicitly as a behavior change). Ownership must be REVALIDATED
IMMEDIATELY BEFORE ADOPTION: the parent task can be reassigned while the
prologue awaits `getSpace`, and adoption from the stale execution/task
snapshot replaces the manager indexes, registration, and completion callback
before the heal tail re-reads the task (:4265) — that later check throws on
the run mismatch but adoption is class-owned with no compensation, leaving the
wrong session canonical after the failed heal; revalidate task/execution
ownership before adoption or make adoption reversible, and pin reassignment
during the space lookup. TERMINAL STATUS must also be fenced across adoption
and healing: cancellation/archive/stop/run-cancel/block do NOT change
`workflowRunId` or execution ownership that the ownership revalidation checks,
so a terminal transition completing while `getSpace` is pending (or right after
the check) lets adoption replace the manager indexes, cache registration, and
completion callback after terminal cleanup has run (the heal tail's
workflow-run check passes too) — and `blocked` is the dangerous one: it is
TERMINAL (`isTerminalTaskStatus`), reachable DIRECTLY from `in_progress`, and
the dependency-added block path CANCELS the task's agents
(`stopBlockedWorkflowTask` cancels every non-idle execution session and
`taskAgentSessionId`, space-runtime.ts:2887–2928), so a block completing during
the space load cancels the very session self-heal would adopt and restart —
revalidate terminal status under the fence spanning adoption + healing, or make
adoption reversible, with cancellation- and block-during-space-load/adoption
race tests. Three fences: (a) the prologue's
...
| TAM-PG | Pins for `deliverSpaceAgentPendingRow` restricted to SURVIVING current outcomes: settlement/error behavior AS IT BEHAVES TODAY, injector-rejection locality (later rows still delivered), near-expiry deferral cases — the status-safe race contract CANNOT land here (the primitives arrive with TAM-G1/TAM-G2) | test-only | 0 | ≲60 | — |
...
| TAM-B1 | `create-node-agent-sub-session` stagedRun, unwired (gates + both arms + compensations + session-guarded stale-owner CAS with concurrency pin), with the CREATE-STAGE LOCK HANDOFF in the contract: the group is invoked by F1 while the owner ALREADY HOLDS the non-reentrant injection lock (unconditional acquisition would deadlock) and by B2/B3 without an owner-held handle (standalone acquisition) — the group accepts an already-held-or-acquire handle with EXPLICIT normal/error release — but B1 RELEASES ONLY SELF-ACQUIRED handles: a handle BORROWED from the caller (F1's already-held handle) stays HELD for the caller (F1's post-create ownership/terminal rechecks and E1 delivery run under it; releasing it in the group would reopen the terminal-transition race), and both paths are tested; BOTH PRIMARY binds (reuse :1148–1157, fresh :1347–1356) pass `expectAgentSessionId` with the observed session (`null` for a fresh target) with same-status foreign-bind race rows; the FAILED-RESTORATION stale-binding repair (status-and-session-guarded, applied ONLY to the TYPED proven missing/archived outcome — a bare null also covers refusal states (missing execution/parent task, terminal task, missing space, cancelled run :3486–3585) whose fresh-arm entry would start a replacement for invalid work; a typed result or re-run admissions gates the repair) with its retry/orphan and terminal/missing-space no-fresh-worker tests; adds a CREATED-only compensation for STREAMING-STARTUP rejection — when `startStreamingQuery` rejects (:1389) the session is already registered/indexed (and, non-deferred, execution-bound), yet `createSubSession` throws BEFORE returning an ownership result, so neither the spawn flow (which records `attempt.sessionId` only after the awaited create returns, spawn-flow.ts:281) nor TAM-F1's post-create cleanup can see it. Cleanup is COMPLETE and ownership-safe: stop/clean the owned instance (it was persisted by `AgentSession.fromInit` and may have started queue/process/timer resources), remove ALL TaskAgentManager and SessionManager registrations, clear only a matching execution binding, AND delete the newly created durable session through the repository cleanup path — an incomplete cleanup leaks durable rows and suffixed-ID retries; both deferred- and normal-bind STARTUP-FAILURE tests land WITH TAM-B1 (asserting no cached/indexed session, live resources, execution binding, or sessions row remains, and that a retry reuses the base session ID) — they cannot pass in the surviving-behavior pin slice | build | ≲250 | ≲250 | TAM-PB, TAM-LH (held-aware self-heal primitive for the missing-MCP startup row) |
| TAM-B3C | OWNERSHIP-AWARE COMPENSATION ADAPTER (additive build): the three-way ownership-aware spawn-flow compensation protocol — CREATED cancel, COLD-RESTORED unwind gated on no-successful-joiner, LIVE-REUSED intact — constructed and tested in isolation against the B0 provenance result; nothing calls it yet | build | ≲60 | ≲80 | TAM-B0, TAM-B1 |
| TAM-B3 | Spawn-flow stage-list swap ONLY: split the `createSpawnedSession` dependency (spawn-flow.ts :281's monolithic `Promise<string>` seam) into preparation inputs + the create stage group and compose those stages into the owning stage list, consuming the TAM-B3C compensation adapter — the interface change plus the isolated swap; no compensation logic is authored here; B3 MUST NOT wire the live spawn flow before TAM-LH2's callback-propagation contract is available: B1 holds the non-reentrant injection lock through `startStreamingQuery`, and a missing workflow MCP discovered at startup would send the awaited `onMissingWorkflowMcpServers` callback through the unpropagated boundary (a deadlock against the held lock) or store a callback carrying a released capability for a later restart — B2 inherits this ordering through B3 | wire | ≲50 | ≲60 | TAM-B1, TAM-B0, TAM-B3C, TAM-LH2 (the stored-callback retirement/held-handle propagation must precede the first live wiring of the create group) |
...
| TAM-LH | The shared INJECTION-LOCK-HANDLE contract (additive, PRIMITIVE-ONLY): the class-owned acquire/release handle over `withSessionInjectLock`/`withSessionResetCoordination` (which today expose only the callback-form lock, task-agent-manager.ts:1876–1877) PLUS the held-handle acceptance so `mcpSelfHeal` and `startStreamingQuery`'s `onMissingWorkflowMcpServers` path (query-runner.ts:793–796 → mcpSelfHeal :4252) do not deadlock against an owner's held handle; tests are PRIMITIVE-LEVEL ONLY (acquire/release, stale-handle detection/reacquisition, held-handle acceptance) — the PERSISTENT-CALLBACK and TAM-F0 fence-handle PROPAGATION contracts are TAM-LH2's seam (a dependent slice), and the post-S2 missing-MCP startup INTEGRATION row is TAM-F2T's, never this slice's; TAM-LH OWNS this API so E1 (acquire/release), S1 (self-heal), and the create pipelines (held startStreamingQuery) all consume the SAME primitive rather than reworking each other's API; E1 depends on it | build | ≲80 | ≲100 | — |
| TAM-LH2 | HELD-HANDLE PROPAGATION contract (additive build — the ONE seam carrying held handles across the persistent missing-MCP self-heal boundary; split out of TAM-LH so the injection-lock API lands as a pure primitive; the row's two ordered contracts below are its internal construction sequence within that single seam): (a) the PERSISTENT SELF-HEAL CALLBACK retirement protocol — `onMissingWorkflowMcpServers` stays stored on the AgentSession and `QueryRunner` invokes it on later starts/restarts, so the stored callback must not capture the owner's released handle (the capability DETECTS release and reacquires normally, or the callback is replaced before release; a second post-start invocation is tested); (b) the TAM-F0 TERMINAL-FENCE propagation — when F1 holds the task/run fence through B1 creation and `startStreamingQuery` invokes the missing-MCP self-heal callback, the self-heal flow must acquire THAT SAME fence across adoption+healing (or a release/reacquire sequence preserves the terminal race guarantee), because TAM-LH alone makes only the injection lock held-aware; protocol-level tests for BOTH handle kinds land here, while the post-S2 startup INTEGRATION row stays TAM-F2T's | build | ≲50 | ≲70 | TAM-LH, TAM-F0 |
...
| TAM-C1 | `restore-post-approval-worker` stagedRun, unwired (pre-stream registration boundary), with a class-owned RESTORE-LOCK HANDLE protocol: the public index FAST PATH (:2177) JOINS the fenced protocol — the reservation/durable-identity/workflow-run/workspace/terminal checks run on a cache hit too (or the fast return is moved after them): in the dead-A/replacement-B race, F1a can probe A dead and reserve a replacement, another recovery can index A, and a concurrent restore would otherwise return A immediately without observing the live P44 reservation, handing callers A while B is created; the fast path never bypasses the refusal/revalidation checks and the in-lock index recheck stays between acquire and release — `stagedRun` cannot suspend around the `withSessionRestoreLock` callback, so the lock is an acquire/release effect-stage pair (class supplies the handle); pin public-check-before-acquire + in-lock-recheck ordering. Because `stagedRun` does NOT unwind compensations on successful halts, EVERY successful path after acquisition — the in-lock cache hit and all refusal gates — must reach an EXPLICIT release effect before halting (compensation covers errors only); pin lock release on normal early outcomes and failures, or a post-lock refusal leaves every later restoration for that session blocked. Concurrent restores amplify this: a second caller can acquire the handle, hit the in-lock recheck, and complete — pin a THIRD waiter released after the successful recheck/refusal cases. The in-lock recheck must ALSO re-read the DURABLE identity: `postApprovalSessionId` may change while restoration is queued or awaiting `getSpace` (the identity was captured at :2174), and a concurrent dispatch can record session B while C1 restores stale session A — add an in-lock durable-identity re-read that must still match the requested session, and pin the identity-change race rather than testing only the cache recheck — and ORDER that identity re-read AFTER the LAST PRE-REGISTRATION AWAIT as well (re-reading immediately after lock acquisition still lets a concurrent dispatch change `postApprovalSessionId` while C1 awaits `getSpace`/`ensureNodeAgentAttached`, after which C1 registers/starts stale session A for a task now assigned session B); pin the identity-change-during-attachment race. C1 must EXCLUDE RESTORING A WORKER WHILE A DISPATCH IS RESERVED: a P44 dispatch reservation does not change `postApprovalSessionId`, so C1 can reread the same dead session A and pass every identity check while F1 is creating replacement B — if C1's liveness snapshot precedes F1's registration and C1 crosses its registration boundary before P46 stamps B, both A and B are registered/streaming and the later stamp orphans A; C1 rejects or JOINS a live dispatch reservation, or serializes restoration with the reservation/spawn lifecycle, and the dead-A-versus-replacement-dispatch interleaving is pinned. The final reread must ALSO match the task's ORIGINAL `workflowRunId`: `UpdateSpaceTaskParams` permits `workflowRunId` changes (space.ts:692) and the generic update path does not clear the post-approval identity, so the durable-identity reread can return the same `postApprovalSessionId` while the run changed — C1 would register a worker configured from the stale run's workflow/prompt/MCP context; match the original `workflowRunId` as well, and pin reassignment-during-attachment. The WORKSPACE must be re-resolved and compared after the FINAL await: restoration entered through `hintSessionId` or the `done`-task durable-session fallback can have no `postApprovalSessionId`, and `updateTask` permits a non-active task's workspace to change during the attachment/startup/replay awaits — the identity/run/terminal checks alone would return a worker whose metadata, MCP server, and prompt were prepared for the OLD repository; re-resolve and compare with ownership-safe rollback on refusal, or fence workspace mutation across the complete restore. OWNERSHIP MUST ALSO BE FENCED THROUGH STREAMING STARTUP AND REPLAY: `startStreamingQuery` and pending-message replay await AFTER the registration boundary, so a `workflowRunId` change during either await passes the pre-registration reread and leaves the old-run worker installed in the manager indexes and SessionManager (the reassignment does not clear the unchanged post-approval identity) — fence ownership through startup/replay, or perform an ownership-safe rollback after a FINAL POST-START reread. When the FINAL identity check REFUSES, the restore path has ALREADY mutated session A's persisted workspace and its prompt, guards, skills, and runtime MCP configuration (:2246–2294) — the late-refusal path must RESTORE the captured session state, or identity changes must be FENCED across those effects (especially damaging for the cached branch, where the globally-owned cached session remains registered with the stale mutations even though C1 returns a refusal); pin the cached-session race. The SAME ROLLBACK/FENCING applies to TERMINAL-STATUS refusals: cancellation during attachment is a separate refusal path that has already mutated the cached session's workspace/prompt/guards/skills/MCP config (:2246–2292), and cancellation cleanup cannot find the not-yet-indexed session — restore the captured state or fence through the terminal reread too, and pin cached cancellation-during-attachment. TASK/RUN TERMINAL STATUS must be re-read AFTER the last pre-registration await: `getSpace` and `ensureNodeAgentAttached` both await before the worker is indexed/registered, so a cancellation during either await is invisible to cancellation cleanup and C1 could register/start a worker for the cancelled task (the restore lock does not coordinate status writers) — re-read terminal status after the final pre-registration await (or hold the TAM-F0 terminal fence) and pin cancellation-during-attachment | build | ≲180 | ≲150 | TAM-PC, P44 (post-approval dispatch reservation contract — C1 inspects or joins a live reservation, so it cannot be implemented before P44's reservation primitive exists; a CROSS-PLAN dependency reflected in both ladders, and C2 must not wire restoration before that support is available) |
| TAM-C2 | Wire `restorePostApprovalWorkerSession`; MUST NOT wire restoration before the P44 reservation support C1 inspects is available | wire | ≲40 | ≲50 | TAM-C1 |
| TAM-D1 | `rehydrate-sub-session` stagedRun, unwired (`resolveTaskWorkspace` routing, system-prompt-only overlay), consuming the TAM-FH fenced repair helper; CARRIES the CACHED-SESSION OWNERSHIP decision: the cached branch selects an existing SessionManager instance (:3568–3570) and currently registers it unconditionally (:3652) and unregisters on stream/replay failure (:3675–3683) — but D1's OWN bookkeeping (TaskAgentManager indexes + completion callback it installs before streaming) is ALWAYS DETACHED on stream/replay failure (a cached instance's operation-owned entries otherwise make future lookups return the failed worker and block a clean retry), while ONLY the `SessionManager.unregisterSession` step is conditioned on `createdNow` (matching the documented distinction). RE-READS the `NodeExecution` after the final pre-registration await: another spawn can rebind the execution from session A to B during `getSpace`/`ensureNodeAgentAttached` (the restore lock is keyed by session A and doesn't serialize the execution CAS, and the terminal re-read still passes while both are active) — require its `agentSessionId` and owning task/run to still match this restoration before crossing the registration boundary, with a rebind-during-attachment race test. RE-RESOLVES AND COMPARES THE TASK WORKSPACE after the final await: when restoring a `done` task with a node-execution session but no task/post-approval session ID or worktree, `SpaceTaskManager.updateTask` permits `workspacePath` to change while `getSpace`/attachment awaits, so the binding+task/run match alone still registers a worker prepared against the old repository — compare the workspace (or fence workspace mutation through registration), with a workspace-change-during-rehydration row. When the FINAL RECHECK refuses, the session has ALREADY received the old workspace metadata, system prompt, and runtime MCP configuration (:3597–3629) — the late-refusal path must RESTORE the captured cached-session state, or these mutations are fenced through the final check (the cached instance stays globally owned by SessionManager, so a later lookup/restoration otherwise observes config prepared for the stale task state), with a cached-session race row. The rollback/fence ALSO applies to UNCACHED late refusals: the `AgentSession.restore` branch mutates the same durable row via `updateMetadata({ workspacePath })` → `db.updateSession` (session-config-handler.ts:58–90), and if the final workspace/ownership check refuses, the uncached object is discarded but the row keeps the stale workspace — the next restoration's fallback prefers the persisted session workspace (:3588–3596), pinning retries to the checkout that caused the refusal; apply the rollback/fence to uncached restored instances too, with an uncached workspace-change row. For CALLER-OWNED restorations (E1's pre-lock rehydrate), SUPPRESS the detached flush and return a CONTINUATION E1 invokes after its locked admission commits (E1's post-lock refusal unwinds the restored worker while D1's flush could already be injecting/settling pending rows) — or await/cancel the flush before ownership cleanup. CLOSES THE CANCELLATION RACE: `performSubSessionRehydrate` checks task/run terminal status BEFORE awaiting `getSpace` and `ensureNodeAgentAttached`, then registers/starts — a concurrent terminal transition during those awaits finishes cleanup while the session is still absent from the manager indexes, letting D1 register/start a worker for the cancelled task; D1 re-reads terminal status AFTER the final pre-registration await (or consumes the fully wired TAM-F0 terminal fence — update dependencies if chosen), with race tests for cancellation during space loading AND attachment proving no session is registered or started afterward. OWNERSHIP MUST ALSO BE FENCED THROUGH STREAMING STARTUP AND REPLAY: `startStreamingQuery` and pending-message replay await AFTER the registration boundary, so a `workflowRunId`, workspace, or execution-binding change during either await passes the earlier checks and D1 returns session A with its indexes, callback, and restored configuration intact even though the task/execution now targets B (nonterminal changes do not clean up A) — fence ownership through startup/replay or perform an ownership-safe rollback after a FINAL POST-START reread, with reassignment and rebind race rows | build | ≲220 | ≲200 | TAM-PD, TAM-FH |
| TAM-D2 | Wire `rehydrateSubSession` | wire | ≲40 | ≲50 | TAM-D1 |
| TAM-E1 | `deliver-injected-message` pipeline over the COMPLETE operation (outer admissions + inner arms), composing `decideInjectDelivery`'s gates directly; uses the TAM-FH fenced repair helper and the TAM-LH lock-handle contract; RESOLVES THE OWNING TASK for both pre-lock and post-lock predicates (session identity/ownership map, not `listByWorkflowRunIncludingArchived(...)[0]`) with the corrected opposite-status multi-task rows; REQUIRES EXPLICIT TAM-LH lock release on every successful halt — the locked delivery group selecting `noop`/`defer` completes without unwinding compensations, so a compensation-only release leaves the reset-coordination lock held and blocks later injections/resets (release pinned on normal outcomes with a waiter test); CONSUMES TAM-B0's restoration provenance/shared-ownership so a POST-LOCK REFUSAL unwinds a rehydration owned by THIS call — a task cancelled during the pre-lock `rehydrateSubSession` await can't be found by cancellation cleanup (not yet registered), and after rehydration the post-lock terminal gate returns `noop` leaving the newly registered streaming worker alive; unwind a restoration owned by this call on refusal (cancel-during-rehydrate race test); delivery stage group exported for direct composition by owning pipelines | build | ≲230 | ≲180 | TAM-PE, TAM-FH (fenced repair helper), TAM-LH (lock-handle contract), TAM-B0 (restoration provenance) |
...
| TAM-E3 | Split the spawn flow's `injectKickoffMessage(sessionId, message): Promise<void>` dependency (the monolithic seam the `kickoff-session` effect invokes) into TARGET/LOCK preparation inputs + a CONTINUATION-SAFE E1 delivery stage group — the group RECORDS its `defer`/`noop` result WITHOUT halting, because a halt would terminate `spawn-execution` and skip `activate-pool-assignment` + `flush-pending-messages` (:377–428), leaving the model-pool reservation unactivated; the standalone injection pipeline (E2) adds its own terminal adapter; compose those stages into the owning stage list — exactly as B3 does for creation (with the seam unchanged, E3 could only retain the imperative call or nest the E1 runner); pin that a deferred/noop kickoff still reaches both downstream spawn stages — DISTINGUISHING terminal refusal from an idempotent already-consumed noop: a terminal-refusal noop (task/run terminal after session creation but before kickoff) must NOT continue through `activate-pool-assignment`/`flush-pending-messages` (that would activate capacity and flush into a worker for a terminal task); only the idempotent noop and `defer` continue, while terminal refusal runs ownership-aware spawn cleanup and releases the reservation. E3 must CONSUME THE FULLY WIRED TAM-F0 FENCE THROUGH THE DELIVERY RESULT (or an equivalent atomic status-and-enqueue protocol): its current dependencies omit TAM-F0 and the spawn flow releases `reserve-task-spawn` at :340–346 BEFORE attachment and kickoff injection (:348–408) — a task/run going terminal after E1's post-lock status reread but before the delivery effect can therefore commit, wait for the injection lock during cleanup, and still have the new pipeline enqueue a kickoff to the now-terminal worker; a separate wiring seam and business path from TAM-E2; MUST land before TAM-E4 deletes the old cascade | wire | ≲90 | ≲90 | TAM-E1, TAM-B3, TAM-B0, TAM-F0 + TAM-F0-W1..W7 (terminal fence through the delivery result — see the E3 fence note; W7 is REQUIRED here even though the post-approval arm starts from `approved`: the owning spawn flow also serves WORKFLOW-NODE executions whose tasks are `in_progress` while the node runs, `in_progress → blocked` is a valid transition, and `stopBlockedWorkflowTask` then cancels the new worker's session between the final status check and kickoff enqueue — E3 would deliver to a cancelled worker; TAM-SW fences only the post-approval `approved` departure, not this generic path — blocked-between-reread-and-enqueue race row), (the spawn flow receives CREATED/LIVE-REUSED/COLD-RESTORED provenance only after B3 composes B1's create group with B0's result — E3's terminal-refusal ownership-aware cleanup requires it, since the current SpawnAttemptBox has only a session ID and its compensation unconditionally calls cancelSpawnedSession, spawn-flow.ts:136–146,294–299; pin the three ownership outcomes) |
| TAM-E4 | Delete the old imperative `injectMessageIntoSession` cascade — removal-only, zero new logic; every caller is converted by then (TAM-E2 origin path, TAM-E3 spawn-flow kickoff, the rewired post-approval arms) | delete | ≲190 | 0 | TAM-E2, TAM-E3, TAM-F2W (the LIVE spawner body still calls `injectMessageIntoSession` at :4912/:5041 until F2W's method swap — TAM-F2 is dead code and converts nothing live; F2W is the spawner-swap owner that resolves those callers), P47 (the post-approval router arms' full conversion) |
...
| TAM-F0-W6 | Wire the TASK-DONE status writer family to honor the TAM-F0 fence: `approved → done` is a valid transition (:39) performed via `setTaskStatus(..., 'done')` by `completeTask`, the end-node handler, and `space-agent-tools.ts` — none of the listed fences covers it, so a completion landing after F1 admission but before delivery/commit lets the kickoff reach an already-done task (P46's expected-`approved` CAS can only reject the stamp afterward); race tests included | wire | ≲30 | ≲30 | TAM-F0 |
| TAM-F0-W7 | Wire the BLOCKED-status writer family to honor the TAM-F0 fence: `blocked` is TERMINAL (`isTerminalTaskStatus`, space-task-manager.ts:723; space-runtime.ts:3186–3187) and `in_progress → blocked` is a VALID transition (:27–37), and the block path CANCELS THE TASK'S AGENTS — the runtime's direct blocked writer (`updateTaskAndEmit`'s `status === 'blocked'` branch, space-runtime.ts:3213–3247) routes the dependency-added arm through `stopBlockedWorkflowTask` (:2887–2928), which transitions the run to `blocked`, calls `cancelBySessionId` on every non-idle execution's agent session AND on `taskAgentSessionId`, and clears the binding — PLUS the SpaceTaskManager `setTaskStatus(..., 'blocked')` family (`blockTask` :416, the dependency-added auto-block :509–520, `blockDependentTasks` :609); without this writer the terminal fence has a hole exactly where self-heal and restoration race, because a block cancels agents without being a W1–W6 writer (block-during-dispatch/self-heal race rows) | wire | ≲35 | ≲40 | TAM-F0 |
| TAM-OW | Shared NONTERMINAL OWNERSHIP FENCE PRIMITIVE (construction only, additive): the fence/API + contract tests so a `workflowRunId`/workspace reassignment during delivery is fenced (TAM-F0 covers only terminal transitions) — split from writer wiring so the primitive lands as additive dead code | build | ≲50 | ≲60 | TAM-PF |
| TAM-OW-W1 | WIRE the `workflowRunId` writer family to honor the TAM-OW fence | wire | ≲30 | ≲30 | TAM-OW |
| TAM-OW-W2 | WIRE the workspace writer family to honor the TAM-OW fence | wire | ≲30 | ≲30 | TAM-OW |
| TAM-SW | APPROVED-STATUS-DEPARTURE FENCE PRIMITIVE (construction only, additive): the fence/API + contract tests for every departure from the admitted `approved` state (neither a terminal transition TAM-F0 covers nor a run/workspace change TAM-OW covers) — split from writer wiring so it lands as additive dead code | build | ≲40 | ≲50 | TAM-PF |
| TAM-SW-W | WIRE the nonterminal `approved`→`in_progress` transitions — `SpaceTaskManager.setTaskStatus` and its runtime entry points, PLUS the DIRECT workflow-recovery writer (`SpaceRuntime.recoverWorkflowBackedTask`'s own `taskRepo.updateTask({ status: targetStatus })`: the `spaceTask.recoverWorkflow` RPC can set `in_progress` and `approved → in_progress` passes the transition table, so a recovery after P44 admission otherwise bypasses the fence mid-delivery — recoverWorkflow race row) — to honor the TAM-SW fence; F1 gates on this WIRED slice; the approved-to-in_progress race test lands here | wire | ≲30 | ≲40 | TAM-SW |
| TAM-DS | DAEMON-STARTUP RECONCILIATION for deferred settlement tracking: restore the attempt→message correlation and late-settlement subscriptions for EVERY unresolved durable DEFERRED attempt — including CAS-loss rows whose dispatch stamp did NOT survive (P46 permits a DEFERRED attempt to lose `record-dispatched-session` and retain only its durable unfinalized fence; an unstamped attempt is never rearmed by a stamp-scoped query, its later consumption/dead-letter cannot finalize the fence, and P44 then blocks every future reservation indefinitely) — F1 is an unwired build and F2 wires only the spawner, so this startup wiring is its own slice, and the router must not be exposed before it (deferred-consumption, deferred-failure, and unstamped-CAS-loss restart rows; SINGLE STARTUP OWNER — TAM-DS is the one daemon-startup reconciliation wiring seam: cross-plan P46-R's reconciler call is consolidated INTO TAM-DS's startup entry (P46-R supplies the reconciler operation only, no separate startup wiring; TAM-DS depends on it), so no two slices can claim the same call-site seam); reconciliation is AWAITED BEFORE any session restoration or queued-message replay begins (TaskAgentManager.rehydrate's startStreamingQuery/replay path must not replay a queued PREPARED/ENQUEUED kickoff before the TAM-F0/TAM-OW/TAM-SW handles are reconstituted — reconciliation-before-replay row) | wire | ≲40 | ≲60 | TAM-F1d (the durable-correlation owner), P44 (cross-plan: the dispatch-record schema and finalizePostApprovalDispatch operation this slice invokes are owned by P44), P46-W (cross-plan: the atomic deferred-message settlement primitive this slice's reconciliation invokes is owned by P46-W), P46-R (cross-plan: supplies the prepared-fence reconciler operation TAM-DS's startup seam invokes — no separate startup wiring in P46-R) |
| TAM-F1a | `spawn-post-approval-worker` stagedRun CORE, unwired, returning the P46 delivery outcome with a THREE-WAY ownership result (CREATED / LIVE-REUSED / COLD-RESTORED) carried from the create-stage result AND a DELIVERED/SKIPPED/DEFERRED delivery state — a reused worker that is busy/rate-limited/fails defer admission persists the kickoff as DEFERRED, and F1a OWNS the message-specific consumption tracking contract (finalization ties to an ATTEMPT-CORRELATED IDLE/completion signal or verified termination — consumption alone transitions the attempt to DELIVERED semantics but NEVER finalizes the fence (handleMessageYielded marks consumed when the message is yielded to the SDK, before processing runs; a fence lifted at consumption would admit a retry enqueueing the same kickoff concurrently), or await actual delivery before DELIVERED is reported. The DEFERRED→DELIVERED transition REARMS completion tracking ATOMICALLY (the one-shot listener may already have fired and been removed on the reused worker's first unrelated idle while the attempt was still DEFERRED — P46-W2 correctly ignores that event, but the listener is gone — so without a rearm at the transition, or a persistent MESSAGE-CORRELATED completion subscription instead of a one-shot listener, the eventual attempt-correlated idle has no listener and the unresolved fence blocks every future P44 reservation indefinitely; rearm-on-transition row). COMPOSES the B1 create stage group and the E1 delivery stage group directly (no runner nesting); the reuse-arm terminal gate runs BEFORE the reuse mutation sequence under the same lock (fenced by TAM-F0); the fresh arm's post-create recheck covers every create outcome (CREATED skip → F1c cleanup; COLD-RESTORED skip → F1c unwind; REUSED skip leaves the worker untouched). After the final create/attach await, REVALIDATE task ownership: re-read and match `workflowRunId` AND RE-RESOLVE AND MATCH THE WORKSPACE (a task with no recorded post-approval session is not treated as active — hasActiveTaskSession, space-task-manager.ts:127–134 — so a concurrent `workspacePath` update can change the target checkout; deliver only when it still matches, workspace-change-during-create test; apply ownership-specific cleanup on refusal). The LIVE-REUSE arm needs the same post-awaits ownership reread (it awaits workspace reinjection after its snapshot :4879–4912; a reassignment during `reinjectNodeAgentMcpServer` leaves the pre-await guard satisfied — reassignment-during-reinjection row) and must REARM message/attempt-specific COMPLETION TRACKING before each injection (`registerCompletionCallback` is one-shot `fired` and the first idle unsubscribes :3086–3111, while the live-reuse arm :4860–4917 does not re-register — already-idle-then-reused row). When the OWNERSHIP REREAD REFUSES after reuse mutations, the rollback scope follows the ARM: the full B1 mutation rollback (:1148–1277) applies ONLY to COLD-RESTORED outcomes, while the LIVE-REUSE arm — which never calls `createSubSession` and must not have B1 composed into it — rolls back only its ACTUAL mutations (workspace/MCP reinjection and the completion-tracking rearm); live- and cold-reuse reassignment rows | build | ≲120 | ≲120 | TAM-B0, TAM-B1 (composed directly — F1a never calls the public B2 wrapper), TAM-E1, TAM-F0 (the reuse-arm terminal gate is fenced by it — F1a's contract and its terminal-race tests require the primitive), TAM-PF (P46 outcome contract) |
| TAM-F1b | FENCE-CONTINUATION CONTRACT for the F1a outcome: F1a returns RELEASABLE FENCE / COMMIT CONTINUATIONS FOR the TAM-F0 terminal fence, the TAM-OW nonterminal ownership fence, AND the TAM-SW approved-status-departure fence (matching P46's settlement and S2's propagation; under the fence-based implementation P46 has no handle to retain if fewer are returned, and a writer can acquire the missing fence before `record-dispatched-session`), which P46 settles on EVERY path (success, halt, error, timeout) — OR ownership validation + kickoff enqueue + dispatch stamping move into ONE atomic operation. OWNERSHIP MUST STAY STABLE THROUGH DISPATCH COMMIT: the protected interval ends before the composed E1 delivery and P46's `record-dispatched-session` (E1 awaits context clearing/backlog replay/status publication/consumption, and F1 returns before the CAS), and TAM-F0 does not block nonterminal `workflowRunId`/workspace changes — fence ownership through ACTUAL DELIVERY and the dispatch CAS, or make ownership validation AND ENQUEUE ATOMIC; pin races in BOTH windows (create-to-delivery and delivery-to-commit). EVERY POST-ADMISSION task-status DEPARTURE is fenced too: `approved` → `in_progress` after P44 admits but before delivery/commit is a valid transition (:39) that neither TAM-F0 (terminal) nor TAM-OW (run/workspace) blocks — fence ALL departures from the admitted `approved` state through enqueue and commit (approved-to-in-progress race row). F1a RELEASES the TAM-LH injection handle before EVERY completed outcome (completed halts do not unwind compensations; pin a subsequent lock waiter) | build | ≲40 | ≲60 | TAM-F1a, TAM-F0, TAM-F0-W1..W6 (W7 is unnecessary in THIS slice's post-approval window — `approved → blocked` is invalid and the `in_progress` departure is TAM-SW-fenced through enqueue and commit — but the spawn flow's kickoff window, which also serves `in_progress` workflow-node tasks, is TAM-E3's and does require W7), TAM-OW, TAM-OW-W1, TAM-OW-W2, TAM-SW, TAM-SW-W, TAM-LH |
| TAM-F1c | COMPENSATION PROTOCOLS for every refusal/failure outcome, landed WITH their tests: terminal-skip rows for both outcomes; after-recheck-before-delivery transition; post-create attach/inject-failure compensation per ownership — CREATED stopped/unregistered + durable-row deleted + status-and-session-guarded EXECUTION UNBIND (B1's fresh arm CAS-bound the execution; stopping alone leaves it `in_progress` pointing at the deleted worker — both the terminal-skip and attach/inject-failure cleanups include the unbind with execution-row assertions); COLD-RESTORED unwound without deleting the durable session AND only while no successful concurrent joiner retains it (owner-fails-joiner-succeeds race, cold-cache unwind); LIVE-REUSED intact | build | ≲40 | ≲60 | TAM-F1a, TAM-F1b |
| TAM-F1d | DURABLE ATTEMPT/MESSAGE CORRELATION + restart rearming: the correlation is DURABLE and REARMED/reconciled at startup (completion callbacks and late-settlement subscriptions are process-local while a DEFERRED kickoff plus its dispatch stamp survive a restart — a post-restart dead-letter must still conditionally clear the matching stamp, and P45's alreadyRouted gate must not block every retry while the worker stays live; deferred-consumption and deferred-failure restart rows, consumed by TAM-DS) | build | ≲25 | ≲30 | TAM-F1a, P44 (cross-plan: the correlation and crash-safe delivery fence use P44's dispatch-record schema and persistence API) |
| TAM-F1e | MODEL-POOL RESOURCE CLEANUP: RELEASE THE RESERVATION on every skip (the fresh arm reserves the slot before creation :4961 and a staged terminal-skip halt completes normally rather than entering the catch :5049–5051 — capacity-reuse assertion) and the exported result CARRIES an owner-provided, SESSION-SCOPED CLEANUP OPERATION for the ACTIVATED assignment, exposed for EVERY activated ownership outcome (P46 invokes it for CREATED termination AND the VERIFIED COLD-RESTORED unwind; capacity-reuse tests covering CAS loss, another post-spawn failure, and the cold-unwind case) | build | ≲25 | ≲30 | TAM-F1a |
| TAM-F1f | POST-SPAWN FLUSH CONTINUATION: EXPOSES the continuation WITHOUT STARTING it — P46 invokes the flush ONLY after `record-dispatched-session` wins the CAS (CAS-loss/later-route-failure race row with pending rows present) | build | ≲15 | ≲20 | TAM-F1a, TAM-LH |
| TAM-F2 | ADDITIVE dead-code construction (kind = build, not wire): the F1a-delegated spawner implementation merges as a SEPARATE, UNREACHABLE-FROM-PRODUCTION function beside the legacy method — no production path changes in this slice; the LIVE method keeps the legacy body. The production wiring lands as TWO ORDERED wire slices — the ROUTER call-site swap is P47's and lands FIRST (it consumes this delegated implementation directly, registers as the continuation consumer before invoking — deactivating the adapter fallback in the same registration check — and performs the GUARDED outcome-mapped task writes), and the SPAWNER-METHOD swap is TAM-F2W's and lands SECOND: until P47 rewires the router, the legacy router leg still performs UNSUPPRESSIBLE unconditional post-call writes (the success leg stamps `postApprovalSessionId`/`postApprovalStartedAt`; the superseded/transient failure legs clear pending state and write `postApprovalBlockedReason` — post-approval-router.ts:262–285), and a spawner-side adapter cannot guard CALLER-side writes, so a losing concurrent dispatch's legacy failure leg could mark the winner's live dispatch blocked — F2W is NOT shippable before P47; after P47 the legacy method has no remaining production caller (the router is its only one, post-approval-router.ts:252 via space-runtime.ts:2701), so F2W is a near-trivial body swap retiring the last `injectMessageIntoSession` caller. No atomic combination, no stranded-fence window. (The fence-propagation test is TAM-F2T's, not this slice's.) | build | ≲40 | ≲60 | TAM-F1a, TAM-F1b, TAM-F1c, TAM-F1d, TAM-F1e, TAM-F1f, TAM-DS, TAM-C2 (the message-handler fallback at space-task-message-handlers.ts:233–238 can otherwise still invoke the legacy restoration path while a reservation is held — the dead-A-versus-replacement-B race), P44 (dispatch-reservation primitive), P45, P46-R, P46-W2/W3 (the only valid fence lifters: idle/attempt-correlated completion, message-correlated failure, and verified/cancelled termination — a generic `session.error` never lifts one), P53 (P47's router swap requires the CAS-capable goal-terminal handler first) |
| TAM-F2T | The missing-MCP FENCE-PROPAGATION integration test ONLY (startup self-heal reusing the held fences while B1's startStreamingQuery invokes the missing-MCP callback) — exercises the TAM-F2 dead-code delegated path directly, never through the live router; exposure stays gated by P47 alone | test-only | 0 | ≲60 | TAM-F2, TAM-S2 |
| TAM-F2W | SPAWNER-METHOD SWAP wire slice, landing AFTER P47: the live `spawnPostApprovalSubSession` body switches to the delegated implementation — by then the router (its only production caller, post-approval-router.ts:252) already consumes the delegated function directly, so this swap retires the last legacy body and its `injectMessageIntoSession` calls (TAM-E4's gate); the SELF-SETTLING adapter remains the fallback that settles every continuation (guarded stamp + release) for any consumer that has not registered, with P44's durable fence as the backstop — the swap is NOT shippable before P47, because the unrewired legacy router's unconditional post-call writes (post-approval-router.ts:262–285) cannot be suppressed from the spawner side | wire | ≲40 | ≲50 | TAM-F2, TAM-F2T, P47 (cross-plan: the router call site must be rewired first — see the TAM-F2 ordering note) |
...
| TAM-GW | REWIRE the late-settlement watcher (:1534–1586) to consume TAM-G1's status-guarded settlement — `markDelivered` already includes `WHERE status = 'pending'` (pending-agent-message-repository.ts:230–242) so the write itself cannot overwrite a failed row, but the watcher UNCONDITIONALLY calls `emitPendingDelivered` after that no-op (:1549–1555), leaving clients observing a delivered event for a durably failed row; G1 must REPORT whether its conditional settlement won and the watcher emits ONLY on a win (as TAM-GR already requires); explicit deps + tests for the read-pending/injector-error/watcher-emit interleaving; lands before the old behavior is removed | wire | ≲40 | ≲60 | TAM-G1 |
| TAM-GR | REWIRE `reconcileRows` (`space-agent-pending-drain.ts:78–82`) to consume G1's guarded settlement — it retains its own `getById(...pending)` + `markDelivered` + UNCONDITIONAL `onSettled`, so a concurrent failure makes the repository update a no-op but `onSettled` still emits `space.pendingMessage.delivered`; emit only when the settlement WINS. The ERROR arm (:108) is rewired TOO: `recordDeliveryError`'s SQL is guarded only by `id` (pending-agent-message-repository.ts:261–268), so after a late watcher's `markDelivered` wins, the error write still stamps failure metadata onto a durably delivered row — GR uses G1's STATUS-GUARDED error operation and calls `onFailed` only when that conditional write wins; read-pending/concurrent-failure interleavings pinned | wire | ≲40 | ≲60 | TAM-G1 |
| TAM-S1 | `self-heal-node-agent` stagedRun, unwired (prologue gates via the TAM-FH fenced repair helper + adoption admission composed in front of the existing heal stages; heal workspace routed through `resolveTaskWorkspace`; adoption at the manager-owned pre-heal boundary) | build | ≲140 | ≲120 | TAM-PS, TAM-FH, TAM-LH (shared lock-handle contract — S1 consumes no E1 delivery behavior, so it depends on the primitive directly, not the delivery build), TAM-LH2 (held-handle/fence propagation for the nested F1 startup-self-heal path, or reversible adoption + final ownership check), TAM-F0 + the terminal-status writers (TAM-F0-W1..W7 — W7 is REQUIRED: `in_progress → blocked` is a valid transition, `blocked` is terminal, and the dependency-added block path cancels the task's agents, so a block completing during the self-heal space load otherwise slips a terminal transition plus agent cancellation past the fence), TAM-OW + TAM-OW-W1/W2 (nonterminal ownership fenced across adoption + healing — a workflowRunId/workspace reassignment after adoption while the heal tail awaits MCP reinjection/restart must not leave the adopted session canonical for the old run). S1 carries the SELF-HEAL LOCK BOUNDARY: displaced-session adoption finishes BEFORE acquisition, only the heal tail runs while the handle is held, and successful completion EXPLICITLY releases (compensations do not unwind on success) — with a waiter test; wiring the whole runner under the callback-form lock or omitting the success release would move adoption under coordination or permanently block later injection/reset callers. S1 CARRIES THE TERMINAL-STATUS RACE CONTRACT: cancellation/archive/stop/run-cancel/block must be FENCED across adoption and healing (via TAM-F0 + TAM-F0-W1..W7 dependencies), OR adoption is fully reversible — otherwise a terminal transition completing during the space load or after the ownership check lets adoption replace the manager indexes, cache registration, and completion callback after terminal cleanup has run (the heal tail's workflow-run check passes too), reviving a worker for terminal work; pin the cancellation- AND block-during-space-load/adoption race tests (block-during-self-heal: a dependency-added block completing while self-heal awaits the space load cancels the session and clears `taskAgentSessionId` — adoption must not restart it), and TAM-S2 MUST NOT wire the pipeline until this protocol is available |
| TAM-S2 | Wire `mcpSelfHeal` to the S1 pipeline; index/callback/displaced-session lifecycle effects stay class-owned; MUST NOT wire before S1's terminal-status fence (TAM-F0 + writers) is available; the S2→F1 dependency is DROPPED (F1 → B0 → B1 → LH → S2 would otherwise cycle B1→S2→F1→B0→B1), and the missing-MCP startup propagation test is OWNED BY TAM-F2T alone (S2 carries NO fence-propagation test — the startup self-heal path reusing F1's held fences is verified in F2, which owns that integration; S1's contract to not wait forever on the non-reentrant fence is met by TAM-LH2's held-handle propagation, which F2's test exercises) | wire | ≲40 | ≲50 | TAM-S1 (no TAM-F1 dependency — the F1→B0→B1→LH→S2 path would otherwise cycle B1→S2→F1a→B0→B1) |
...
Pin slices are one per business path (one purpose, one PR): each pin slice
gates only its own build slice, so unrelated pin matrices never gate an
implementation — but the BUILD slices themselves form the broad serial
dependency graph described at the top (shared primitives, wired fences, and
writer wiring precede the pipelines that consume them), and the dependency
cells — not this paragraph — are authoritative for scheduling. Every pin
slice gap-measures against the existing suites in
...
encoded in BOTH ladders: P46 depends on TAM-F1a–F1f (outcome/fence/compensation/correlation/resource/flush contracts) and P47 on
