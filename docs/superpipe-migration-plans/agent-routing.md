...
- **Shell/effect wiring:** The exported `routeTurnEnd(flags, event, ctx)` stays as a thin wrapper over the routing-only gate run in `turn-end-routing.ts` (returning `ctx.plan` with a `makePlan(flags)` fallback); it does NOT call the full `turn-end-pipeline.ts` runner, because it does not have the `usageState`, `resultUsage`, `ackRows`, etc. required by the full `TurnEndPipelineInput` (`packages/daemon/src/lib/agent/turn-end-pipeline.ts:24-43`). The same extracted routing gate functions are also composed directly into `turn-end-pipeline.ts`'s existing gate list in the routing position. The complete SDK turn-end pipeline is an async mixed `stagedRun` (or async `superpipe`) that runs, in order:
  ...
  6. Turn-end idle/timer/replay/flag effects (`finishTurn` with `setIdle` and queue replay if `plan.finishTurn && plan.allowQueueReplay`, timer cancel/rearm, the remaining flag transitions such as `usesSessionStateChangedTurnEnd`/`expectsSessionStateIdleAfterResult`/`clearAwaitingTrailingIdle`/`lastResultWasSuccess` reset for session-state mode, `refreshContextUsage('turn-end')` for every SDK result, and `clearIdleSuppression()` when `settlesArmedClearError`) consume the remaining `plan` fields.
...
- **Shell/effect wiring:** The admission gate functions are called by the first `stagedRun` `snapshot`/`effect` stage, which is the entire body of a `withSessionLock` callback. That stage runs the admission gates; for `feed` and `awaiting_acceptance` it computes `generation` and calls `observer.reportStage('query_ready', { generation })` before either `messageQueue.admitWithId` or returning `awaiting_acceptance` (`packages/daemon/src/lib/agent/agent-session.ts:2574-2587`). For `feed` it then performs the `messageQueue.admitWithId` queue mutation and returns the feed-action snapshot (`acknowledgment`, `generation`, `clearEpoch`); for `awaiting_acceptance` it returns that kind immediately. The remaining top-level `stagedRun` effect stages run after the `withSessionLock` callback returns, so the per-session mutex is not held during the long acknowledgement wait:
  ...
  - Teardown revalidation (`packages/daemon/src/lib/agent/agent-session.ts:2633-2640`).
...
- **Tests:** `delivery-turn-routing.test.ts` is the parity suite. Add `steer-admission-gates.test.ts` with an `it.each` matrix covering every `status` × `deliveryValid` × `hasLiveQuery` × `provider` × `queueOwnsMessage` × `claimCurrent` combination and asserting the first gate that fires. Add `feed-delivery-steer.test.ts` wiring tests that assert the locked stage reports `query_ready` with `generation` before `admitWithId` or `awaiting_acceptance`, the ack-wait and requeue stages run outside the `withSessionLock` callback (e.g., by observing that `resolveSteerAdmission` and `messageQueue.admitWithId` complete before the lock is released and before the ack wait begins), that the abort-signal race preserves both remove-succeeded and remove-failed outcomes and that `aborted.cancel()` runs in every outcome, and that `reopenDeliveryForRetry` is called after `messageQueue.requeueYielded` for `query_ended`.
...
- **Lands:** The complete SDK turn-end business path — pre-handoff armed-clear timer cancellation and thinking reset, limit-recovery handoff (with nonfatal interception marker), pre-publication verdict and armed-clear publication compensation, usage accounting, routing gates, idle-fencing, resnapshot-acknowledgment with matching-clear compensation guard, successful-result side effects, AND turn-end idle/timer/replay/flag effects with common result-tail refresh — runs as one composed operation in production.
- **Excludes:** `routeTurnEnd`'s parity wrapper (kept for the suite); turn-end-routing gate logic changes.
- **Tests:** `turn-end-pipeline.test.ts` extended with effect-stage rows (pre-handoff armed-clear timer cancellation and thinking reset on intercepted limit, `lastResultWasSuccess` set before `sdk.message` publication and preserved on rejection, `sdk.message` publication rejection under an armed clear, limit-recovery handoff accepted/declined with SDK message publication and nonfatal interception-marker failure, intercepted-result usage, idle-fencing order, successful-result side effects including resnapshot-acknowledgment and matching-clear failure compensation, `finishTurn` idle/replay, timer rearm, usage accounting, `refreshContextUsage('turn-end')` on both success and error result routes, and `clearIdleSuppression()` for `settlesArmedClearError`); `turn-end-routing.test.ts` parity unchanged.
- **Depends on:** PR 20.
