...
- **Shell/effect wiring:** The exported `routeTurnEnd(flags, event, ctx)` stays as a thin wrapper over the routing-only gate run in `turn-end-routing.ts` (returning `ctx.plan` with a `makePlan(flags)` fallback); it does NOT call the full `turn-end-pipeline.ts` runner, because it does not have the `usageState`, `resultUsage`, `ackRows`, etc. required by the full `TurnEndPipelineInput` (`packages/daemon/src/lib/agent/turn-end-pipeline.ts:24-43`). The same extracted routing gate functions are also composed directly into `turn-end-pipeline.ts`'s existing gate list in the routing position. The turn-end `stagedRun` (or async `superpipe`) is invoked by `handleSDKMessage` only for top-level `result` and `session_state_changed` events; the common SDK message publication (`state.sdkMessages.delta` and `sdk.message`, `packages/daemon/src/lib/agent/sdk-message-handler.ts:900-920`) stays UNIVERSAL for all SDK messages, and the type-specific handlers for `user`, `system`, `assistant`, `status`, `compact-boundary`, and refusal messages remain outside the turn-end pipeline. The pipeline runs, in order:
  ...
  5. Successful-result side effects, gated on `plan.isSuccess && event.result.isTopLevel`: if `plan.earlySetIdle`, await `stateManager.setIdle()` before the successful-result effects, matching the current ordering for ordinary top-level results (`packages/daemon/src/lib/agent/sdk-message-handler.ts:947-950`, `961-963`); then run `recordResultUsageMetadata` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:1063-1118`), circuit-breaker `markSuccess` on non-zero usage, `acknowledgeOldestQueuedUserOnTurnEnd` (`:464-523`) — which resnapshots `activeDeliveryMessageUuids`, `hasYielded`, and `hasPendingOrClaimed` immediately before `markDeliveriesConsumedAtTurnEnd`, then publishes `messages.statusChanged` and replays the consumed user messages, `acknowledgedPersistedUserThisTurn` reset, `session.errorClear` publish (`:1147-1170`). Nested result successes are saved and published by the common publication stage but MUST NOT reach these side-effect stages. Each awaited effect has a `compensate` (or `stagedRun` unwind) that calls `clearIdleSuppression()` before rethrowing ONLY when `confirmsArmedClear` is true, matching the current `handleResultMessage` catch (`packages/daemon/src/lib/agent/sdk-message-handler.ts:1126-1134`).
...
- **Scope:** Wire the COMPLETE async retry operation. Review correction (PR #2980): `query-runner.ts` calls the synchronous `decideQueryRetry` in two places in `executeQuery` — BEFORE awaiting `onRateLimitExhausted` (`packages/daemon/src/lib/agent/query-runner.ts:1039`) and AGAIN after the callback supplies `accepted`/`declined` (`:1170-1183`) — and a third time in `retryRouteChanged` (`:1666-1680`) to resnapshot the environment after awaited teardown work and immediately compare `route.action` to the expected action. The route classifier (`decideQueryRetry`) must remain synchronous for the resnapshot. This slice therefore introduces a NEW async `route-query-retry-operation` `stagedRun` (or async `superpipe`) that runs:
  ...
  5. For `aborted_noop`/`api_validation`/`terminal` (and the final route after a non-retry `rate_limit_handoff`): an effect stage that clears `messageQueue` (unless `finalizer.skipQueueClear`); for `api_validation`/`terminal`, `beginTerminalIdle` unless `finalizer.skipBeginTerminalIdle`; for `api_validation`, `displayErrorAsAssistantMessage` unconditionally (the `skipErrorManager` finalizer flag only suppresses `errorManager.handleError` for `terminal` routes); for `terminal`, `errorManager.handleError` unless `finalizer.skipErrorManager`; `setIdle` unless `finalizer.skipCatchIdle`.
...
- **Scope:** Review correction (PR #2980): complete phase 6 by wiring the extended `turn-end-pipeline.ts` into the SDK turn-end runtime path as an async mixed `stagedRun` (or async `superpipe`). The pipeline is invoked only for top-level `result` and `session_state_changed` events; the common SDK message publication (`state.sdkMessages.delta` and `sdk.message`, `packages/daemon/src/lib/agent/sdk-message-handler.ts:900-920`) stays UNIVERSAL for all SDK messages, and the type-specific handlers for `user`, `system`, `assistant`, `status`, `compact-boundary`, and refusal messages remain outside the turn-end pipeline. The pipeline runs, in order:
  ...
  4. Pre-publication and idle-fencing effects for turn-end events (`packages/daemon/src/lib/agent/sdk-message-handler.ts:891-920`): set `lastResultWasSuccess` and reset `lastRateLimitInfo`/`lastSdkErrorTag` before any publication (`packages/daemon/src/lib/agent/sdk-message-handler.ts:891-893`); then `beginTerminalIdle` if `plan.idleFence`; then publish `state.sdkMessages.delta` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:900-908`); then await `internalEventBus.publish('sdk.message')` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:911-920`) with a `compensate` that calls `clearIdleSuppression()` before rethrowing when the result confirms an armed clear (`confirmsArmedClear`). If `isLimitRecoveryEngaged` is accepted, mark the result intercepted with the `limitBillingTerminal` flag carried from `limitError.hint.billingTerminal` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:878-883,923-930`) inside a nonfatal `try/catch` that logs and continues, then record usage, refresh context usage, and short-circuit the rest.
  5. Successful-result side effects, gated on `plan.isSuccess && event.result.isTopLevel`: if `plan.earlySetIdle`, await `stateManager.setIdle()` before the successful-result effects, matching the current ordering for ordinary top-level results (`packages/daemon/src/lib/agent/sdk-message-handler.ts:947-950`, `961-963`); then `recordResultUsageMetadata` at `:1063-1118`, circuit-breaker `markSuccess` on non-zero usage, `acknowledgeOldestQueuedUserOnTurnEnd` at `:464-523` — which resnapshots `activeDeliveryMessageUuids`, `hasYielded`, and `hasPendingOrClaimed` immediately before `markDeliveriesConsumedAtTurnEnd`, then publishes `messages.statusChanged` and replays the consumed user messages, `acknowledgedPersistedUserThisTurn` reset, `session.errorClear` publish at `:1147-1170`. Nested result successes are saved and published by the common publication stage but MUST NOT reach these side-effect stages. Each awaited effect has a `compensate`/`unwind` that calls `clearIdleSuppression()` before rethrowing ONLY when `confirmsArmedClear` is true, matching the current `handleResultMessage` catch (`packages/daemon/src/lib/agent/sdk-message-handler.ts:1126-1134`).
