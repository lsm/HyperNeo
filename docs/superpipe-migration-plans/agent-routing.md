...
- **Shell/effect wiring:** The exported `routeTurnEnd(flags, event, ctx)` stays as a thin wrapper over the routing-only gate run in `turn-end-routing.ts` (returning `ctx.plan` with a `makePlan(flags)` fallback); it does NOT call the full `turn-end-pipeline.ts` runner, because it does not have the `usageState`, `resultUsage`, `ackRows`, etc. required by the full `TurnEndPipelineInput` (`packages/daemon/src/lib/agent/turn-end-pipeline.ts:24-43`). The same extracted routing gate functions are also composed directly into `turn-end-pipeline.ts`'s existing gate list in the routing position. The complete SDK turn-end pipeline is an async mixed `stagedRun` (or async `superpipe`) that runs, in order:
  1. A pre-handoff effect stage for every top-level result that resets thinking-token tracking (`packages/daemon/src/lib/agent/sdk-message-handler.ts:880`) and, when a limit error is present, awaits `onResultLimitError` to determine `isLimitRecoveryEngaged` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:877-890`, `922-937`).
  2. `applyUsageAccountingGate` (`packages/daemon/src/lib/agent/turn-end-pipeline.ts:44-54`) and `applyAckSelectionGate` (`:81-96`) produce `usage` and `ackSelection` for non-intercepted, successful top-level results.
  3. The routing gates set `ctx.plan` with fields `idleFence`, `earlySetIdle`, `finishTurn`, `allowQueueReplay`, etc.
  4. Pre-publication and idle-fencing effects (`packages/daemon/src/lib/agent/sdk-message-handler.ts:891-920`): set `lastResultWasSuccess` and reset `lastRateLimitInfo`/`lastSdkErrorTag` before any publication (`packages/daemon/src/lib/agent/sdk-message-handler.ts:891-893`); then `beginTerminalIdle` if `plan.idleFence`; then publish `state.sdkMessages.delta` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:900-908`); then await `internalEventBus.publish('sdk.message')` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:911-920`) with a `compensate` that calls `clearIdleSuppression()` before rethrowing when the result confirms an armed clear (`confirmsArmedClear`). If `isLimitRecoveryEngaged` is accepted, mark the result intercepted, record usage, refresh context usage, and short-circuit the rest (`packages/daemon/src/lib/agent/sdk-message-handler.ts:922-937`). These must run before the result is processed by user/assistant/system handlers and before the successful-result side effects, preserving the legacy ordering pinned in `sdk-message-handler-flag-machine.test.ts`.
  5. Successful-result side effects (`recordResultUsageMetadata` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:1063-1118`), circuit-breaker `markSuccess` on non-zero usage, `acknowledgeOldestQueuedUserOnTurnEnd` (`:464-523`), `acknowledgedPersistedUserThisTurn` reset, `session.errorClear` publish (`:1147-1170`)). Each awaited effect has a `compensate` (or `stagedRun` unwind) that calls `clearIdleSuppression()` before rethrowing ONLY when `confirmsArmedClear` is true, matching the current `handleResultMessage` catch (`packages/daemon/src/lib/agent/sdk-message-handler.ts:1126-1134`).
  6. Turn-end idle/timer/replay/flag effects (`finishTurn` with `setIdle` and queue replay if `plan.finishTurn && plan.allowQueueReplay`, timer cancel/rearm, and the remaining flag transitions such as `usesSessionStateChangedTurnEnd`/`expectsSessionStateIdleAfterResult`/`clearAwaitingTrailingIdle`/`lastResultWasSuccess` reset for session-state mode) consume the remaining `plan` fields.
...
- **Shell/effect wiring:** The admission gate functions are called by the first `stagedRun` `snapshot`/`effect` stage, which is the entire body of a `withSessionLock` callback. That stage runs the admission gates, then performs the `messageQueue.admitWithId` queue mutation, and returns the feed-action snapshot (`acknowledgment`, `generation`, `clearEpoch`) to the rest of the pipeline. The remaining top-level `stagedRun` effect stages run after the `withSessionLock` callback returns, so the per-session mutex is not held during the long acknowledgement wait:
  - Acknowledgment waiting (`packages/daemon/src/lib/agent/agent-session.ts:2609-2622`) with the abort-signal race cleanup preserved: if the delivery signal aborts while the steer is awaiting acknowledgment, first try `messageQueue.remove(messageUuid)` and throw only if that succeeds; if the SDK already consumed the entry, await the acknowledgment and continue as acknowledged (`packages/daemon/src/lib/agent/agent-session.ts:2603-2621`).
...
- **Tests:** The PR 5 matrix end-to-end through the wired pipeline; wiring tests for the ack-waiting (including the remove-vs-await abort race), teardown-revalidation, requeue (including `reopenDeliveryForRetry` after `messageQueue.requeueYielded` for `query_ended`), and metrics stages; a `feed-delivery-steer-acknowledged.test.ts` that asserts the post-ack consumption tail runs for a non-ACP steer and that `signalDeliveryConsumed` is published.
- **Depends on:** PR 6.
...
- **Scope:** Wire the COMPLETE async retry operation. Review correction (PR #2980): `query-runner.ts` calls the synchronous `decideQueryRetry` in two places in `executeQuery` — BEFORE awaiting `onRateLimitExhausted` (`packages/daemon/src/lib/agent/query-runner.ts:1039`) and AGAIN after the callback supplies `accepted`/`declined` (`:1170-1183`) — and a third time in `retryRouteChanged` (`:1666-1680`) to resnapshot the environment after awaited teardown work and immediately compare `route.action` to the expected action. The route classifier (`decideQueryRetry`) must remain synchronous for the resnapshot. This slice therefore introduces a NEW async `route-query-retry-operation` `stagedRun` (or async `superpipe`) that runs:
  ...
  6. A final `finally`/`compensation` stage that runs the recovery-aware finalizer idle (`stateManager.setIdle()` if `!finalizer.skipFinalizerIdle(env)` and the current conditions allow) so rejecting handoffs and retry teardowns still surface the correct idle behavior (`packages/daemon/tests/unit/1-core/agent/query-runner.test.ts:2828-2847`). The existing `runQuery` outer `finally` block (`packages/daemon/src/lib/agent/query-runner.ts:1224-1288`) is refactored so the recovery-aware `setIdle` portion is not duplicated: it moves into this operation stage, while the remaining `finally` only performs non-idle cleanup (`attemptTokens.invalidate`, `releaseStartupPermit`, clear timers, abort, `messageQueue.stop`, close `queryObject`, restore env vars).
...
- **Scope:** Review correction (PR #2980): complete phase 6 by wiring the extended `turn-end-pipeline.ts` into the SDK turn-end runtime path as an async mixed `stagedRun` (or async `superpipe`). The pipeline runs, in order:
  1. A pre-handoff effect stage for every top-level result that resets thinking-token tracking (`packages/daemon/src/lib/agent/sdk-message-handler.ts:880`) and, when a limit error is present, awaits `onResultLimitError` to determine `isLimitRecoveryEngaged` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:877-890`, `922-937`).
  2. `applyUsageAccountingGate` (`packages/daemon/src/lib/agent/turn-end-pipeline.ts:44-54`) and `applyAckSelectionGate` (`:81-96`) to produce `usage` and `ackSelection` for non-intercepted, successful top-level results.
  3. Routing gates that set `plan` with fields `idleFence`, `earlySetIdle`, `finishTurn`, `allowQueueReplay`, etc.
  4. Pre-publication and idle-fencing effects (`packages/daemon/src/lib/agent/sdk-message-handler.ts:891-920`): set `lastResultWasSuccess` and reset `lastRateLimitInfo`/`lastSdkErrorTag` before any publication (`packages/daemon/src/lib/agent/sdk-message-handler.ts:891-893`); then `beginTerminalIdle` if `plan.idleFence`; then publish `state.sdkMessages.delta` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:900-908`); then await `internalEventBus.publish('sdk.message')` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:911-920`) with a `compensate` that calls `clearIdleSuppression()` before rethrowing when the result confirms an armed clear (`confirmsArmedClear`). If `isLimitRecoveryEngaged` is accepted, mark the result intercepted, record usage, refresh context usage, and short-circuit the rest (`packages/daemon/src/lib/agent/sdk-message-handler.ts:922-937`).
  5. Successful-result side effects (`recordResultUsageMetadata` at `:1063-1118`, circuit-breaker `markSuccess` on non-zero usage, `acknowledgeOldestQueuedUserOnTurnEnd` at `:464-523`, `acknowledgedPersistedUserThisTurn` reset, `session.errorClear` publish at `:1147-1170`). Each awaited effect has a `compensate`/`unwind` that calls `clearIdleSuppression()` before rethrowing ONLY when `confirmsArmedClear` is true, matching the current `handleResultMessage` catch (`packages/daemon/src/lib/agent/sdk-message-handler.ts:1126-1134`).
  6. Turn-end idle/timer/replay/flag effects (`finishTurn` with `setIdle` and queue replay if `plan.finishTurn && plan.allowQueueReplay`, timer cancel/rearm, and the remaining flag transitions such as `usesSessionStateChangedTurnEnd`/`expectsSessionStateIdleAfterResult`/`clearAwaitingTrailingIdle`/`lastResultWasSuccess` reset for session-state mode) consuming the remaining `plan` fields.
...
- **Lands:** The complete SDK turn-end business path — pre-handoff thinking reset, limit-recovery handoff, pre-publication verdict and armed-clear publication compensation, usage/ack accounting, routing gates, idle-fencing, successful-result side effects (with matching-clear compensation guard), AND turn-end idle/timer/replay/flag effects — runs as one composed operation in production.
- **Excludes:** `routeTurnEnd`'s parity wrapper (kept for the suite); turn-end-routing gate logic changes.
- **Tests:** `turn-end-pipeline.test.ts` extended with effect-stage rows (pre-handoff thinking reset on intercepted limit, `lastResultWasSuccess` set before `sdk.message` publication and preserved on rejection, `sdk.message` publication rejection under an armed clear, limit-recovery handoff accepted/declined with SDK message publication, intercepted-result usage, idle-fencing order, successful-result side effects including matching-clear failure compensation, `finishTurn` idle/replay, timer rearm, usage accounting, and ack selection per plan arm); `turn-end-routing.test.ts` parity unchanged.
- **Depends on:** PR 20.
