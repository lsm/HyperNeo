...
- **Shell/effect wiring:** The exported `routeTurnEnd(flags, event, ctx)` stays as a thin wrapper over the routing-only gate run in `turn-end-routing.ts` (returning `ctx.plan` with a `makePlan(flags)` fallback); it does NOT call the full `turn-end-pipeline.ts` runner, because it does not have the `usageState`, `resultUsage`, `ackRows`, etc. required by the full `TurnEndPipelineInput` (`packages/daemon/src/lib/agent/turn-end-pipeline.ts:24-43`). The same extracted routing gate functions are also composed directly into `turn-end-pipeline.ts`'s existing gate list in the routing position. The complete SDK turn-end pipeline is an async mixed `stagedRun` (or async `superpipe`) that runs, in order:
  1. The `onResultLimitError` handoff for top-level limit results (`packages/daemon/src/lib/agent/sdk-message-handler.ts:877-890`, `922-937`) — an AWAITED effect stage that sets whether limit recovery is engaged, marks the result intercepted, records usage, and refreshes context usage when accepted, then short-circuits the rest of the turn-end path.
  ...
  3. The routing gates set `ctx.plan`.
  4. Successful-result side effects (`lastRateLimitInfo`/`lastSdkErrorTag` reset, `recordResultUsageMetadata` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:1063-1118`), circuit-breaker `markSuccess` on non-zero usage, `acknowledgeOldestQueuedUserOnTurnEnd` (`:464-523`), `acknowledgedPersistedUserThisTurn` reset, `session.errorClear` publish (`:1147-1170`)) run after usage/ack selection and before the idle/timer/replay/flag effects.
  5. Turn-end idle/timer/replay/flag effects (`setIdle`, timer cancel/rearm, queue replay, flag transitions) consume `plan`.
...
- **Pure core design:**
  ...
  Every gate begins with `if (ctx.route !== null) return ctx;` (first-match-wins), because the predicates overlap: a `park` while `waitingForInput` also matches the generic `park` gate, and an over-budget `park` or `awaiting_acceptance` also matches its later generic requeue gate. The downstream effect/mutation stages (`deadLetter` check, `settleSkipped`, `jobQueue.requeue`/`requeueParked`/`requeueAs`) also no-op when `ctx.route` is already set, so a dead-letter or waiting-input route is never overwritten by a later requeue stage.
...
- **Step-by-step migration:**
  ...
  2. Port each branch to a self-guarding stage (`if (ctx.route !== null) return ctx;`) that sets `ctx.route` to the chosen `HandlerOutcomeRoute`; `MESSAGE_DELIVERY_PARK_MS` stays a module-level constant.
  3. Port the downstream effect stages (`deadLetter`, `settleSkipped`, `requeue`/`requeueParked`/`requeueAs`) so each checks `ctx.route` and no-ops when a route is already set.
- **Tests:** `handler-outcome-routing.test.ts` is parity. Add `feed-steer-outcome-gates.test.ts` covering `park` with and without `waitingForInput`, at and over `MAX_STEER_PARKS`, `awaiting_acceptance` at and over `MAX_ACP_STEER_PARKS`, `promote`, `aborted`, and `consumed`. Add a contract test that a `park` with `waitingForInput` is not overwritten by the later generic `requeue` stage and that an over-budget `awaiting_acceptance` is not overwritten by the later `requeueParked` stage.
- **Risks/caveats:** The park-budget thresholds (`MAX_STEER_PARKS`, `MAX_ACP_STEER_PARKS`) are the boundary between requeue and dead-letter. The `message-delivery.handler.ts` also has a pre-check for `MAX_ACP_STEER_PARKS` when `sendStatus === 'submitted'`; do not change that. `waitingForInput` bypasses the normal steer park budget, so the `applyParkWaitingInputGate` must fire before the budget gate. The self-guard is load-bearing because these gates populate `ctx.route`, not `ctx.decision`, so `decisionRun` does not halt after the first match.
...
- **Scope:** Wire the COMPLETE async retry operation. Review correction (PR #2980): `query-runner.ts` calls the synchronous `decideQueryRetry` in two places in `executeQuery` — BEFORE awaiting `onRateLimitExhausted` (`packages/daemon/src/lib/agent/query-runner.ts:1039`) and AGAIN after the callback supplies `accepted`/`declined` (`:1170-1183`) — and a third time in `retryRouteChanged` (`:1666-1680`) to resnapshot the environment after awaited teardown work and immediately compare `route.action` to the expected action. The route classifier (`decideQueryRetry`) must remain synchronous for the resnapshot. This slice therefore introduces a NEW async `route-query-retry-operation` `stagedRun` (or async `superpipe`) that runs:
  ...
  3. For retry routes (`startup_timeout_retry`/`message_not_found_retry`/`transient_retry`/`provider_backoff`): an effect stage that runs `runRetryTeardown` with the route-specific options and returns (the retry superpipe handles its own finalizer idle; the operation does not continue to `setIdle`).
  4. For `rate_limit_handoff`: an effect stage that clears `messageQueue` (unless `finalizer.skipQueueClear`); an AWAITED `onRateLimitExhausted` effect; a `resnapshot`/`decide` stage that re-runs the synchronous `route-query-retry` `decisionRun` over the amended env (`hasRateLimitHandoff: false`, `rateLimitHandoffResult: 'accepted' | 'declined'`) and stamps a new `route`/`finalizer`; if the callback rejects, capture the error, treat the handoff result as `thrown`, run the `thrown` finalizer/recovery while suppressing terminal handling, then rethrow the original error so the query promise rejects (`packages/daemon/tests/unit/1-core/agent/query-runner.test.ts:2828-2847`).
  5. For `aborted_noop`/`api_validation`/`terminal` (and the final route after a non-retry `rate_limit_handoff`): an effect stage that clears `messageQueue` (unless `finalizer.skipQueueClear`); for `api_validation`/`terminal`, `beginTerminalIdle` unless `finalizer.skipBeginTerminalIdle`; for `api_validation`, `displayErrorAsAssistantMessage` unless `finalizer.skipErrorManager`; for `terminal`, `errorManager.handleError` unless `finalizer.skipErrorManager`; `setIdle` unless `finalizer.skipCatchIdle`.
  6. A final `finally`/`compensation` stage that runs the recovery-aware finalizer idle (`stateManager.setIdle()` if `!finalizer.skipFinalizerIdle(env)` and the current conditions allow) so rejecting handoffs and retry teardowns still surface the correct idle behavior (`packages/daemon/tests/unit/1-core/agent/query-runner.test.ts:2828-2847`).
...
- **Tests:** `query-retry-routing.test.ts` parity end-to-end (including the resnapped-env `skipFinalizerIdle` case); PR 15's declined-recompute contract must pass unchanged; the `retryRouteChanged` ownership-change resnapshot tests stay green (calling the synchronous `decideQueryRetry`); add a rejecting-callback wiring test mirroring `packages/daemon/tests/unit/1-core/agent/query-runner.test.ts:2828-2847` (handoff rejects, queue is cleared, `beginTerminalIdle`/`errorManager.handleError` are not called, and the finalizer idle runs once); add effect-stage rows for `messageQueue.clear()`, `runRetryTeardown`, terminal error handling, `setIdle`, and the finalizer idle.
- **Depends on:** PR 17.
...
- **Scope:** Review correction (PR #2980): complete phase 6 by wiring the extended `turn-end-pipeline.ts` into the SDK turn-end runtime path as an async mixed pipeline. The pipeline includes:
  1. The `onResultLimitError` handoff (`packages/daemon/src/lib/agent/sdk-message-handler.ts:877-890`, `922-937`) — an AWAITED effect stage that determines `isLimitRecoveryEngaged`, marks the result intercepted, records usage, and refreshes context usage when accepted, then short-circuits.
  ...
  3. Routing gates that set `plan`.
  4. Successful-result side effects (`lastRateLimitInfo`/`lastSdkErrorTag` reset, `recordResultUsageMetadata` at `:1063-1118`, circuit-breaker `markSuccess` on non-zero usage, `acknowledgeOldestQueuedUserOnTurnEnd` at `:464-523`, `acknowledgedPersistedUserThisTurn` reset, `session.errorClear` publish; `packages/daemon/src/lib/agent/sdk-message-handler.ts:1147-1170`) run after usage/ack selection and before idle/timer/replay/flag effects.
  5. Turn-end idle/timer/replay/flag effects (`setIdle`, timer cancel/rearm, queue replay, flag transitions) consuming `plan`.
...
- **Lands:** The complete SDK turn-end business path — limit-recovery handoff, usage/ack accounting, routing gates, successful-result side effects, AND turn-end idle/timer/replay/flag effects — runs as one composed operation in production.
- **Excludes:** `routeTurnEnd`'s parity wrapper (kept for the suite); turn-end-routing gate logic changes.
- **Tests:** `turn-end-pipeline.test.ts` extended with effect-stage rows (limit-recovery handoff accepted/declined, intercepted-result usage, successful-result side effects, idle transition, timer rearm, queue replay, usage accounting, and ack selection per plan arm); `turn-end-routing.test.ts` parity unchanged.
- **Depends on:** PR 20.
