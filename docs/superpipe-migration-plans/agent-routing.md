...
- **Shell/effect wiring:** The admission gate functions are called by the first `stagedRun` `snapshot`/`effect` stage, which is the entire body of a `withSessionLock` callback. That stage runs the admission gates; for `feed` and `awaiting_acceptance` it computes `generation` and calls `observer.reportStage('query_ready', { generation })` before either `messageQueue.admitWithId` or returning `awaiting_acceptance` (`packages/daemon/src/lib/agent/agent-session.ts:2574-2587`). For `feed` it then performs the `messageQueue.admitWithId` queue mutation and returns the feed-action snapshot (`acknowledgment`, `generation`, `clearEpoch`); for `awaiting_acceptance` it returns that kind immediately. The remaining top-level `stagedRun` effect stages run after the `withSessionLock` callback returns, so the per-session mutex is not held during the long acknowledgement wait:
  ...
  - Requeue: `messageQueue.requeueYielded`, then, for the `query_ended` acknowledgment-race outcome, `reopenDeliveryForRetry(messageUuid)` so the durable row is reopened, not just the in-memory entry (`packages/daemon/src/lib/agent/agent-session.ts:2625-2628`); then metrics (`deliveryMetrics.recordFeed`).
  - Post-ack consumption tail for an acknowledged non-ACP steer: report `sdk_admitted`, run `classifyAcknowledgedSteer`, call `markDeliveryConsumed`, and `signalDeliveryConsumed` (`packages/daemon/src/lib/agent/agent-session.ts:2642-2650`).
...
- **Pure core design:**
  - `applyClearThenFlushGate` — all four conditions true; sets `ctx.contextReset = { action: 'clear_then_flush' }`.
...
- **Step-by-step migration:**
  ...
  2. Replace `message-delivery-pipeline.ts:applyFlushContextResetGate` with the guard gate sequence in the outer `decisionRun` (`packages/daemon/src/lib/agent/message-delivery-pipeline.ts:181-186`). Each later guard must begin with `if (ctx.contextReset !== null) return ctx;`.
...
- **Scope:** Wire the COMPLETE steer-feed operation at `AgentSession.feedDeliverySteer` as one async `feed-delivery-steer` pipeline. The admission + `messageQueue.admitWithId` queue mutation form a single locked stage inside the existing `withSessionLock` (`packages/daemon/src/lib/agent/agent-session.ts:2560-2590` and `packages/daemon/src/lib/agent/message-delivery.ts:454-480`), because `withSessionLock` only releases the mutex in `finally` after its callback returns. That stage runs the admission gates; for `feed` and `awaiting_acceptance` it computes `generation` and calls `observer.reportStage('query_ready', { generation })` before either `messageQueue.admitWithId` or returning `awaiting_acceptance` (`packages/daemon/src/lib/agent/agent-session.ts:2574-2587`). For `feed` it then performs the `messageQueue.admitWithId` queue mutation and returns the feed-action snapshot (`acknowledgment`, `generation`, `clearEpoch`); for `awaiting_acceptance` it returns that kind immediately. The remaining top-level `stagedRun` effect stages run after the `withSessionLock` callback returns, so the per-session mutex is not held during the long acknowledgement wait:
  ...
  - Teardown revalidation (`packages/daemon/src/lib/agent/agent-session.ts:2633-2640`): if the session is no longer `processing`, terminal idle is pending, or the queue's clear epoch changed, call `reopenDeliveryForRetry(messageUuid)` and throw so the durable row is restored to `enqueued` and the job retries; retain the existing interrupt, terminal-idle, and bare-clear assertions in `packages/daemon/tests/unit/1-core/agent/agent-session.test.ts:6479-6505,6571-6627`.
  - Requeue: `messageQueue.requeueYielded`, then, for the `query_ended` acknowledgment-race outcome, `reopenDeliveryForRetry(messageUuid)` so the durable row is reopened, not just the in-memory entry (`packages/daemon/src/lib/agent/agent-session.ts:2625-2628`); then metrics (`deliveryMetrics.recordFeed`).
  - Post-ack consumption tail for an acknowledged non-ACP steer: report `sdk_admitted`, run `classifyAcknowledgedSteer`, call `markDeliveryConsumed`, and signal `signalDeliveryConsumed` (`packages/daemon/src/lib/agent/agent-session.ts:2642-2650`).
...
- **Scope:** Wire the COMPLETE async retry operation. Review correction (PR #2980): `query-runner.ts` calls the synchronous `decideQueryRetry` in two places in `executeQuery` — BEFORE awaiting `onRateLimitExhausted` (`packages/daemon/src/lib/agent/query-runner.ts:1039`) and AGAIN after the callback supplies `accepted`/`declined` (`:1170-1183`) — and a third time in `retryRouteChanged` (`:1666-1680`) to resnapshot the environment after awaited teardown work and immediately compare `route.action` to the expected action. The route classifier (`decideQueryRetry`) must remain synchronous for the resnapshot. This slice therefore introduces a NEW async `route-query-retry-operation` `stagedRun` (or async `superpipe`) that runs:
  ...
  6. A final `finally`/`compensation` stage that first runs the non-idle cleanup (`attemptTokens.invalidate`, `releaseStartupPermit`, clear timers, abort, `messageQueue.stop`, close `queryObject`, restore env vars) in the same order as the current outer `finally` (`packages/daemon/src/lib/agent/query-runner.ts:1224-1265`), then runs the recovery-aware `setIdle()` if `!finalizer.skipFinalizerIdle(env)` and the current conditions allow (`packages/daemon/src/lib/agent/query-runner.ts:1266-1276`) — the cleanup must precede the idle so delivery waiters cannot start replacement work while the failed query still owns its queue, controller, and transport.
...
- **Scope:** Review correction (PR #2980): complete phase 6 by wiring the extended `turn-end-pipeline.ts` into the SDK turn-end runtime path as an async mixed `stagedRun` (or async `superpipe`). The pipeline runs, in order:
  1. Pre-handoff effects (`packages/daemon/src/lib/agent/sdk-message-handler.ts:865-890`): if the message matches an armed clear (`observesArmedClearResult`), `cancelSuppressedResultTimer()` before any awaited work; then for every top-level result reset thinking-token tracking (`packages/daemon/src/lib/agent/sdk-message-handler.ts:880`); then, when a limit error is present, await `onResultLimitError` to determine `isLimitRecoveryEngaged` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:877-890`, `922-937`).
  ...
  3. Routing gates that set `plan` with fields `idleFence`, `earlySetIdle`, `finishTurn`, `allowQueueReplay`, etc.
  4. Pre-publication and idle-fencing effects (`packages/daemon/src/lib/agent/sdk-message-handler.ts:891-920`): set `lastResultWasSuccess` and reset `lastRateLimitInfo`/`lastSdkErrorTag` before any publication (`packages/daemon/src/lib/agent/sdk-message-handler.ts:891-893`); then `beginTerminalIdle` if `plan.idleFence`; then publish `state.sdkMessages.delta` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:900-908`); then await `internalEventBus.publish('sdk.message')` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:911-920`) with a `compensate` that calls `clearIdleSuppression()` before rethrowing when the result confirms an armed clear (`confirmsArmedClear`). If `isLimitRecoveryEngaged` is accepted, mark the result intercepted with the `limitBillingTerminal` flag carried from `limitError.hint.billingTerminal` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:878-883,923-930`) inside a nonfatal `try/catch` that logs and continues, then record usage, refresh context usage, and short-circuit the rest.
  5. Successful-result side effects (`recordResultUsageMetadata` at `:1063-1118`, circuit-breaker `markSuccess` on non-zero usage, `acknowledgeOldestQueuedUserOnTurnEnd` at `:464-523` — which resnapshots `activeDeliveryMessageUuids`, `hasYielded`, and `hasPendingOrClaimed` immediately before `markDeliveriesConsumedAtTurnEnd`, then publishes `messages.statusChanged` and replays the consumed user messages, `acknowledgedPersistedUserThisTurn` reset, `session.errorClear` publish at `:1147-1170`). Each awaited effect has a `compensate`/`unwind` that calls `clearIdleSuppression()` before rethrowing ONLY when `confirmsArmedClear` is true, matching the current `handleResultMessage` catch (`packages/daemon/src/lib/agent/sdk-message-handler.ts:1126-1134`).
  ...
  The current `sdk-message-handler.ts` imperative turn-end cascade (`:870-938` and `:1140-1296`) is replaced by this single pipeline, so production never consumes a decision-only fragment. PRs 19–20's gates may exist test-only until this slice lands, but the phase is not done without it.
