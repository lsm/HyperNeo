...
- **Shell/effect wiring:** The exported `routeTurnEnd(flags, event, ctx)` stays as a thin wrapper over the routing-only gate run in `turn-end-routing.ts` (returning `ctx.plan` with a `makePlan(flags)` fallback); it does NOT call the full `turn-end-pipeline.ts` runner, because it does not have the `usageState`, `resultUsage`, `ackRows`, etc. required by the full `TurnEndPipelineInput` (`packages/daemon/src/lib/agent/turn-end-pipeline.ts:24-43`). The same extracted routing gate functions are also composed directly into `turn-end-pipeline.ts`'s existing gate list in the routing position. The complete SDK turn-end pipeline is an async mixed `stagedRun` (or async `superpipe`) that runs, in order:
  1. The `onResultLimitError` handoff for top-level limit results (`packages/daemon/src/lib/agent/sdk-message-handler.ts:877-890`, `922-937`) — an AWAITED effect stage that sets whether limit recovery is engaged, marks the result intercepted, records usage, refreshes context usage, and publishes the SDK message (`messageHub.event` at `:900-908` and `internalEventBus.publish('sdk.message')` at `:911-920`) when accepted, then short-circuits the rest of the turn-end path.
  ...
  4. Idle-fencing effects (`packages/daemon/src/lib/agent/sdk-message-handler.ts:896-899` and `:947-951`): `beginTerminalIdle` if `plan.idleFence`, then publish `state.sdkMessages.delta` and `sdk.message` (`:900-920`), then `setIdle` if `plan.earlySetIdle`. These must run before the result is processed by user/assistant/system handlers and before the successful-result side effects, preserving the legacy ordering pinned in `sdk-message-handler-flag-machine.test.ts`.
  5. Successful-result side effects (`lastRateLimitInfo`/`lastSdkErrorTag` reset, `recordResultUsageMetadata` (`packages/daemon/src/lib/agent/sdk-message-handler.ts:1063-1118`), circuit-breaker `markSuccess` on non-zero usage, `acknowledgeOldestQueuedUserOnTurnEnd` (`:464-523`), `acknowledgedPersistedUserThisTurn` reset, `session.errorClear` publish (`:1147-1170`)). Each awaited effect has a `compensate` (or `stagedRun` unwind) that calls `clearIdleSuppression()` before rethrowing, matching the current `handleResultMessage` catch (`packages/daemon/src/lib/agent/sdk-message-handler.ts:1126-1134`).
  6. Turn-end idle/timer/replay/flag effects (`finishTurn` with `setIdle` and queue replay if `plan.finishTurn && plan.allowQueueReplay`, timer cancel/rearm, flag transitions) consume the remaining `plan` fields.
...
- **Shell/effect wiring:** The admission gate functions are called by the first `stagedRun` `snapshot`/`effect` stage, which is the entire body of a `withSessionLock` callback. That stage runs the admission gates, then performs the `messageQueue.admitWithId` queue mutation, and returns the feed-action snapshot (`acknowledgment`, `generation`, `clearEpoch`) to the rest of the pipeline. The remaining top-level `stagedRun` effect stages run after the `withSessionLock` callback returns, so the per-session mutex is not held during the long acknowledgement wait:
  - Acknowledgment waiting (`packages/daemon/src/lib/agent/agent-session.ts:2609-2622`).
  - Teardown revalidation (`packages/daemon/src/lib/agent/agent-session.ts:2633-2640`).
  - Requeue (`messageQueue.requeueYielded`) and metrics (`deliveryMetrics.recordFeed`).
  - Post-ack consumption tail for an acknowledged non-ACP steer: report `sdk_admitted`, `classifyAcknowledgedSteer`, `markDeliveryConsumed`, and `signalDeliveryConsumed` (`packages/daemon/src/lib/agent/agent-session.ts:2642-2650`).
...
- **Scope:** Wire the COMPLETE steer-feed operation at `AgentSession.feedDeliverySteer` as one async `feed-delivery-steer` pipeline. The admission + `messageQueue.admitWithId` queue mutation form a single locked stage inside the existing `withSessionLock` (`packages/daemon/src/lib/agent/agent-session.ts:2560-2590` and `packages/daemon/src/lib/agent/message-delivery.ts:454-480`), because `withSessionLock` only releases the mutex in `finally` after its callback returns. That locked stage returns the feed-action (`acknowledgment`, `generation`, `clearEpoch`) to the rest of the pipeline. The acknowledgment-wait, teardown-revalidation, requeue, metrics, and post-ack consumption stages then run outside the lock (the consumption tail for an acknowledged non-ACP steer reports `sdk_admitted`, runs `classifyAcknowledgedSteer`, calls `markDeliveryConsumed`, and signals `signalDeliveryConsumed`; `packages/daemon/src/lib/agent/agent-session.ts:2642-2650`). The complete pipeline may use `stagedRun` or an async `superpipe`; it is never an admission-only wiring.
...
- **Tests:** The PR 5 matrix end-to-end through the wired pipeline; wiring tests for the ack-waiting, teardown-revalidation, requeue, and metrics stages; a `feed-delivery-steer-acknowledged.test.ts` that asserts the post-ack consumption tail runs for a non-ACP steer and that `signalDeliveryConsumed` is published.
- **Depends on:** PR 6.
...
- **Scope:** Wire the COMPLETE async retry operation. Review correction (PR #2980): `query-runner.ts` calls the synchronous `decideQueryRetry` in two places in `executeQuery` — BEFORE awaiting `onRateLimitExhausted` (`packages/daemon/src/lib/agent/query-runner.ts:1039`) and AGAIN after the callback supplies `accepted`/`declined` (`:1170-1183`) — and a third time in `retryRouteChanged` (`:1666-1680`) to resnapshot the environment after awaited teardown work and immediately compare `route.action` to the expected action. The route classifier (`decideQueryRetry`) must remain synchronous for the resnapshot. This slice therefore introduces a NEW async `route-query-retry-operation` `stagedRun` (or async `superpipe`) that runs:
  ...
  4. For `rate_limit_handoff`: an effect stage that clears `messageQueue` (unless `finalizer.skipQueueClear`); an AWAITED `onRateLimitExhausted` effect; a `resnapshot`/`decide` stage that re-runs the synchronous `route-query-retry` `decisionRun` with `env.rateLimitHandoffResult` set to the callback outcome (`accepted`/`declined`/`thrown`) — the `applyRateLimitHandoffFinalizeGate` reclassifies only on `declined` (by setting `hasRateLimitHandoff: false` inline within the same run), while `accepted` and `thrown` finalize the original `rate_limit_handoff` route; if the callback rejects, capture the error, treat the handoff result as `thrown`, run the `thrown` finalizer/recovery while suppressing terminal handling, then rethrow the original error so the query promise rejects (`packages/daemon/tests/unit/1-core/agent/query-runner.test.ts:2828-2847`).
...
- **Tests:** `query-retry-routing.test.ts` parity end-to-end (including the resnapped-env `skipFinalizerIdle` case); PR 15's declined-recompute contract must pass unchanged; the `retryRouteChanged` ownership-change resnapshot tests stay green (calling the synchronous `decideQueryRetry`); add a `message_not_found_retry` wiring test asserting `consumePendingResumeSessionAt()` is called before `runRetryTeardown`; add accepted/thrown handoff wiring tests asserting the original `rate_limit_handoff` route is preserved (only `declined` reclassifies with `hasRateLimitHandoff: false`); add a rejecting-callback wiring test mirroring `packages/daemon/tests/unit/1-core/agent/query-runner.test.ts:2828-2847` (handoff rejects, queue is cleared, `beginTerminalIdle`/`errorManager.handleError` are not called, and the finalizer idle runs exactly once after the outer `finally` setIdle is folded into the operation); add effect-stage rows for `messageQueue.clear()`, `runRetryTeardown`, terminal error handling, `setIdle`, and the finalizer idle.
- **Depends on:** PR 17.
...
- **Scope:** Review correction (PR #2980): complete phase 6 by wiring the extended `turn-end-pipeline.ts` into the SDK turn-end runtime path as an async mixed `stagedRun` (or async `superpipe`). The pipeline runs, in order:
  1. The `onResultLimitError` handoff (`packages/daemon/src/lib/agent/sdk-message-handler.ts:877-890`, `922-937`) — an AWAITED effect stage that determines `isLimitRecoveryEngaged`, marks the result intercepted, records usage, refreshes context usage, and publishes the SDK message (`messageHub.event` at `:900-908` and `internalEventBus.publish('sdk.message')` at `:911-920`) when accepted, then short-circuits.
  2. `applyUsageAccountingGate` (`packages/daemon/src/lib/agent/turn-end-pipeline.ts:44-54`) and `applyAckSelectionGate` (`:81-96`) to produce `usage` and `ackSelection`.
  3. Routing gates that set `plan` with fields `idleFence`, `earlySetIdle`, `finishTurn`, `allowQueueReplay`, etc.
  4. Idle-fencing effects (`packages/daemon/src/lib/agent/sdk-message-handler.ts:896-899` and `:947-951`): `beginTerminalIdle` if `plan.idleFence`, then publish `state.sdkMessages.delta` and `sdk.message`, then `setIdle` if `plan.earlySetIdle`.
  5. Successful-result side effects (`lastRateLimitInfo`/`lastSdkErrorTag` reset, `recordResultUsageMetadata` at `:1063-1118`, circuit-breaker `markSuccess` on non-zero usage, `acknowledgeOldestQueuedUserOnTurnEnd` at `:464-523`, `acknowledgedPersistedUserThisTurn` reset, `session.errorClear` publish at `:1147-1170`). Each awaited effect has a `compensate`/`unwind` that calls `clearIdleSuppression()` before rethrowing, matching the current `handleResultMessage` catch (`packages/daemon/src/lib/agent/sdk-message-handler.ts:1126-1134`).
  6. Turn-end idle/timer/replay/flag effects (`finishTurn` with `setIdle` and queue replay if `plan.finishTurn && plan.allowQueueReplay`, timer cancel/rearm, flag transitions) consuming the remaining `plan` fields.
...
- **Lands:** The complete SDK turn-end business path — limit-recovery handoff (with publication), usage/ack accounting, routing gates, idle-fencing, successful-result side effects (with compensation), AND turn-end idle/timer/replay/flag effects — runs as one composed operation in production.
- **Excludes:** `routeTurnEnd`'s parity wrapper (kept for the suite); turn-end-routing gate logic changes.
- **Tests:** `turn-end-pipeline.test.ts` extended with effect-stage rows (limit-recovery handoff accepted/declined with SDK message publication, intercepted-result usage, idle-fencing order, successful-result side effects including armed-clear failure compensation, `finishTurn` idle/replay, timer rearm, usage accounting, and ack selection per plan arm); `turn-end-routing.test.ts` parity unchanged.
- **Depends on:** PR 20.
