...
- **At spawn**, one task works in exactly one repo: `createTaskWorktree`
  ...
  **fails closed**: a worktree-creation error aborts the spawn rather than falling back to
  ...
  agent in the shared space workspace`). The resolved repo root is verified to sit inside a
  usable git repository (root, bare repository, or subdirectory resolved to its checkout)
  before any worktree side effects — `Workspace is not a git repository: ...` thrown as
  `WorkspaceNotGitRepositoryError` — and that unusable-workspace failure classifies as a
  `PermanentSpawnError`: the runtime cancels the execution and blocks the task once with
  that reason instead of crash-retrying it into `agent_crashed` (#3589). Other
  worktree-creation failures keep the bounded spawn-retry classification. Crashed creations
  are recovered idempotently via a `hyperneo-claim` marker written into the worktree's
  private git dir.
- **Creation fail-fast (#3589)**: `create_standalone_task` — both the space-agent tool and
  the node-agent tool, which also accepts an explicit `workspace` argument — validates the
  default before recording an unpinned task: in a space with more than one registered
  workspace whose primary cannot host a task worktree, omitting `workspace` is rejected
  with the list of usable workspaces
  (`SpaceWorkspaceManager.validateDefaultTaskWorkspace`) instead of silently defaulting to
  a primary that would fail at spawn. Single-workspace spaces and explicit `workspace`
