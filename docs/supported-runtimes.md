...
| --- | --- | --- | --- |
| Bun | 1.4.2 (`packageManager` in root `package.json`) | **Release runtime.** All dev/prod entry points, tests, and compiled binaries. | Full CI: check job, daemon unit/online shards, web shard, release build |
...
- **Bun**: root `package.json` `packageManager: "bun@1.4.2"` plus `bun-version: 1.4.2` in every CI setup step.
...
- **Copilot CLI not embedded.** The binary imports `@github/copilot-sdk`'s JS but `CopilotClient.start()` spawns the SDK's platform-specific runtime from the `@github/copilot-sdk-<platform>` optional dependency packages (lockfile records platform optional binaries). Without `--include` (which we cannot use without re-adding the workspace prodDeps balloon), the binary cannot start a Copilot session on a machine without `copilot` on PATH. The HTTP/WS/migration smoke is unaffected because the provider registration is lazy; the failure surfaces only when an `anthropic-copilot` model is selected.
