...
1. **C3-PR1 — contract + RPC seam (daemon).** Generalize
   ...
   too — Ollama (`/api/tags`), Copilot (`client.listModels()`), and
   ...
   (`customEndpoints.listModels` + `normalizeModelList`) — their `getModels()`
...
2. **C3-PR2 — discovery cache (daemon).** Per-provider discovered-list cache
   ...
   **replaces the provider's slice in the global `modelsCache` before
   ...
   `providers.config_json` so restarts don't blank curated setups — keyed
...
4. **C3-PR4 — picker flow.** Curation applies inside the `model-service` merge
   ...
   validation there with a direct `session.create` regression test. Pickers
