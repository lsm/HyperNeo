...
- Amplifier: at startup, if the provider's own `getCredentials()` is null,
  ...
  nothing ever calls `setCredentials` to clear the null. `importFromCodexAuth`
  can fail transiently (file read at `:856-861`, or the network refresh inside
  it at `:883`), hardening the null.
...
- On the next `models.list`, `detectStrandedProviders` gives each missing
  ...
  **only** by the global `clearModelsCache()` (`:274`) — i.e. by a provider
  create/update/delete. Session-scoped cache clears don't touch it.
...
| Availability machinery | per-provider `isAvailable` (inventory in ACUTE 1); OAuth refresh scheduler D`lib/credentials/oauth-refresh-scheduler.ts` (5-min tick, 3-strike retry cap :66-90) | see ACUTE 1 inventory | providers.list/test/healthCheck, auth.providers, model-service | thin — **no tests** for stranded lifecycle, retry-cap, import cache | low |
...
4. **C3-PR4 — picker flow.** Curation applies inside the `model-service` merge
   ...
   (`:435-436`) is also filtered so switching stays consistent. Pickers need
   **zero changes** (they consume `models.list`). Pin: with curation set,
   `models.list` shows only the subset; without, current behavior.
...
4. **C4 — availability hygiene sweep (small, optional):** make
   ...
   C2-PR3 + a timeout on custom-endpoint probes in the `providers.list` path);
   consider persisting `OAuthRefreshScheduler` retry state across restarts.
   Only if C2-PR3 leaves gaps.
