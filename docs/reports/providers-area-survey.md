...
| **C1-PR2 (fix, model-service)** | Decouple listing from probing | In `loadModelsFromProviders` (model-service.ts:183-189): when `isAvailable()=true` and `getModels()` **rejects or returns an empty list**, fall back in two steps — first `provider.getCachedModels?.()`, then the provider's static metadata (`STATIC_MODEL_METADATA` already exists, `:65-71`; Minimax missing from it — add; copilot is missing too: its `getModels()` catches embedded-server start failure and returns `[]` rather than rejecting — copilot `provider.ts:241-245` — and the static seam holds only the 1-entry legacy overlay, so its 7-entry `COPILOT_ANTHROPIC_MODELS` (copilot `provider.ts:56-134`, exported for this) must be added; Anthropic has the same swallow-shaped gap — `getModels()` catches every `loadModelsFromSdk()` error and returns `[]` (anthropic-provider.ts:124-138), so its failure must surface through the same failure-set propagation rather than vanishing). The empty-result case matters for the same reason as the rejection case: an available provider yielding no models is a failed fetch, not a real empty catalog. The cached-models step is the ACP fallback: ACP has no static entry by design (its list is per-command and dynamic), but its curated `config_json.models` is hydrated into `cachedModels` at startup sync (acp-provider.ts:137-173, 233-235), so a failed subprocess probe still lists the curated set; an empty configured `models: []` also maps to the synthetic default (acp-provider.ts:164-167). The command-only case — no `models` key, so `parseAcpConfig` yields `undefined` (provider-sync.ts:21-33) and `setAcpModels(undefined)` leaves `cachedModels` null (acp-provider.ts:169-172) — needs one extra step: `getCachedModels()` must synthesize the default entry when `cachedModels` is null but a command identity is configured, mirroring the healthy `getModels()` tail (acp-provider.ts:221-225) and the null branch of `updateAcpModelCache` (acp/acp-query-runner.ts:1112-1118), which already splices that same synthetic default into the global cache — so the synthesis is behavior-consistent with both existing consumers. Custom endpoints likewise have no static entry; a two-line `getCachedModels()` returning their configured `config.models` covers them, since the probe only verifies a list that is config, not discovery (custom-endpoint-provider.ts:183-186). Single seam, no provider-specific branches beyond that accessor. Tradeoff (flag in PR): with the fallback, a failing provider stays listed and selectable — and as written nothing surfaces the failure: the probe-based providers' `getAuthStatus()` checks only key *presence* (glm-provider.ts:150-157, kimi/deepseek/minimax likewise), the probe rejection is dropped by `allSettled`, and `loadModelsFromProviders` writes no health status, so an invalid-key provider looks authenticated and healthy until a real request fails. The fix must therefore propagate the failure recorded by C1-PR3(c)'s failure set into `getAuthStatus` using C2-PR2(e)'s `errorKind`: probe 401/403 → `'credential'` (picker hides the provider, matching the definitive-hide rule), 5xx/timeout/network → `'transient'` (models stay listed via the fallback, provider shown degraded). Recording must also **notify**: `providers.changed` at mutation time can fire before the concurrent `models.list` probe records the failure, and a definitive failure arms no retry that would publish a later event — so publish a status-change event (or re-publish `providers.changed`) when a failure is first recorded or changes kind; otherwise auth-status consumers, which load only on connection change or `providers.changed`, never observe the propagated failure and the provider stays green indefinitely — and mind the consumer split: `NewChatModelPicker` loads auth status only on `isConnected` change and has no `providers.changed` listener (W`components/NewChatModelPicker.tsx:54-73`; the `useModelSwitcher` listener refreshes models only, W`hooks/useModelSwitcher.ts:278-287`), so the fix must also add an auth-status subscription/refetch there, or publish a status event every picker observes; otherwise the new-chat picker keeps the stale authenticated status and the fallback models stay selectable there until reconnection. |
| **C1-PR3 (fix, retry + TTL)** | Make recovery self-healing | (a) Replace the `refreshedMissingProviders` Set with timestamped attempts + retry backoff (e.g. 60s), **with a scheduled trigger — not just timestamps**: arm an unref'd per-provider timer when a failure is recorded that re-fetches that provider when the backoff expires and, on success, replaces its cache slice and publishes `providers.changed` — timestamps alone only permit a retry when a later `models.list` reaches the detection path, and a user sitting at an already-open picker generates no such request, so recovery would still wait on navigation/reconnection/manual action; the timer must be lifecycle-safe: provider mutation and global-clear paths cancel pending timers, and the timer callback re-resolves the provider from the registry and re-checks its configuration before fetching — and, because an in-flight probe cannot be cancelled, re-verifies the provider/config generation again immediately before replacing the cache slice (or mutation paths abort/await in-flight retry work), so a slow probe completing after a disable/reconfigure cannot install stale results past the mutation's cache clear; a closure over the old instance would otherwise resurrect a disabled/reconfigured provider's models into the global cache after the backoff expires. Retries arm only for **transient** failures — a definitive failure stays dormant until credentials or provider configuration change, since `isAvailable()` remains true on key presence and only a mutation can repair it: HTTP 401/403 credential rejections, and ACP command parse/spawn/protocol errors (a configured-but-broken executable is permanent until the provider config changes, so the timer must not re-spawn it every backoff; ACP timeouts and crashes remain transient). Otherwise the timer re-probes an invalid key or broken command forever. The dormancy rule also needs its automatic exit: `OAuthRefreshScheduler` performs exactly the credential change dormancy waits for yet publishes nothing and clears no caches (oauth-refresh-scheduler.ts:76-84), so a provider that recorded a definitive 401 stays hidden after its token self-heals — treat a successful scheduled refresh of a provider with a recorded definitive failure as a credential mutation: clear the failure, refresh its cache slice, publish the status-change event. Do **not** clear markers on every refresh completion: a provider whose `isAvailable()` stays true while `getModels()` keeps failing would be re-detected as stranded on every cached `models.list`, re-probing and re-triggering an upstream refresh (hammering an already failing or rate-limited service); clearing a provider's marker is safe only when that provider actually recovered in the refresh; (b) codex `cachedBridgeAuth=null` gets a TTL (5 min, matching copilot's `tokenCache` pattern) or re-checks on auth.json mtime change; (c) once C1-PR2's fallbacks populate the cache with the failed provider's models, `detectStrandedProviders` considers that provider present and never creates a retry attempt for it — so the failure must be recorded by `loadModelsFromProviders` itself (return a per-provider failure set alongside the models, tracked with the same backoff timestamps and retried independently of cache presence), leaving the stranded-probe path only for providers absent from the cache entirely. Re-run PR1 pins inverted: GLM present via fallback during probe failure; codex re-reads after credentials appear. |
...
1. **C3-PR1 — contract + RPC seam (daemon).** Generalize
   ...
   shape, but each provider declares its **own listing URL and auth strategy**
   — do not derive the listing endpoint from the Anthropic probe base: Kimi
   deliberately separates them (global `https://api.moonshot.ai/anthropic`
   for messages vs `https://api.moonshot.ai/v1` for listing,
   kimi-provider.ts:21-35 — reusing the probe base would request
   `/anthropic/v1/models` and break discovery); the shared helper covers the
   fetch/normalize plumbing only). Existing dynamic providers need adapters
   too — Anthropic (SDK `supportedModels()`, cached, anthropic-provider.ts:124-167),
   Ollama (`/api/tags`), Copilot (`client.listModels()`), and
   OpenRouter (`/api/v1/models/user`) currently expose their lists only
   through `getModels()`, so either each gets a `listRemoteModels()` adapter
   — and the adapters must be force-capable: bypassing the per-provider
   cache and propagating fetch failures rather than serving cached/fallback
   lists (Ollama caches 5 min, Copilot 5 min, OpenRouter indefinitely, and
   all three convert remote errors into fallback lists, so a plain
   `getModels()` fallback is not an explicit refresh; custom endpoints route
   through the existing remote-list implementation
   (`customEndpoints.listModels` + `normalizeModelList` — which must itself
   gain a force mode: it serves a 30 s `modelListCache`
   (custom-endpoint-handlers.ts:24-25,374-382), so an explicit refresh would
   otherwise return the cached list) — their `getModels()`
   merely probes and returns the configured `config.models`,
   custom-endpoint-provider.ts:183-186, so it cannot discover new candidates)
   — or the C3-PR3 editor must be
   explicitly scoped to providers this RPC supports — as written, opening the
   editor for them would have nothing to fetch against. Routing acceptance is part of the contract: providers
...
2. **C3-PR2 — discovery cache (daemon).** Per-provider discovered-list cache
   ...
   **replaces the provider's slice in the global `modelsCache` before
   ...
   refresh a provider that is already present — and only once the global
   cache is initialized, or while explicitly preserving an uninitialized
   state: the pattern treats a missing cache as `[]`
   (acp/acp-query-runner.ts:1100-1117), so a slice replace racing ahead of
   `initializeModels` creates a partial cache whose mere existence makes
   `initializeModels` early-return (model-service.ts:220-225), leaving every
   other provider to stranded recovery instead of the normal initial load),
   then publishes
   ...
   `providers.config_json` so restarts don't blank curated setups — bounded
   to fit the existing 64 KiB `configJson` limit (`MAX_JSON_FIELD_LEN`,
   provider-handlers.ts:33,55; an uncapped catalog serialized as last-good —
   OpenRouter today — would otherwise fail every subsequent
   `providers.update` validation; cap the persisted list to ids+names with a
   truncation marker — **curated-first ordering**: every configured
   `config_json.models` id persists before any discovered overflow, so
   truncation can never drop a curated ID and restart-during-outage always
   finds metadata for the user's selection — or use a dedicated table as the
   sanctioned escape hatch) — and keyed
   by/invalidated on the effective provider configuration (region, base URL,
   ACP command, credential identity): a Kimi china→global switch changes only
   `defaultRegion` today, and without invalidation the next merge can mix
   models discovered from the old regional endpoint into the new one; clear
   both the in-memory cache and the persisted list on relevant provider
   updates — and revalidate after the fetch: a refresh can snapshot and fetch
   against the old region/baseUrl/command/credential while a concurrent
   provider update changes the configuration and clears the caches, so the
   generation must be re-checked before persisting/applying, mirroring the
   retry-timer generation recheck in C1-PR3(a).
   **ADR shape:** fetch+cache = `requestRun`-shaped staged flow (snapshot →
   fetch → cache → notify; P8) — note this is a **multi-write** sequence, not
   a single idempotent write: the per-provider cache, the global slice, and
   the persisted last-good list must commit in order — persist the last-good
   list first, then mutate in-memory state, and publish `providers.changed`
   only once durable and live state are coherent; otherwise a mid-sequence
   failure leaves pickers showing unpersisted models that a restart reverts
   (or, reversed, persisted-but-not-live state), and per the ADR's `stagedRun`
   effect-stage conditions the flow needs compensation or a versioned
   last-good re-read on restart; the cache-merge decision itself = P1
   transform.
3. **C3-PR3 — curation storage + UI.** Generalize the ACP precedent:
   ...
   or the editor must prohibit saving an empty selection. Either way,
   reconcile with C1-PR2's empty-is-failure rule: a configured curation —
   including the intentional empty set — counts as a **successful** load; only
   a rejection or an empty result with *no* configured curation records a
   failure and arms retries. And carry successful-empty through the global
   cache: `initializeModels` treats an aggregate `models.length === 0` as an
   error and installs `FALLBACK_MODELS` (model-service.ts:230-243), while
   `refreshModels` retains the previous nonempty cache (:318-331) — with
   every enabled provider intentionally curated to `[]`, hidden models would
   reappear or remain visible, so the global update must bypass the ordinary
   fallback/shrink paths for a successful empty load. Existing
...
4. **C3-PR4 — picker flow.** Curation applies inside the `model-service` merge
   ...
   `session.model.switch` regression test for a curated-out model, plus
   session creation: `SessionLifecycle.getValidatedModelId` returns an
   unknown requested model unchanged when an explicit provider is supplied
   (D`lib/session/session-lifecycle.ts:1029-1051`) and `session.create`
   performs no `isValidModel` check, so apply the same curation-aware
   validation there with a direct `session.create` regression test — and on
   the SDK refusal-fallback path too: `QueryOptionsBuilder` passes
   `session.config.fallbackModel` to the SDK unvalidated
   (D`lib/agent/query-options-builder.ts:301-328`), so a curated-out fallback
   model must be filtered/revalidated at build time. `session.update` is
   the remaining bypass: it forwards `config` into `sessionManager.updateSession`
   with no `isValidModel`/curation gate (D`lib/rpc-handlers/session-handlers.ts:324-353`)
   while `QueryRunner` executes `session.config.model` directly
   (D`lib/agent/query-runner.ts:456`) — so validate curation-aware whenever
   `config.model` is present, and define behavior for already-persisted
   sessions whose configured model becomes curated-out (revalidate-and-error
   at query start, or pin-and-flag), each with a direct regression test. Pickers
...
1. **C1 — stuck-provider recovery** (acute; 3 PRs above). Highest user pain; PR1
   and PR2 are small — but no longer independent: PR2's status propagation
   requires the failure recording introduced as C1-PR3(c), so land that
   recording inside PR2 (moving it out of PR3) or sequence PR2 behind PR3;
   otherwise an invalid-key provider looks authenticated through its static
