...
- `preferredWorkflowId` resolved by the built-in workflow's **canonical template identity** (`templateSlug: 'learning-workflow'`), not by display name or handle. An existing Space may already contain a user workflow named `Learning Workflow` or using handle `learning-workflow`; `createWorkflow` rejects duplicates on those unique fields, and resolving by handle could fail or select the unrelated custom workflow—bypassing the secured Learner node and filtered MCP provisioning. The built-in seed carries a collision-safe `templateSlug` (distinct from display name/handle), the migration seeds under that identity despite name/handle conflicts, and automation resolves `preferredWorkflowId` to that seeded row. Because built-in workflows retain their template identity while allowing in-place node/prompt/route/agent-slot edits, resolving by `templateSlug` alone could still select a customized topology (an added non-Learner node with ordinary provisioning, or a new route). At dispatch the automation therefore also validates an **expected template hash and canonical Learner-only topology** (exactly one `Learn` node, no extra nodes/routes/agent slots, no `postApproval` route) against the resolved row, and refuses to dispatch on mismatch—or the built-in Learning Workflow is made immutable. Because the workflow row can be edited in the window between dispatch validation and task materialization or worker spawn, runtime re-validates that same hash/topology on every initial spawn and rehydrate before attaching tools (refusing to start the run on mismatch), or the built-in is immutable.
...
An abandoned learning task (cancelled or archived before `done`) must release its goal/scope lease without losing the work it had claimed—and without duplicating work it has already committed, and without resuming business side effects the user just stopped. Prefer staging all gated cycle mutations (episode accept/dismiss, lesson activation, rollup, materialization) into the single terminal transaction so a pre-terminal abandonment has committed nothing to duplicate or resume. If the implementation mutates incrementally instead, cancellation reconciles **only already-committed state** (records it, advances the processed set through evidence that committed tail actually examined, and deduplicates via cycle-key/snapshot invariants) and **leaves uncommitted business effects cancelled**—it must not auto-resume the remaining tail (no rollup, no materialization, no further dispositions) after an explicit cancellation, since the cancelled task may not carry finalized recommendations for those steps and the user has stopped the run. Only a cycle with no committed effect may requeue its full frozen claim and unmerged trigger contexts to the durable pending-trigger set (or reserve a replacement task) before releasing the lease.
  ...
  verificationMethod: {
    ...
    window?: {
      ...
      completionPredicate?: {
        ...
        // operators (not an arbitrary string): { metricKey, op, value } conjuncts, evaluated
        // deterministically by the service at the due point. Acceptance rejects a predicate
        // that references unknown metric keys or unsupported operators.
