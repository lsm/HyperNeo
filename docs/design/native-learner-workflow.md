...
interface TaskProposal {
  ...
  verificationMethod: {
    ...
    window?: {
      ...
      // schedulable due point. A taskCreation/taskCompletion anchor is rejected at acceptance
      // unless a created task exists for the proposal (level-2 acceptance without materialization
      // must use the 'acceptance' anchor), since an unmaterialized task-anchored window has no
      // timestamp and would never become due or could be marked due before the intervention exists.
    };
  } | null;
  acceptedSnapshot: {
    title: string;
