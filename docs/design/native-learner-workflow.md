...
interface TaskProposal {
  ...
  verificationMethod: {
    ...
    window?: {
      ...
      // task does not yet exist: it stays *unresolved* until the proposal is materialized (and,
      // for taskCompletion, until that task completes), at which point the anchor timestamp is
      // stamped; onSelfNag skips proposals whose anchor is still unresolved (not yet due), so a
      // new proposal can declare a task-anchored window without being rejected at acceptance.
    };
  } | null;
  acceptedSnapshot: {
    title: string;
