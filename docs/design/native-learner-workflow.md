...
An abandoned learning task (cancelled or archived before `done`) must release its goal/scope lease without losing the work it had claimed. The cancellation/archive transition atomically returns the frozen evidence claim and any unmerged trigger contexts to the durable pending-trigger set (or reserves a replacement task in the same transaction) before releasing the lease; the cursor is not advanced. The reconciler scans not only completed cycles missing terminal markers but also leased cycles whose task is terminal-abandoned, and re-dispatches the requeued claim, so an abandoned window is retried rather than permanently occupying the scope or being silently dropped.
...
7. If, after trigger attachment, there is still no new evidence, record a no-op decision artifact and complete without creating an empty episode.
...
Enforce this in the `create_task_from_forge_proposal` MCP handler (or a shared evolution scope-policy service called by it), before `episodeService.createTaskFromProposal`. The autonomy leg of the check must use the cycle's pinned `cycleAutonomyMode` (not a live re-read of Space autonomy), so materialization is consistent with the rest of the cycle; the scope opt-in leg still reads current `scope.policy.automation.createTasksFromAcceptedProposals`. Callers using any workflow receive the same policy. Return a structured authorization error when either condition is absent. Hiding the tool from the Learner profile when scope opt-in is false is useful UX but is not the security boundary.
