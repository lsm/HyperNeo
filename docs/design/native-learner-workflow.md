...
Update `episode.outcomeSummary` if needed, then accept or dismiss the episode. Activate only evidence-backed lessons whose every causal `evidenceEpisodeId` is **accepted**—the lesson-activation handler enforces this, so a lesson linked to a dismissed episode cannot be activated, and a level-1 bundle may not pair episode-dismiss with lesson-activate. Dismiss duplicates or unsupported candidates.
