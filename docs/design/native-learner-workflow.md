...
The task is the durable unit of work. Goal automation must serialize cycles at the goal/scope level, not separately by trigger kind: atomically acquire one active-cycle lease for `(goalId, evolutionScopeId)`, merge completed-task, self-nag, and external-event contexts into that cycle's durable pending-trigger set, and release the lease only when no task, review bundle, reserved continuation, or pending continuation outbox remains. Each evidence item can therefore belong to only one in-flight cycle even though trigger cursors remain kind-specific. The Learner selects the final episode evidence (bounded by `maxEvidencePerEpisode`) and runs the judge.
...
2. **Automation dispatch:** resolve the built-in workflow ID; serialize all trigger kinds through a goal/scope lease and merged trigger set; exclude learning tasks from completed-task thresholds; transactionally commit terminal state, cursor, reserved continuation identity, and outbox; treat review bundles/outbox/reservations as active; reconcile incomplete legacy terminal writes; keep manual runs outside automation cursors; and remove direct episode/review-task creation.
3. **Learner prompt + tool policy:** implement the procedure and logical memory namespace; provision filtered Space/node MCP servers with the explicit allowlist (including validation and lesson creation), enforce level 2 in durable learning-disposition/rollup handlers, add a whole-cycle level-1 review-bundle disposition gate, and add the level-3 plus scope-policy gate for proposal materialization.
4. **Validation model:** migrate nullable proposal fields and immutable full causal-contract acceptance snapshots; extend proposal service params and MCP mutation schemas; remove materialization overrides; add an audited legacy accepted-proposal repair transition; add pending/final/rejected proposal-validation operations with replacement proposal/task links; feed validations to episode judging.
5. **Tests:** migration of an existing Space before workflow restamping; cross-trigger goal/scope serialization and trigger-context merging; learning-task self-trigger exclusion at threshold 1; task-based no-op artifact identity; level-1 whole-bundle gate with and without validations; human apply/reject and cursor guard while pending; cursor gap preservation, transactional reserved continuation/outbox, live-notification race, crash reconciliation, and idempotent dispatch; manual cursor isolation; paused-state coalescing; tool allowlist denial plus validation/lesson creation availability; nullable legacy/draft proposal mutation, audited accepted-row repair, completeness guards, full action/expectation snapshots, post-acceptance immutability, and no materialization overrides; proposal materialization autonomy/scope-policy guards; accepted recurring rollup, accepted non-recurring no-rollup, and dismissed no-rollup paths; validation lesson/replacement links; no PR artifact/hook dependency.
