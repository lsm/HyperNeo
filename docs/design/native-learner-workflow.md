...
2. **Automation dispatch:** resolve the built-in workflow ID; serialize all trigger kinds through a goal/scope lease and merged trigger set; use one canonical goal/scope evidence cursor while retaining source cursors only for delivery dedup; exclude learning tasks from completed-task thresholds; transactionally commit terminal state, cursor, reserved continuation identity, and outbox; treat review bundles/outbox/reservations as active; reconcile incomplete legacy terminal writes; keep manual runs outside automation cursors; and remove direct episode/review-task creation.
3. **Learner prompt + tool policy:** implement the procedure and logical memory namespace; provision scope-bound filtered Space/node MCP servers with the explicit allowlist, enforce level 2 in durable learning-disposition/rollup handlers, have `submit_for_approval` atomically create the whole-cycle review bundle, and add the level-3 plus scope-policy gate for proposal materialization.
4. **Validation model:** add cycle-key-idempotent episode creation; migrate nullable proposal fields and immutable full causal-contract acceptance snapshots; extend proposal service params and MCP mutation schemas; remove materialization overrides; add an audited legacy accepted-proposal repair transition; add pending/final/rejected proposal-validation operations with replacement proposal/task links; feed validations to episode judging.
5. **Tests:** migration of an existing Space before workflow restamping; cross-trigger serialization, merged contexts, and canonical-cursor replay/skip prevention; learning-task self-trigger exclusion at threshold 1; cycle-key episode retry after lost response; task-based no-op artifact identity; level-1 atomic submit/bundle creation and retry with/without validations; human apply/reject and cursor guard; cursor gap preservation, continuation races/reconciliation/idempotency; manual cursor isolation; paused-state coalescing; cross-scope read/mutation denial for every allowed object family; validation/lesson creation availability; nullable legacy/draft proposal mutation, audited accepted-row repair, immutable full snapshots, and no materialization overrides; autonomy/scope-policy guards; accepted recurring/non-recurring and dismissed rollup paths; validation lesson/replacement links; no PR dependency.
