...
| Durable wait (A slipped/blocked indefinitely) | `pause_goal` on B with the wait recorded — and if B already has an active dependency-sensitive task, pause first, then cancel that task (`cancel_workflow_run: true` where applicable) and dispose the resulting outcome: pausing alone leaves the task and its run executing through publication. On release: `resume_goal` **and `trigger_goal_task`** (with retained-cadence precautions where applicable) — resuming alone creates no replacement task, since pattern goals run without cadence and with `autoTriggerNext` off | Prompt doctrine; visible in goal state/UI |
