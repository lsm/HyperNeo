...
- **Correct on drift.** When a gating goal's later outcome invalidates dependent work:
  ...
  **everything of B's that is running the stale contract** — the active goal task *and*
  ...
  fresh contract in B's `nextSteps`, dispose **all** pending B outcomes — a
  Forge-gated task that already finished before the drift has one that the
  cancellations did not produce, and mutating B before disposing it makes that review
  stale — then release. Release distinguishes hold reasons: a pause this sequence or the
