...
7. **Consolidation-exempt operational state store** for launcher hold records, saved
   ...
   create-and-record, Forge-path atomicity or reconciliation — §5) — both are
   **prerequisite enablers** of the template slice rather than revisit-triggered
...
operational-state store with its launcher-facing read/write surface, and the crash-safe
gated-task creation change (§5/§7 prerequisites — both land before or with the template
slice and block it otherwise), the launcher prompt template slice (template
registration, mechanical per §7), the
