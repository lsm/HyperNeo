...
- **Rebinding on retry:** dependencies bind to a specific task ID. When A's gated
  ...
  **cascade-cancelled** B (A was cancelled) must instead be recreated — disposing the
  cancelled Forge task's own outcome notification first (its `goalId` produces one; the
  standalone shape has none). Recreation of the
