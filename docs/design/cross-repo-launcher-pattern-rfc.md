...
| Durable wait (A slipped/blocked indefinitely) | `pause_goal` on B with the wait recorded — and if B already has an active dependency-sensitive task, pause first, then cancel that task (`cancel_workflow_run: true` where applicable) and dispose the resulting outcome: pausing alone leaves the task and its run executing through publication. On release: inspect and review any outcome the held task produced during the wait **before** `resume_goal` (resuming first bumps the revision and rejects the review), then conditionally re-trigger — only when setup cancelled the prior task or inspection confirms the active-task pointer is empty (a deliberately-kept non-dependency-sensitive task makes the trigger reject with `autoTriggerNext` off) — with retained-cadence precautions where applicable | Prompt doctrine; visible in goal state/UI |
