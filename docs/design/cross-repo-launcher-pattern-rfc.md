...
- **Release:** review any outcome produced while held (before resuming — the resume
  ...
  cleared; re-trigger only when the active-task pointer is confirmed empty **and, when
  setup retained a task, that task's terminal outcome has been reviewed** — it can
  terminalize after the pre-resume review, and triggering before applying its result
  both skips the state update and advances the revision so that review goes stale; a
...
| Deadlock (every goal waiting on another) | None — this is a judgment state | Doctrine: set an agent reminder (`create_agent_reminder`) **before** pausing — `pause_goal` also pauses the goal's check-in schedule, so check-ins cannot serve as the safety net while paused — then drain each affected goal's pending outcomes (reconciling completions racing the pause) and pause them; escalate to the human; never spin |
