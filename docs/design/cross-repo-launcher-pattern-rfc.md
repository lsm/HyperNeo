...
- **Correct on drift.** When a gating goal's later outcome invalidates dependent work:
  **validate the decision first** — re-read A (`get_goal`) and drain **all** of A's
  ...
  successor) — for a retained-cadence B, snapshot and clear the cadence before
  settlement (settlement empties the pointer against a live schedule otherwise,
  matching the gate-setup ordering) — settle B's active task (terminal state
