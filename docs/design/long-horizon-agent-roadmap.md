...
- Persist a minimal outcome-notification record atomically with the terminal transition at the central goal terminal seam, with one notification per durable terminal generation. The minimal core ships **no missing-notification reconciler**: a crash between the task write and the notification persist loses the wake, and MC6's inactivity nag is the dogfood backstop that re-surfaces the unreviewed outcome. (The deterministic-identity reconciler and pre-installation history watermark land with the robust report path.)
...
- List/read reports for the primary owner. Listings are cursor-paginated with a bounded projection (reports accumulate immutably with no deletion path, so an unbounded list would eventually exceed the model's tool-result context limit); the separate read operation returns one report's full payload.
