...
- Add `getPrimaryGoalOwner(goalId)`. A goal with no usable owner resolves to `no_recipient`; there is no fallback agent (the coordinator fallback was removed once spaces stopped pre-creating an agent).
- Gate the ownership mutation handlers now, not in G3: `assign_agent_to_goal`/`unassign_agent_from_goal` currently perform only same-Space checks and default assignments to `owner`, so an ordinary member could replace a legitimate owner with a controlled LH agent and inherit MC3 apply authority. Require coordinator or explicit human authorization in these MCP handlers from MC1 onward. This is the one authorization gate the minimal core carries, because the core itself introduces owner authority — broader member-isolation gates are deferred (see boundaries).
- Reconcile duplicate owners deterministically before enforcing the invariant.
- Resolve the recipient lazily at delivery time instead of building supersede/reroute machinery. An unprocessed MC2 notification carries no fixed recipient; the delivery path resolves the **current** primary owner at send time, with admission-time revalidation, and drops the wake when no usable owner exists. Reassignment, owner pause/disable/archive, and deletion therefore need no reroute logic: a pending wake always goes to whoever is currently authoritative, and MC3's in-transaction ownership check rejects the former owner.
...
- Persist a minimal outcome-notification record in the **same transaction** as the task terminal transition, terminal generation, and goal bookkeeping at the central goal terminal seam — one notification per durable terminal generation. The minimal core ships no missing-notification reconciler, so the single-transaction requirement is hard: a two-commit callback shape would lose the notification (and the identity `review_goal_outcome` needs) on a mid-transition crash with no recovery path. Audit every reportable terminal writer in MC2 and route it through the atomic transition — including `PostApprovalRouter.route`'s no-route branch, which currently completes with a direct `taskRepo.updateTask` and invokes `handleTaskTerminal` only afterward, so an approved completion would otherwise receive no notification through the whole minimal-core phase. (The deterministic-identity reconciler and pre-installation history watermark land with the robust report path.)
...
- Drop the wake visibly when no usable owner exists; there is no fallback agent.
...
- An ignored wake (delivery completed, but the owner never called `review_goal_outcome`) leaves the notification pending; the minimal core relies on **MC6's inactivity nag as the backstop** that re-prompts the agent to review pending goals and follow-ups. The bounded re-notification/backoff/escalation reconciler is deliberately deferred to G7.
...
**Sequencing:** gate wake injection until MC3 and the receiving agents' MC5 prompt migration are enabled — or ship those contracts atomically with MC2. Injecting wakes the owner cannot yet dispose would leave notifications pending with no way to terminalize them.
...
- the caller-observed goal revision when resubmitting after a stale rejection;
- optional structured observations and metric changes;
- summary, next steps, and follow-up intent.

The tool validates — inside the same transaction that claims the notification — that the caller is the current primary owner or an admitted human, and that the notification is still unsuperseded. It must also bind the three identities: when one agent owns multiple goals, a stale or malformed call can pair goal A's notification with goal B's goal/task IDs, applying one outcome to the wrong goal while permanently marking the real notification processed. Derive the goal and task from the claimed notification — or atomically compare all three identities — before authorization and apply. A pre-transaction ownership check alone races with concurrent reassignment: the former owner could pass validation, lose ownership, and still claim the notification and mutate the goal. It then atomically records the notification as processed and applies the update through `SpaceGoalService`. It must compare the notification's base goal revision inside the same transaction; if the goal changed independently after the notification, reject the stale proposal and return the **current revision and goal state** so the owner can resubmit against fresh state — never overwrite newer summary, metrics, or next steps. A resubmission carries the caller-observed revision and is applied only when it still matches the current goal revision at claim time (an atomic CAS); otherwise it is rejected again with fresh state. Acknowledge/reject/supersede dispositions terminalize the notification without mutating the goal, so a stale or unwanted outcome never loops forever. (The explicit edited-merge operation is deferred to G8, where immutable reports make it necessary.) The goal revision must be a monotonically incremented counter updated by every goal mutation — not `updatedAt`, which every mutation assigns via `Date.now()` and which can therefore collide for two mutations in the same millisecond, letting a stale proposal pass the comparison. The minimal tool applies goal-state updates only (summary, metrics, next steps). The owner creates any follow-up **tasks** itself through ordinary task tools — next steps persist on the goal, and a turn that dies before creating them is re-prompted by the next wake or the MC6 nag, so no follow-up outbox is needed in the minimal core. (The durable follow-up command/outbox lands with G8's decision transaction.) Retrying a durable wake or retrying after a tool timeout must return the original processed result rather than appending duplicate goal events or reapplying metrics. Workers do not get this tool.

Discovery: calling the tool **without** a notification identity returns the pending notification identities for goals the caller **currently owns** (authorized, bounded). Because MC2's recovery path is the generic MC6 nag, a nagged, restarted, or newly reassigned owner otherwise has no way to learn the persisted identity `review_goal_outcome` requires. A goal with no usable owner has no discoverable notification at all: it is neither woken nor claimable until an owner is assigned.
...
Use existing goal history, task links, and goal editing surfaces — goal events already render in the current UI. Latest-outcome and next-check-in panels are deferred polish.
...
- Cover already-persisted agents by **dynamic injection**, not migration: `create_agent_from_template` copies `template.instructions` into the agent row and prompt construction reads that stored value, so existing template-created agents (including the coordinator and the first dogfood agents) never see updated stock instructions. Inject the core owner-review contract at prompt-build time from a runtime constant so every LH agent always receives the current contract; user-customized prompts are never touched, and a versioned migration of stored prompts is deferred.
- Teach goal workers to report bounded results without claiming ownership of overall strategy.
- Bound the outcome-inspection path those prompts rely on: `list_goal_tasks` currently loads every Space task, filters in memory, and returns full records; for a long-lived goal this grows without bound. Add cursor pagination and a compact list projection, with separate retrieval for one task's details.
...
- Atomically claim a deterministic inactivity window before injecting. While a nag delivery attempt for that window is accepted/in flight, subsequent and concurrent scans must not enqueue another nag — `lastActivityAt` stays stale until consumption, so an unclaimed scan would duplicate autonomous turns on every pass. Advance the attempt generation only after a terminal delivery failure. Re-check immediately before V2 admission with two cheap state checks — no separate watermark ledgers: skip the nag when the session is not idle (a turn is processing/queued/in cooldown), when any accepted-but-unconsumed delivery is already pending for the session, or when `lastActivityAt` has advanced since the claim (fresh human message, external event, or reminder). Bind the claim to the current agent/Space availability and watchdog configuration state and revalidate both at admission, so a pause/edit between claim and admission invalidates the nag instead of firing an obsolete wake.
...
- Make the threshold and prompt configurable per agent, with the injected prompt bounded like every other projection: an operator-configured prompt larger than the provider context window would otherwise inject verbatim and end consumed-but-turn-failed on every occurrence, leaving the agent degraded instead of self-nagged. Validate or truncate the text passed to V2 while retaining the full configuration for editing.
- Support enable/pause/resume and run-now controls.
- Do not fire while the agent or Space is paused, stopped, disabled, or archived — enforced at scan time and again at admission per the claim rule above.
...
- member-isolation authorization beyond the ownership mutation itself: gating goal mutations (`update_goal`, pause/resume, `trigger_goal_task`), agent lifecycle (`update_agent`/`pause_agent`/`archive_agent`), Forge rollups (`apply_forge_rollup`), and goal-linked task create/edit/lifecycle (`retry_task`, `cancel_task`, `archive_task`, `approve_task`, `update_task`, `create_task_from_forge_proposal`) all lands in G9. These are pre-existing member capabilities today — deferring their gating regresses nothing — and the minimal core assumes a **trusted Space** (a single operator with operator-controlled agents). Do not deploy the minimal core in a Space with untrusted member sessions; G9 hardens these handlers before that. Also deferred to G9: the report-decision override matrix, worker-mutation deprecation, and the atomic/idempotent Forge rollup transaction.
- the bounded re-notification/backoff/escalation reconciler for ignored wakes — MC6's inactivity nag is the dogfood backstop, and the reconciler lands with G7;
- the missing-notification reconciler and pre-installation history watermark — MC2 persists atomically at the central terminal seam, and the MC6 nag backstops a wake lost to a mid-transition crash;
- the follow-up work outbox — MC3 applies goal-state updates only, and the owner creates follow-up tasks itself through ordinary tools (durable follow-up commands land with G8);
- the explicit edited-merge review operation — MC3 rejects stale proposals with current revision/state for resubmission; edited-merge lands in G8;
- a versioned migration of persisted agent prompts — MC5 injects the core contract dynamically at prompt-build time instead;
- latest-outcome and next-check-in UI panels — goal events already render in the existing UI.
...
- Bound the injected prompt: `formatExternalEventEssence` copies `payload.body` into the prompt without a size limit, so a large event body can exceed the provider context limit and fail delivery repeatedly without the agent ever seeing the event. Persist the complete event for `get_external_event`, but inject only a bounded summary plus the stable event ID — and bound the detail read too: `get_external_event` currently returns the complete event including `rawPayload` and `body` in one result, so the payload too large to inject would also overflow on fetch. Add bounded field projections or chunked/cursor retrieval for large event fields while preserving the full stored record.
...
- Persist the V2 job/message correlation and convert terminal failed/rejected delivery **before SDK consumption** into another inbox attempt before the row leaves pending scans. Each retry needs a fresh V2 message identity: `flushLongTermAgentInbox` currently uses the inbox row ID as the message ID, so retrying with the same UUID would resolve the existing failed SDK row instead of admitting a new prompt — persist an attempt generation and include it in each retry UUID while retaining the inbox row ID as the logical dedupe key. A post-consumption turn failure must not create another attempt — the row is already delivered and re-injection would replay a model-visible prompt, repeating autonomous tool side effects; it follows the orthogonal-state contract (visible degraded state or domain-idempotent recovery) instead.
- Bound the injected message: `SpaceAgentInboxRepository.enqueue` accepts an unbounded string and `flushLongTermAgentInbox` injects `row.message` verbatim, so an oversized message can consume and then fail the turn on provider context limits — and the post-consumption policy forbids blind retry, leaving the message permanently unprocessed. Validate size at ingress or inject a bounded projection while preserving the full inbox record; a truncated projection must include the stable inbox-row ID and an authorized read tool to fetch the full record (otherwise the omitted portion is permanently unavailable despite successful delivery).
...
- List/read reports for the primary owner. Listings are cursor-paginated with a bounded projection (reports accumulate immutably with no deletion path, so an unbounded list would eventually exceed the model's tool-result context limit); the single-report read returns bounded field projections with chunked/cursor retrieval for large fields (summary, observations, recommendations, proposal, evidence) rather than unconditionally returning everything — the bounded wake directs the owner here for omitted details, so this path must itself stay within context limits.
...
- Pick up the member-isolation gates the minimal core defers under its trusted-Space assumption: agent lifecycle handlers (`update_agent`, `pause_agent`, `archive_agent` — with target-agent self-service limited to explicitly safe lifecycle fields, never `custom_prompt`, tool allowlist, provider/model, or `setting_sources`), Forge rollups (`apply_forge_rollup`), and every goal-linked task create/edit/lifecycle handler (`retry_task`, `cancel_task`, `archive_task`, `approve_task`, `update_task`, `create_task_from_forge_proposal`), retaining narrowly scoped authority for a worker completing its own task.
...
- Full reminder list for the selected LH agent, cursor-paginated with a limit and a bounded list projection: preserved fired/cancelled rows accumulate without bound, and the current `list_agent_reminders` path returns all matches, so an unbounded list would eventually exceed the model's tool-result context. Individual reminder detail stays available per row.
