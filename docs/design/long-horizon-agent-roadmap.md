...
- Bind each delivery to a subscription configuration generation and recheck immediately before V2 admission: a subscription edited, paused, or deleted after V2 acceptance is not revoked by source-side bookkeeping (`clearLongHorizonRetries` cannot revoke a transferred V2 job), so topic, filter, status, target-agent availability, and Space availability must be revalidated at admission — or explicitly define an earlier cutoff after which subscription changes can no longer prevent delivery.
