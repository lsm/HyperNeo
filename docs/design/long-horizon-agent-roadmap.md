...
- Capture a base goal revision — the monotonic per-goal counter defined in MC2 — with each report and validate it inside the decision transaction. For reports carrying a worker proposal, this is the revision observed at composition time (recorded with the G6 pending outcome), not merely the post-terminal-bookkeeping revision. Terminal goal bookkeeping (active/last task pointer updates) and report creation must share one transaction, with the base revision sampled after that bookkeeping, or the revision semantics must explicitly exclude those automatic changes. If an independent goal change has occurred since the report, reject the stale proposal and require explicit merge or edited apply; a full replacement apply must never overwrite newer goal state.
