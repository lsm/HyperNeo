...
| `packages/daemon/src/lib/rpc-handlers/dialog-handlers.ts` | 22, 30, 37, 57, 70, 145 | `osascript`/`zenity`/`kdialog`/`powershell`/`which` | `Bun.spawn([cmd, ...args], { stdout: 'pipe', stderr: 'pipe' })` with no `env` | **sanitize** | Folder-picker subcommands need only a safe desktop baseline (PATH, HOME, USER, XDG dirs, locale). |
...
`packages/daemon/src/lib/acp/acp-model-fetcher.ts:27` (`disposeAcpSessions`) constructs `new AcpClient({ command, args, cwd, ... })` **without** `env` or `replaceEnv`.  This causes the ACP transport to fall back to cloning the live `process.env`.  This is a one-line fix under 0g: add `env: buildAcpSafeEnv()` and `replaceEnv: true`.
