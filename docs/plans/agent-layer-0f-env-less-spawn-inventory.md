...
| `packages/daemon/src/lib/space/managers/space-worktree-manager.ts` | 62, 72, 79, 86, 101, 160, 172, 235, 250 | `git worktree` / `git branch` | `execFileSync('git', ...)` with no `env` (10 sites) | **sanitize** | A platform-aware `buildGitCommandEnv` allowlist: PATH, HOME, USER, XDG_CONFIG_HOME, `GIT_*`, plus on Windows `USERPROFILE`, `HOMEDRIVE`/`HOMEPATH`, `APPDATA`, `LOCALAPPDATA`, `SystemRoot`, `TEMP`/`TMP`, `PATHEXT`, `COMSPEC`. |
| `packages/daemon/src/lib/space/artifact-git-ops.ts` | 35 | `git` diff / log / remote | `execFile('git', args, { cwd, encoding, timeout, maxBuffer }, ...)` with no `env` | **sanitize** | Same platform-aware `buildGitCommandEnv`. |
| `packages/daemon/src/lib/space/managers/space-manager.ts` | 275 | `git rev-parse --git-dir` | `execAsync('git ...', { cwd })` with no `env` | **sanitize** | Same platform-aware `buildGitCommandEnv`. |
...
| `packages/daemon/src/lib/providers/anthropic-copilot/provider.ts` | 869–878 | Copilot client start | `new CopilotClient({ useStdio: true, env: buildCopilotEnv({ ...this.env }) })` where `this.env` defaults to live `process.env` | **sanitize** | 0g builds the Copilot client env from a credential-free OS baseline, plus `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` from the provider's immutable baseline, plus a purpose-specific proxy/TLS allowlist (`HTTPS_PROXY`, `NO_PROXY`, `NODE_EXTRA_CA_CERTS`, etc.) sourced from that same baseline; it must not spread live `process.env`.
| `packages/daemon/src/lib/acp/acp-model-fetcher.ts` | 27, 65 | ACP session disposal and model discovery | `disposeAcpSessions` passes no `env`; `fetchAcpModels` uses `buildAcpDiscoveryEnv()` (alias of `buildAcpSafeEnv`) with `replaceEnv: true`, which strips credentials | **sanitize** | `disposeAcpSessions` and model discovery need a credential-bearing ACP env associated with the provider/session (e.g., the same `acpEnv` used by the query runner), not the safe-only baseline; `acp-provider.ts:28` `defaultAcpCommandProbe` has the same gap. |
...
| `packages/daemon/src/lib/session/session-lifecycle.ts` | 957 | Title generation query | `env: mergedEnv` from `buildSdkQueryEnv` | **verify** | SDK `query` called with explicit `env` inside ProviderService enrollment. |
| `packages/daemon/src/lib/agent/limit-error-llm-classifier.ts` | 267 | Limit-error classification query | `env: mergeProviderEnvVars(providerEnvVars)` | **verify** | SDK `query` called with explicit `env` inside ProviderService enrollment. |
| `packages/daemon/src/lib/space/evolution-episode-service.ts` | 761 | Episode-judge query | `env: mergeProviderEnvVars(providerEnvVars)` | **verify** | SDK `query` called with explicit `env` inside ProviderService enrollment. |
| `packages/daemon/src/lib/space/evolution-conversation-analysis-service.ts` | 296 | Conversation-friction query | `env: mergeProviderEnvVars(providerEnvVars)` | **verify** | SDK `query` called with explicit `env` inside ProviderService enrollment. |
| `packages/daemon/src/lib/space/runtime/llm-workflow-selector.ts` | 61 | Workflow-selection query | `env: mergedEnv` from `mergeProviderEnvVars` | **verify** | SDK `query` called with explicit `env` inside ProviderService enrollment. |
...
5. **0g mechanical application** will: (a) add `env` to every call in §1 using a platform-aware `build*Env` helper; (b) update `runtime-spawn` to require a propagated lease token before using the active coordinator baseline, and otherwise fall back to a credential-free OS/runtime allowlist; (c) fix `acp-model-fetcher.ts:27` and `acp-model-fetcher.ts:65` (and `acp-provider.ts:28`) with a credential-bearing ACP env, not just `buildAcpSafeEnv`; (d) add verification rows for the already-explicit paths in §4; (e) remove the `process.env` set/restore blocks in the GitHub agents and `anthropic-provider.ts:169-185` by using the SDK `env` option; (f) build `anthropic-copilot/provider.ts:869-878` Copilot client env from a credential-free baseline, the needed tokens, and a proxy/TLS allowlist sourced from the provider's immutable baseline; (g) update `hook-executor.ts:153-191` (`buildHookRestrictedEnv`) to use the hook owner's immutable env or the active coordinator baseline only when the matching lease token is present; (h) add `condition.allowedEnv` to the workflow schema and runner, with a non-bypassable credential/routing-name filter and values sourced from an immutable baseline, never live `process.env`.
