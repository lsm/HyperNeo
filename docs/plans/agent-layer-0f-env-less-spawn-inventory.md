...
| `packages/daemon/src/lib/space/runtime/workflow-executor.ts` | 33–37, 127–137 | `sh -c <user condition expression>` | `spawnProcess(args, { cwd, ... })` with no `env` | **sanitize** | Condition expressions are arbitrary shell and must not see session API keys. 0g passes a fixed, credential-free workflow baseline (PATH, HOME, TMPDIR, etc.).  If a condition needs non-standard variables, the workflow schema must declare them (new `condition.allowedEnv` field) so 0g can inject only those keys. |
