...
| B5d | **all** post-await watchdog-write fencing at the watchdog boundary — the post-await `retryCount` write inside `scheduleRetry` (:132 fn; write at :233–237, before its generation check), `triedKeys`/`chain` :149–176, and the detached `fireImmediateFallback` finally (:456 fn; unfenced finally ~:476–480) — `scheduleCooldown` itself (:323, called :244/:295) is already generation-checked and is the control, not the gap — lands on #2779's pure gates; owner coordination, excluded-module rule holds | ~30 | ~55 |
