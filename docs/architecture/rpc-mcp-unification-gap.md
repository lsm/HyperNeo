...
- `requireMetadataCallerScope` (`space/operations/task-metadata.ts`) returns allow
  ...
  `admitSpaceTaskCaller` reaches it from 5 call sites across 4 source files.
...
already implements this pipeline for `call_action`: its eight-stage composition carries five
policy gates — `applySafetyClass` (5 lines), `resolveTargets` (46), `applyRoleAdmission` (18),
`applyAutonomyGate` (61) and `applyRateAndAudit` (75) — plus two audit helpers,
`denialAuditCtx` (18) and `writeAuditEntry` (22). 245 lines in total. The remaining stages
(`resolveAction`, `executeAction`, `formatResult`) are resolution, effect and presentation,
not policy.
...
3. **The pipeline must exist before the bulk porting.** Every action ported to an operation
   moves from five gates to zero until pre-invocation is in place.
