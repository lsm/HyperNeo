...
3. Verify the review state covers the current head with NO effective changes-requested and an AUTHORIZED approver, then merge bound to it:
   ...
   Bind the review gates AND the approvals to the base: compare the current `baseRefName` (`gh pr view {{pr_url}} --json baseRefName`) with the base in effect when the review gate ran — the `base_ref` recorded in your `external-review-gate` artifact, or for internal modes the `base_ref` recorded in your `review-base` note artifact (key "base"). Retargeting a PR changes the reviewed diff WITHOUT changing `headRefOid`, so a base change invalidates the gate and every approval given against the old base — re-run the applicable gates and obtain fresh approval before merging.
...
4. On any blocker (merge failed, CI not passing, unresolved threads, missing current-head approval, DIRTY conflict, BEHIND rebase, BLOCKED ruleset, permissions, merge queue):
   ...
   b. Then EITHER fix it yourself OR report it:
      - FIXABLE blocker (DIRTY conflict, BEHIND rebase, UNSTABLE because the code fails a required check, or an unresolved review thread you can resolve by replying on the PR): fix it in this PR and push the new head. A push changes the head, so the prior approval no longer covers it — do NOT merge the new head until it is re-approved. Reply on any review thread you resolve, then ask {{approval_authority}} to re-approve the CURRENT head and continue:
        ...
        Then STOP and wait. When the review policy routed this run's gate to external bots (`external`/`both`, or `auto` with bots), the fix push also made that gate stale: before merging, re-trigger the gate-set bots and wait for fresh current-head passes exactly as in the coding-phase gate — and when a re-triggered bot engages but stalls past its window or errors out (the same failure the Reviewer's backup covered at the initial gate), do NOT wait on it forever: apply the coding-phase dead-bot rules, then route the revalidation back through Review exactly as the initial gate did — send Review the same post-approval-fix handoff as the internal path below, naming the dead bot in the summary, and the Reviewer's backup re-review stands in for that bot's pass (an explicitly-`external` repository that never had any review bot is the one exception: that missing gate stays a reported blocker). When the policy requires the internal review (`internal`/`both`, or `auto` without bots), the changed PR must go through Review BEFORE any re-approval: send the gated PR handoff to Review — send_message(target="Review", message="Post-approval fix on {{pr_url}}: <one-line summary>. Please re-review the CURRENT head and post your current-head review, then reply to me.", data: { pr_url: "{{pr_url}}", headRefOid: "<new headRefOid>", reason: "merge_fix_pushed" }) — and wait for the Reviewer's current-head review; a test-only re-approval from a QA authority never substitutes for the review source's gate on a changed head. When {{approval_authority}} is itself Review (plain Coding), that single handoff is the re-approval request; when it is QA (Coding with QA), do BOTH in order — Review re-reviews the changed head first, then {{approval_authority}} re-approves — and only after {{approval_authority}} replies to continue, go to step 3 again (re-verify against the CURRENT head).
