...
3. Verify the review state covers the current head with NO effective changes-requested and an AUTHORIZED approver, then merge bound to it:
   ...
   MERGED -> continue to step 5 (cleanup + sync + audit). Still open but queued/processing -> keep re-querying roughly once a minute, up to ~10 attempts. If the queue entry is removed or its merge-group check fails (PR open, never MERGED), treat it as a blocker per step 4. Do NOT treat a null autoMergeRequest as failure (a PR added directly to the queue with checks passing legitimately has it null). While queued, the base can still advance under the PR — another queued entry lands first and the queue rebases onto it — and `--match-head-commit` binds only the PR head, never the base. Poll the `baseRefOid` alongside `state` on every re-query: when it differs from the base OID recorded in your gate artifact, the merged result will contain a diff no gate reviewed — do NOT treat that merge as verified: escalate per step 4 with the base mismatch, and if the entry merged anyway, record the mismatch in the audit rather than a clean success. Accept `MERGED` as verified ONLY when the final `baseRefOid` still equals the one recorded in your gate artifact.
