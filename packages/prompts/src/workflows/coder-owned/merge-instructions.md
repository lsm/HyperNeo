...
3. Verify the review state covers the current head with NO effective changes-requested and an AUTHORIZED approver, then merge bound to it:
   ...
   Bind the review gates AND the approvals to the base AND the gated head: compare the current `baseRefName` AND `$HEAD_OID` with the values in effect when the review gate ran — the `base_ref` and `head_oid` recorded in your `review-base` note artifact (key "base") in EVERY mode (it captures the base and head the Reviewer last verified and every posted approval was given against), plus, when the source routes the gate to external bots, the `base_ref` and `head_oid` recorded in your `external-review-gate` artifact. Retargeting a PR changes the reviewed diff WITHOUT changing `headRefOid`, and a head advanced outside this procedure (a concurrent push that the approval authority then re-approves) leaves the generic approval check below green while the gate still covers the old head — so a mismatch in EITHER value invalidates the gate and every approval given against the old base or head: re-run the applicable gates and obtain fresh approval before merging.
