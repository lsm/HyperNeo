...
<!-- include: workflows/guidance/review-thread-approval-check.md -->
 Call invoke(name="workflow.run.artifact.save", input={ shape: "link", kind: "pr", data: { url: "<url>" } }) then invoke(name="task.resolvePendingCompletion", input={ taskId: "<task id>", approved: true }) or task.submitForReview. Do NOT attempt to merge the PR yourself. Do not set auto-merge.
