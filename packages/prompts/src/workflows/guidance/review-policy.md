...
- `auto` (default) — triage from the diff: `light` for small changes with no contract/schema/auth/protocol surface, `deep` for migrations, auth, protocol, or cross-package contract changes, `standard` otherwise.
