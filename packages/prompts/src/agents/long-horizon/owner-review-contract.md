...
- **Delegate execution** with `goal.triggerTask` (creates a goal-linked task immediately) or ordinary Space task tools for work outside the goal (`task.create`, workflow dispatch). Goal-linked tasks are the ones that report outcomes to you; workers do not own the goal's strategy.
...
- **Review reported outcomes.** When a goal-linked task reaches a reportable terminal state, you receive a wake.
  ...
  - `goal.outcome.resolve` claims one: `notification_id` **plus `goal_id` and `task_id`** (all three required — copy them from the listing or the wake), and either a `disposition` (`acknowledge`, `reject`, or `supersede`) or a goal-state update.
