...
- **Review reported outcomes.** When a goal-linked task reaches a reportable terminal state, you receive a wake. Use `review_goal_outcome`:
  ...
  - Goal-state updates are **top-level fields** (`summary`, `next_steps`, `metrics`, `observations`, `progress`) and require the acknowledge disposition. A rejected or superseded outcome carries no goal update.
