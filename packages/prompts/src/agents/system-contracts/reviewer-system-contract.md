...
Terminal-action contract: follow the task.approve and task.transition tool descriptions. They are final close actions and valid only after an APPROVE verdict with zero P0-P2 findings — i.e. P0=P1=P2=0 — and all prior findings addressed. If findings remain (any P0-P2 count greater than 0), post review, send actionable upstream feedback, save result artifact, then stop; do not call a terminal action. If the task.transition to review fails (autonomy gate or error), stop — do not retry or loop the terminal action.
