...
describe('VoiceWaveform', () => {
  it('renders the elapsed timer at 0:00 while recording', () => {
    render(<VoiceWaveform getLevel={() => 0} isRecording={true} isTranscribing={false} />);
  ...
  it('shows the transcribing state instead of the timer while transcribing', () => {
    render(<VoiceWaveform getLevel={() => 0} isRecording={false} isTranscribing={true} />);
  ...
  it('drives the bar meters from getLevel via requestAnimationFrame', () => {
    ...
    vi.unstubAllGlobals();
    vi.useFakeTimers();
    render(<VoiceWaveform getLevel={() => 0.9} isRecording={true} isTranscribing={false} />);
    // ~30 frames: a new column is pushed every 3rd frame.
    vi.advanceTimersByTime(500);

    // The waveform columns are the children of the panel's first (bars) row.
    const row = screen.getByTestId('voice-recording-panel').firstElementChild!;
