...
const transitionTaskMock: Mock = vi.fn();
...
describe('PendingPostApprovalBanner', () => {
  ...
  it('Send back transitions to in_progress and lets the daemon clear post-approval fields', async () => {
    const task = makeTask({
      status: 'approved',
  ...
  it('Mark done transitions to done and lets the daemon clear post-approval fields', async () => {
    const task = makeTask({
      status: 'approved',
    ...
    expect(transitionTaskMock).toHaveBeenCalledWith('task-1', 'done');
  });
  ...
  it('surfaces error when transitionTask rejects', async () => {
    transitionTaskMock.mockRejectedValueOnce(new Error('rpc exploded'));
    const task = makeTask({
      status: 'approved',
      postApprovalBlockedReason: 'stuck',
    });
