/**
 * Agents page for a Space — lists task agents (Coordinator + specialists).
 * Long-horizon agent management lives on the dedicated Agents page.
 */
...
function AgentCard({ agent, drifted, syncing, onEdit, onDelete, onSync }: AgentCardProps) {
  ...
  return (
    <div class="group rounded-lg border border-white/10 bg-white/[0.025] px-3 py-3">
      <div class="flex items-start justify-between gap-4">
        <div class="min-w-0 flex-1">
          ...
          {agent.description && (
            <p class="mt-1 line-clamp-2 text-xs leading-5 text-gray-400">{agent.description}</p>
          )}
          <div class="mt-1.5 flex flex-wrap items-center gap-1.5 text-xs text-gray-400">
            {agent.model && <span class="font-mono text-gray-400">{agent.model}</span>}
            {agent.model && toolCount > 0 && <span>·</span>}
            {toolCount > 0 && <span>{toolCount} tools</span>}
            {agent.tools?.slice(0, 3).map((tool) => (
              <span key={tool} class="rounded border border-white/10 px-1.5 py-0.5 text-gray-400">
                {tool}
              </span>
            ))}
          </div>
...
export function SpaceAgentList() {
  ...
  const visibleAgents = agents.filter((agent) => !isCoordinatorAgent(agent));
