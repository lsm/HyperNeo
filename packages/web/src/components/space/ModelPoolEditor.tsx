...
import { newPoolEntry } from './agent-model-pool';
import {
  WorkflowModelSelect,
...
export const POOL_EMPTY_HINT_SPACE_DEFAULT =
  'No models — this agent uses the space default until one is added.';
...
export function ModelPoolEditor({
  modelPool,
  emptyHint = POOL_EMPTY_HINT_SPACE_DEFAULT,
  onModelPoolChange,
  ...
  return (
    <div data-testid="agent-model-pool">
      ...
      {modelPool.length === 0 ? (
        <p class="text-xs text-fg-faint leading-snug">{emptyHint}</p>
      ) : (
        <div class="space-y-2">
            ...
            >
              ...
              <label class="flex items-center gap-1 text-xs text-fg-muted flex-shrink-0">
                ...
                <input
                  ...
                  onInput={(e) => editNumeric(index, 'max', (e.target as HTMLInputElement).value)}
          ...
          <p class="text-xs text-fg-faint leading-snug">
            Max is the per-model concurrency cap across the space (1 or more) — any agent&rsquo;s
            runs of the same model count toward it. Weight is 0 or more and controls this
            model&rsquo;s share of the remaining capacity (higher = preferred); the pool needs at
            least one positive-weight entry. Each spawn picks by remaining capacity × weight; when
            every model is at its cap the spawn waits. A model pinned on a workflow slot or task
            override bypasses the pool.
          </p>
        </div>
      )}
    </div>
