        ...
        <DialogPanel class="mx-auto w-full max-w-2xl transform overflow-hidden rounded-[28px] border border-line bg-[#252527]/80 shadow-[0_28px_90px_rgba(0,0,0,0.55)] ring-1 ring-line/5 backdrop-blur-2xl transition-all data-[closed]:scale-[0.98] data-[closed]:opacity-0 data-[enter]:duration-200 data-[enter]:ease-out data-[leave]:duration-150 data-[leave]:ease-in">
          <div class="border-b border-line px-4 pt-3">
            <div class="mb-2 flex items-center justify-between">
              ...
              <div class="flex rounded-full bg-black/25 p-0.5 text-xs text-fg-muted">
            ...
            <div class="grid grid-cols-1">
              <input
                ...
                class="col-start-1 row-start-1 h-12 w-full bg-transparent pl-10 pr-4 text-[17px] text-fg outline-none placeholder:text-fg-faint"
                data-testid="command-palette-input"
              />
              <span
                class="pointer-events-none col-start-1 row-start-1 ml-1 self-center text-fg-faint"
                aria-hidden="true"
              >
          ...
          <div class="flex items-center justify-between border-t border-line px-4 py-3 text-xs text-fg-faint">
  ...
  return (
    <button
      ...
      class={cn(
        ...
        activeMode === mode ? 'bg-fill text-fg shadow-sm' : 'hover:bg-fill'
      )}
    >
      {children}
    </button>
    ...
    <ul>
      {groups.map((group, groupIndex) => (
        <li key={`${group.key}:${groupIndex}`} class="py-1">
          <div class="px-3 pb-1 pt-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-fg-faint">
      ...
      >
        <span
          class={cn(
            ...
            active ? 'border-line-strong bg-fill text-fg' : 'border-line bg-fill-soft text-fg-muted'
          )}
        >
          {iconFor(item.type)}
        </span>
        <span class="min-w-0 flex-1">
          ...
          <span class="block truncate text-xs text-fg-faint group-hover:text-fg-muted">
            {item.type === 'message' ? item.snippet : item.subtitle}
          </span>
        </span>
        {item.type === 'command' && item.shortcut && (
          <kbd class="ml-3 flex-none rounded-md border border-line bg-scrim-soft px-1.5 py-0.5 font-mono text-[11px] text-fg-faint">
            {item.shortcut}
          </kbd>
        )}
      </button>
