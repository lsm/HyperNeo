 ...
 *
 * Landing announcements are the daemon's concern now: the flush asserts write
 * NO marker (the daemon emits session.voiceLanded on the session channel), and
 * the retired design's localStorage namespaces are swept at startup.
 */
...
describe('voice transcript outbox', () => {
  ...
  it('writes no landed marker — the daemon announces landings server-side', async () => {
    // The retired design recorded a localStorage landing marker per session;
    // the daemon's session.voiceLanded channel event replaced it. The flush
    // must leave nothing behind in localStorage beyond the entry keys.
  ...
  it('drops the local mirror when another tab removes a shared entry key', () => {
    ...
    // replays an already-delivered id.
  ...
  it('sweeps the retired design’s localStorage namespaces at startup', () => {
    ...
    localStorage.setItem(
      'hyperneo_voice_transcript_outbox_v1.draft.s1.sometab',
      JSON.stringify({ content: 'edits', ts: Date.now(), generation: 1 })
    );
    localStorage.setItem(
      'hyperneo_voice_transcript_outbox_v1.clear.s1.sometab',
      JSON.stringify({ ts: Date.now() })
    );
    localStorage.setItem(
      'hyperneo_voice_transcript_outbox_v1.superseded.s1.1.1',
      JSON.stringify({ generation: 1, beforeTs: Date.now() })
    );
    localStorage.setItem('hyperneo_tab_heartbeat.sometab', String(Date.now()));
    ...
    expect(
      localStorage.getItem('hyperneo_voice_transcript_outbox_v1.superseded.s1.1.1')
    ).toBeNull();
    expect(localStorage.getItem('hyperneo_tab_heartbeat.sometab')).toBeNull();
    expect(getPendingTranscripts().map((e) => e.text)).toEqual(['still deliverable']);
    stopVoiceTranscriptOutboxFlush();
  });
  ...
  it('clearPendingTranscripts drops everything', () => {
    enqueueTranscript('s1', 'a');
    enqueueTranscript('s2', 'b');
    clearPendingTranscripts();
    expect(getPendingTranscripts()).toHaveLength(0);
  });
});
