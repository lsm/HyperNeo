...
export function createSpaceTables(db: BunDatabase): void {
  ...
  db.exec(`
		CREATE TABLE IF NOT EXISTS space_tasks (
			...
			status TEXT NOT NULL DEFAULT 'open'
				CHECK(status IN ('rate_limited', 'usage_limited', 'draft', 'open', 'in_progress', 'review', 'done', 'blocked', 'cancelled', 'archived', 'approved', 'stopped')),
			...
			pending_completion_generation INTEGER NOT NULL DEFAULT 0,
  ...
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_space_tasks_owner_created ON space_tasks(space_id, created_at DESC, id DESC) WHERE status != 'archived'`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_space_tasks_owner_status_created ON space_tasks(space_id, status, created_at DESC, id DESC)`
  );
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_space_tasks_owner_updated ON space_tasks(space_id, updated_at DESC, id DESC) WHERE status != 'archived'`
  );
  db.exec(
