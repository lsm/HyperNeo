...
describe('exportWorkflow', () => {
  ...
  test('exports the agentRef fallback alongside templateKey for migrated slots', () => {
    ...
    expect(exported.nodes[0].agents[0].templateKey).toBe('migrated.agent.agent-uuid-1');
    expect(exported.nodes[0].agents[0].agentRef).toBe('My Coder');
  });

  test('omits the agentRef fallback for migrated slots whose agent is gone', () => {
    const workflow = makeWorkflow({
      nodes: [
        {
          id: 'n1',
...
describe('validateExportedAgent', () => {
  ...
  test('rejects modelPool in version 3 agents', () => {
    const exported = exportAgent({
      ...makeAgent(),
      modelPool: [{ model: 'sonnet', maxConcurrent: 2, weight: 50 }],
    });
    const data = { ...exported, version: 3 } as unknown;
    const result = validateExportedAgent(data);
    expect(result.ok).toBe(false);
    if (!result.ok) {
      expect(result.error).toContain('modelPool requires export version 4');
    }
  });
  ...
  test('accepts a modelPool provider at any supported version (import strips pre-v7 pins)', () => {
    const data = {
      version: 6,
      type: 'agent',
      name: 'Bot',
  ...
  test('accepts modelPool in version 4 agents', () => {
    ...
    expect(result.ok).toBe(true);
    if (result.ok) {
      expect(result.value.modelPool).toHaveLength(1);
    }
  });
...
describe('validateExportedWorkflow', () => {
  ...
  test('rejects templateKey in workflow version < 5', () => {
    const data = {
      version: 4,
      type: 'workflow',
      name: 'Bad',
