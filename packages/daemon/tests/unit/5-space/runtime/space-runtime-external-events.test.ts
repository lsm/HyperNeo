...
describe('SpaceRuntime external event subscriptions', () => {
  ...
  test('clears derived subscriptions when the primary link disappears', async () => {
    ...
    // Establish the link via hook state and materialize (gate data was removed
    // with the legacy gate subsystem; hook state and artifacts are the sources).
    const hookStateRepo = new WorkflowHookStateRepository(db);
    hookStateRepo.ensure(run.id, 'pr-ready', { pr_url: PR_URL });
    runtime.materializeRunTopicFromInterests(run.id);
    ...
    hookStateRepo.update(run.id, 'pr-ready', {
      expectedVersion: hookStateRepo.get(run.id, 'pr-ready')!.version,
      localState: { pr_url: '' },
    });
    runtime.invalidatePrimaryLinkForRun(run.id);
    runtime.materializeRunTopicFromInterests(run.id);
