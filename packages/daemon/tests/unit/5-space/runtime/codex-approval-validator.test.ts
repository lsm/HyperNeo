...
describe('codexReviewApprovedValidator', () => {
  ...
  test('terminal allow verified against current PR head', async () => {
    ...
    const ctx = makeContext({
      ...
      lastResult: {
        type: 'allow',
        data: { terminalOutcome: 'allow', currentHeadSha: 'abc123', prUrl: PR_URL },
      } as WorkflowHookResult,
    });
    const result = await codexReviewApprovedValidator(ctx);
    expect(result.type).toBe('allow');
  ...
  test('terminal allow invalidated when PR head changes', async () => {
    ...
    const ctx = makeContext({
      ...
      lastResult: {
        type: 'allow',
        data: { terminalOutcome: 'allow', currentHeadSha: 'old-sha', prUrl: PR_URL },
      } as WorkflowHookResult,
    });
    const result = await codexReviewApprovedValidator(ctx);
  ...
  test('terminal allow invalidated when PR URL changes', async () => {
    mockPrWith('abc123', []);
    const ctx = makeContext({
      currentArtifacts: PR_ARTIFACT,
      lastResult: {
  ...
  test('terminal block verified against current PR head', async () => {
    ...
    const ctx = makeContext({
      ...
      lastResult: {
        type: 'block',
        reason: 'timeout',
        data: { terminalOutcome: 'block', currentHeadSha: 'abc123', prUrl: PR_URL },
      } as WorkflowHookResult,
    });
    const result = await codexReviewApprovedValidator(ctx);
    expect(result.type).toBe('block');
  ...
  test('transient GitHub failure preserves prior Codex state', async () => {
    fetchMock = async () => new Response('nope', { status: 500, statusText: 'Server Error' });
    const ctx = makeContext({
      currentArtifacts: PR_ARTIFACT,
      lastResult: {
        type: 'retryable_block',
        reason: 'Waiting',
        data: { currentHeadSha: 'abc123', prUrl: PR_URL, checkStartedAt: 123 },
      } as WorkflowHookResult,
    });
    const result = await codexReviewApprovedValidator(ctx);
    expect(result.type).toBe('block');
  ...
  test('forced legacy hook enforces even without node flag', async () => {
    mockPrWith('abc123', []);
    const ctx = makeContext({
      workflow: {
        ...WORKFLOW,
...
describe('withSyntheticCodexHooks', () => {
  ...
  test('limits node-flag synthetic hooks to approval-gated targets', () => {
    const workflow = withSyntheticCodexHooks({
      ...
      channels: [
        { from: 'Coding', to: 'Review', gateId: 'approval-gate' },
  ...
  test('includes wildcard source channels when synthesizing Codex hooks', () => {
    const workflow = withSyntheticCodexHooks({
      ...WORKFLOW,
      hooks: undefined,
      gates: [
