...
function makeManager(opts: {
  ...
  const resetStartupTimeoutRetryBudget = mock(() => {});
  ...
  return {
    ...
    session: {
      ...
      ensureStartedMock,
      enqueueMock,
      resetStartupTimeoutRetryBudget,
      getProcessingState,
      saveUserMessage,
      jobQueueEnqueue,
      reopenDeliveryByUuid,
...
describe('resetContextPerTurn — TaskAgentManager injection gating', () => {
  ...
  it('clears SDK context before a task handoff to a resetContextPerTurn slot with prior context', async () => {
    ...
    const live = {
      ...
      resetStartupTimeoutRetryBudget: session.resetStartupTimeoutRetryBudget,
...
describe('injectMessageIntoSession — v2 idempotent persist (Codex P1)', () => {
  ...
  it('a retry finding an existing FAILED row reopens it and re-enqueues without a duplicate row', async () => {
    ...
    // Round-15 P2: the inject lane reset the startup-timeout budget for the
    // reopened uuid.
    expect(session.resetStartupTimeoutRetryBudget).toHaveBeenCalledTimes(1);
