...
describe('ensureAgentSession() / isAgentTargetLifecycleEligible()', () => {
  ...
  test('skips the session config refresh when the session archives mid-refresh', async () => {
    const db = makeTestDb();
    seedEnsureSpace(db);
    const repo = new SpaceLongHorizonAgentRepository(db as never);
    const stampedSessionId = longTermAgentSessionId(ENSURE_SPACE_ID, 'lh-late-archive');
    repo.create({
      id: 'lh-late-archive',
      spaceId: ENSURE_SPACE_ID,
      handle: 'researcher',
    ...
    const stampedSession = {
      ...
      getSessionData: mock(() => {
        ...
        return {
          id: stampedSessionId,
          status: statusReads > 1 ? 'archived' : 'active',
          metadata: {},
          config: {},
        };
      }),
    ...
    await svc.refreshLongHorizonAgentSession(ENSURE_SPACE_ID, 'lh-late-archive');

    expect(stampedSession.updateConfig).not.toHaveBeenCalled();
    expect(stampedSession.restart).not.toHaveBeenCalled();
  });

  test('archives the provisional session when space registration fails', async () => {
    const db = makeTestDb();
    seedEnsureSpace(db);
    const repo = new SpaceLongHorizonAgentRepository(db as never);
    repo.create({ id: 'lh-regfail-1', spaceId: ENSURE_SPACE_ID, handle: 'researcher' });
    const agentSessionId = longTermAgentSessionId(ENSURE_SPACE_ID, 'lh-regfail-1');
    const createdSession = {
      mergeRuntimeMcpServers: mock(() => {}),
      updateConfig: mock(async () => {}),
      resetQuery: mock(async () => ({ success: true })),
    ...
    const spaceManager = {
      getSpace: mock(async () => makeEnsureSpace()),
      addSession: mock(async () => {
        throw new Error('space gone');
      }),
    } as unknown as SpaceManager;
    const svc = new SpaceRuntimeService(buildEnsureConfig(db, repo, spaceManager, sessionManager));

    const ensured = await svc.ensureAgentSession(ENSURE_SPACE_ID, 'lh-regfail-1');

    expect(ensured).toBeNull();
    expect(updateSession).toHaveBeenCalledWith(agentSessionId, { status: 'archived' });
    expect(repo.getById('lh-regfail-1')?.sessionId).toBeNull();
  });

  test('restores terminal state when revival provisioning fails', async () => {
    const db = makeTestDb();
    seedEnsureSpace(db);
    const repo = new SpaceLongHorizonAgentRepository(db as never);
    repo.create({ id: 'lh-revfail-1', spaceId: ENSURE_SPACE_ID, handle: 'researcher' });
    const agentSessionId = longTermAgentSessionId(ENSURE_SPACE_ID, 'lh-revfail-1');
    const sessionState = { status: 'archived' };
    const archivedSession = {
      mergeRuntimeMcpServers: mock(() => {}),
      updateConfig: mock(async () => {}),
    ...
    const updateSession = mock(async (_id: string, updates: { status: string }) => {
      sessionState.status = updates.status;
    });
    const sessionManager = {
      getSessionAsync: mock(async () => archivedSession),
      updateSession,
    ...
    const ensured = await svc.ensureAgentSession(ENSURE_SPACE_ID, 'lh-revfail-1');

    expect(ensured).toBeNull();
    expect(updateSession).toHaveBeenNthCalledWith(1, agentSessionId, { status: 'active' });
    expect(updateSession).toHaveBeenNthCalledWith(2, agentSessionId, { status: 'archived' });
    expect(sessionState.status).toBe('archived');
    expect(repo.getById('lh-revfail-1')?.sessionId).toBeNull();
  });

  test('skips the stamped-session refresh for inactive agents', async () => {
    const db = makeTestDb();
    seedEnsureSpace(db);
    const repo = new SpaceLongHorizonAgentRepository(db as never);
    const stampedSessionId = longTermAgentSessionId(ENSURE_SPACE_ID, 'lh-inactive');
    repo.create({
      id: 'lh-inactive',
      spaceId: ENSURE_SPACE_ID,
      handle: 'researcher',
    ...
    await svc.refreshLongHorizonAgentSession(ENSURE_SPACE_ID, 'lh-inactive');
  ...
  test('coalesces concurrent ensures for the same agent session', async () => {
    ...
    seedEnsureSpace(db);
    const repo = new SpaceLongHorizonAgentRepository(db as never);
    repo.create({ id: 'lh-coalesce-1', spaceId: ENSURE_SPACE_ID, handle: 'researcher' });
    const agentSessionId = longTermAgentSessionId(ENSURE_SPACE_ID, 'lh-coalesce-1');
    const createdSession = {
      mergeRuntimeMcpServers: mock(() => {}),
      updateConfig: mock(async () => {}),
      resetQuery: mock(async () => ({ success: true })),
    ...
    const first = svc.ensureAgentSession(ENSURE_SPACE_ID, 'lh-coalesce-1');
    await new Promise((resolve) => setTimeout(resolve, 0));
    const second = svc.ensureAgentSession(ENSURE_SPACE_ID, 'lh-coalesce-1');
    await new Promise((resolve) => setTimeout(resolve, 0));
    ...
    expect(repo.getById('lh-coalesce-1')?.sessionId).toBe(agentSessionId);
  });
  ...
  test('aborts unstamped when the agent pauses during provisioning', async () => {
    const db = makeTestDb();
    seedEnsureSpace(db);
    const repo = new SpaceLongHorizonAgentRepository(db as never);
    repo.create({ id: 'lh-ensure-4', spaceId: ENSURE_SPACE_ID, handle: 'researcher' });
    const agentSessionId = longTermAgentSessionId(ENSURE_SPACE_ID, 'lh-ensure-4');
    const createdSession = {
      mergeRuntimeMcpServers: mock(() => {}),
      updateConfig: mock(async () => {}),
      resetQuery: mock(async () => ({ success: true })),
    ...
    const ensured = svc.ensureAgentSession(ENSURE_SPACE_ID, 'lh-ensure-4');
    await new Promise((resolve) => setTimeout(resolve, 0));
    repo.update('lh-ensure-4', { status: 'paused' });
    resolveCreate();
  ...
  test('aborts unstamped when the space pauses during provisioning', async () => {
    ...
    seedEnsureSpace(db);
    const repo = new SpaceLongHorizonAgentRepository(db as never);
    repo.create({ id: 'lh-ensure-6', spaceId: ENSURE_SPACE_ID, handle: 'researcher' });
    const agentSessionId = longTermAgentSessionId(ENSURE_SPACE_ID, 'lh-ensure-6');
    ...
    const ensured = svc.ensureAgentSession(ENSURE_SPACE_ID, 'lh-ensure-6');
    await new Promise((resolve) => setTimeout(resolve, 0));
    space.paused = true;
    resolveCreate();
    const result = await ensured;
  ...
  test('leaves the session unstamped when edits keep landing past the refresh bound', async () => {
    ...
    const agentSessionId = longTermAgentSessionId(ENSURE_SPACE_ID, 'lh-ensure-5');
    ...
    const ensured = svc.ensureAgentSession(ENSURE_SPACE_ID, 'lh-ensure-5');
    for (let round = 1; round <= 4; round++) {
      await new Promise((resolve) => setTimeout(resolve, 0));
      repo.update('lh-ensure-5', { instructions: `v${round}` });
      resolveUpdate();
    }
    const result = await ensured;

    expect(result).toBeNull();
    expect(repo.getById('lh-ensure-5')?.sessionId).toBeNull();
  });
});
