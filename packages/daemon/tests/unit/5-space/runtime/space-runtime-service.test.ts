...
function buildLongHorizonAgent(
  ...
  return {
    ...
    templateKey: null,
    status: 'active',
    sessionId: longTermAgentSessionId(overrides.spaceId ?? 'space-1', overrides.id ?? 'lh-agent-1'),
    instructions: '',
    autonomyLevel: null,
    model: null,
    thinkingLevel: null,
...
describe('SpaceRuntimeService', () => {
  ...
  describe('agent session delivery', () => {
    ...
    test('long-horizon delivery starts inactive session without worker inbox', async () => {
      ...
      const longHorizonAgentRepo = {
        getById: mock(() => ({
          ...
          templateKey: null,
          status: 'active',
          sessionId: longTermAgentSessionId(mockSpace.id, 'lh-agent-1'),
    ...
    test('session.reset re-provisions reset long-term Space agents before query replay', async () => {
      ...
      const svc = new SpaceRuntimeService(config);
      const longTermSessionId = longTermAgentSessionId(mockSpace.id, 'lh-agent-1');
  ...
  describe('deliverGoalOutcomeWake()', () => {
    ...
    function makeWakeSessionManager(
      ...
      return {
        ...
        getSession: mock((sessionId: string) => sessions.get(sessionId) ?? null),
        getOperationRegistry: mock(() => createOperationRegistry([])),
  ...
  describe('ensureAgentSession()', () => {
    ...
    test.each(cases)('registers session %s with publish failure %s', async (existing, fails) => {
      ...
      const sessionData = {
        id: sessionId,
        title: 'Task Manager',
        workspacePath: mockSpace.workspacePath,
        createdAt: NOW,
        lastActiveAt: NOW,
        status: 'active',
        config: {},
      ...
      const longHorizonAgentRepo = {
        getById: mock(() =>
          buildLongHorizonAgent({
            ...
            instructions: 'Triage and track Space tasks.',
          })
        ),
        update: mock(() => {}),
      } as unknown as SpaceRuntimeServiceConfig['longHorizonAgentRepo'];
  ...
  describe('Space agent tool attachment', () => {
    ...
    function buildMemberConfig(opts: {
      ...
      return {
        db: {} as BunDatabase,
        dbPath: opts.dbPath,
        spaceManager: opts.spaceManager ?? createMockSpaceManager(mockSpace),
        spaceWorkflowManager: {
          listWorkflows: mock(() => []),
        } as unknown as SpaceWorkflowManager,
        workflowRunRepo: {} as SpaceWorkflowRunRepository,
    ...
    function makeActiveLhAgentRepo(
      overrides: Partial<SpaceLongHorizonAgent> = {}
    ): SpaceRuntimeServiceConfig['longHorizonAgentRepo'] {
      return {
        getById: mock(() =>
          buildLongHorizonAgent({
            id: 'agent-1',
    ...
    test('start() attaches tools only to the long-term agent sessions listed by sessionManager', async () => {
      const agent = makeMemberAgentSession();
      const sessionManager = makeSessionManager(agent);
    ...
    test('leaves a long-term agent session that already carries a role prompt untouched', async () => {
      ...
      const agent = makeMemberAgentSession({
        id: sessionId,
        config: { tools: {}, systemPrompt: 'Existing role text' },
        metadata: {
          promptProvenance: {
            source: 'test',
            hash: 'hash',
    ...
    test('repairs a long-term agent session whose config lost its role system prompt', async () => {
      const sessionId = longTermAgentSessionId(mockSpace.id, 'agent-1');
      const agent = makeMemberAgentSession({
        id: sessionId,
        config: { tools: {} },
      ...
      const systemPrompt = agent.getSessionData().config.systemPrompt as { append?: string };
      expect(systemPrompt.append).toContain('Triage and track Space tasks.');
    });

    test('onMissingMemberSpaceMcpServers self-heal callback re-attaches long-term agent tools', async () => {
      const agent = makeMemberAgentSession({
        id: longTermAgentSessionId(mockSpace.id, 'agent-1'),
        metadata: {
          promptProvenance: { source: 'test', hash: 'hash', agentId: 'agent-1' },
        },
      });
      const sessionManager = makeSessionManager(agent);
      const svc = new SpaceRuntimeService(
      ...
      await (agent as unknown as AgentSession).onMissingMemberSpaceMcpServers?.(
        longTermAgentSessionId(mockSpace.id, 'agent-1'),
        ['space-agent-tools']
      );
    ...
    test('long-term agent reattach preserves existing runtime MCP servers and avoids duplicate server names', async () => {
      ...
      expect(serverNames.sort()).toEqual(['external-runtime']);
  ...
  describe('internalEventBus session.created subscription (Task #137 regression)', () => {
    ...
    test('installs the operations provider when internalEventBus emits session.created with a UUID sessionId', async () => {
      const agent = makeMemberAgentSession();
      const sessionManager = {
        getSessionAsync: mock(async () => agent),
        getSession: mock(() => agent),
        ...
        longHorizonAgentRepo: {
          getById: mock(() =>
            buildLongHorizonAgent({
              id: 'agent-uuid',
              spaceId: mockSpace.id,
              sessionId: 'worker-session-uuid-123',
            })
          ),
          listBySpaceId: mock(() => []),
        } as unknown as SpaceRuntimeServiceConfig['longHorizonAgentRepo'],
      ...
      const session = makeMemberSession({
        metadata: {
          promptProvenance: { source: 'test', hash: 'hash', agentId: 'agent-uuid' },
        },
      } as Partial<Session>);
  ...
  describe('ready() — startup provisioning gate', () => {
    ...
    test('ready() resolves only after the member-session sweep has completed', async () => {
      ...
      const sessionManager = {
        ...
        listSessions: mock(() => [
          {
            ...makeMemberSession('member-gated'),
            metadata: {
              promptProvenance: { source: 'test', hash: 'hash', agentId: 'agent-gated' },
            },
          } as Session,
        ]),
      } as unknown as SessionManager;
...
describe('clearLongTermAgentSessionProvider — provider-override clear (P2)', () => {
  function buildService(session: unknown): SpaceRuntimeService {
    ...
    return new SpaceRuntimeService({
      ...
      sessionManager,
      longHorizonAgentRepo: {
        getById: () => buildLongHorizonAgent({ id: 'agent-1', spaceId: 'space-1' }),
      } as unknown as SpaceRuntimeServiceConfig['longHorizonAgentRepo'],
    });
  }
