...
describe('stagedRun declared-key access', () => {
  ...
  test('non-idempotent reads enumeration cannot bypass the stale-read validation', async () => {
    type Pair = { a: number; b: number };
    let enumerations = 0;
    const shift = ['a'];
    Object.defineProperty(shift, 0, {
      enumerable: true,
      get: () => {
        enumerations += 1;
        return enumerations === 1 ? 'a' : 'ghost';
      },
    });
    let seen: Record<string, unknown> = {};
    const flow = stagedRun<Pair>('shifting-enumeration', (s) => [
