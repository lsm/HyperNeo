...
describe('handleGoalAutomationExecute', () => {
  ...
  it('skips a completion-only manual note', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Completion only', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Completion only',
      objective: 'Bare completion words are process status, not work artifacts',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'manual_note',
      sourceId: null,
      // "Done" is deliberately excluded from AFFIRMATIVE_OUTCOME_RE (process
      // status, not an artifact) — it must land in the thin-note vocabulary
      // instead so the note does not bypass the gate.
      summary: 'Done',
      createdAt: 30,
    });
    let episodeCreated = false;
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-completion-only',
        reason: 'self_nag',
        scheduleId: 'schedule-completion-only',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async () => {
            episodeCreated = true;
            throw new Error('should not create episode for a completion-only note');
          },
          preflightEvidence: () =>
            makePreflight({
              level: 'low',
  ...
  it('still produces an episode for a modal-led future clause after a completed outcome', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Modal-led clause', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Modal-led clause',
      objective: 'A new subject with a modal auxiliary is an independent clause',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'manual_note',
      sourceId: null,
      // "and deployment will start" is a future clause with its own subject;
      // the completed "passed" outcome must not inherit its qualifier.
      summary: 'Tests passed and deployment will start tomorrow',
      createdAt: 30,
    });
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-modal-led-clause',
        reason: 'self_nag',
        scheduleId: 'schedule-modal-led-clause',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async ({ evidenceIds }) => ({
            episode: evolutionRepo.createEpisode({
              scopeId: scope.id,
              title: 'Modal-led clause retrospective',
              evidenceIds,
              outcomeSummary: 'Modal-led clause outcome',
              findings: [],
            }),
            proposals: [],
            lessons: [],
  ...
  it('still produces an episode for a diagnosis joined by a conjunction', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Conjunction mix', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Conjunction mix',
      objective: 'A status clause and a diagnosis joined only by a conjunction still split',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'manual_note',
      sourceId: null,
      summary: 'Rollout is blocked but root cause was lock contention',
      createdAt: 30,
    });
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-conjunction-mix',
        reason: 'self_nag',
        scheduleId: 'schedule-conjunction-mix',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async ({ evidenceIds }) => ({
            episode: evolutionRepo.createEpisode({
              scopeId: scope.id,
              title: 'Conjunction mix retrospective',
              evidenceIds,
              outcomeSummary: 'Conjunction mix outcome',
              findings: [],
            }),
            proposals: [],
            lessons: [],
  ...
  it('skips a note targeting a future artifact release', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Target artifact', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Target artifact',
      objective: 'Goal words as prefixes make artifact references prospective',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'manual_note',
      sourceId: null,
      // "release" supplies artifact context, but "Target" as a prefix marks
      // the reference prospective, not achieved.
      summary: 'Target release v2.4.1',
      createdAt: 30,
    });
    let episodeCreated = false;
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-target-artifact',
        reason: 'self_nag',
        scheduleId: 'schedule-target-artifact',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async () => {
            episodeCreated = true;
            throw new Error('should not create episode for a target, not an achieved release');
          },
          preflightEvidence: () =>
            makePreflight({
              level: 'low',
  ...
  it('skips a pending PR whose coordinated predicate has no subject', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Elliptical clause', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Elliptical clause',
      objective: 'An omitted-subject coordinated predicate is not an independent clause',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'manual_note',
      sourceId: null,
      // "and is still pending" shares the PR subject — the PR reference
      // must stay non-affirmative pending work.
      summary: 'PR #123 opened and is still pending',
      createdAt: 30,
    });
    let episodeCreated = false;
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-elliptical-clause',
        reason: 'self_nag',
        scheduleId: 'schedule-elliptical-clause',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async () => {
            episodeCreated = true;
            throw new Error('should not create episode for a pending PR');
          },
          preflightEvidence: () =>
            makePreflight({
              level: 'low',
  ...
  it('still produces an episode for a negative diagnostic statement', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Negative finding', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Negative finding',
      objective: 'A negated technical finding is a diagnosis, not process status',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'manual_note',
      sourceId: null,
      summary: 'No index covers the query',
      createdAt: 30,
    });
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-negative-finding',
        reason: 'self_nag',
        scheduleId: 'schedule-negative-finding',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async ({ evidenceIds }) => ({
            episode: evolutionRepo.createEpisode({
              scopeId: scope.id,
              title: 'Negative finding retrospective',
              evidenceIds,
              outcomeSummary: 'Negative finding outcome',
              findings: [],
            }),
            proposals: [],
            lessons: [],
  ...
  it('still produces an episode for a diagnosis attached to a status marker', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Causal diagnosis', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Causal diagnosis',
      objective: 'Content after because explains why and is substantive',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'manual_note',
      sourceId: null,
      // "blocked" is a hard status marker, but the causal clause after
      // "because" carries the diagnosis and keeps the note substantive.
      summary: 'Rollout is blocked because the cache key ignores tenant IDs',
      createdAt: 30,
    });
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-causal-diagnosis',
        reason: 'self_nag',
        scheduleId: 'schedule-causal-diagnosis',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async ({ evidenceIds }) => ({
            episode: evolutionRepo.createEpisode({
              scopeId: scope.id,
              title: 'Causal diagnosis retrospective',
              evidenceIds,
              outcomeSummary: 'Causal diagnosis outcome',
              findings: [],
            }),
            proposals: [],
            lessons: [],
  ...
  it('still produces an episode for completed work with an object', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Completed object', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Completed object',
      objective: 'A completion word opening a description of the work is substantive',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'manual_note',
      sourceId: null,
      summary: 'Completed migration to tenant-scoped cache',
      createdAt: 30,
    });
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-completed-object',
        reason: 'self_nag',
        scheduleId: 'schedule-completed-object',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async ({ evidenceIds }) => ({
            episode: evolutionRepo.createEpisode({
              scopeId: scope.id,
              title: 'Completed object retrospective',
              evidenceIds,
              outcomeSummary: 'Completed object outcome',
              findings: [],
            }),
            proposals: [],
            lessons: [],
  ...
  it('skips a status note whose colon suffix is shorthand', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Label shorthand', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Label shorthand',
      objective: 'A bare identifier or task shorthand after a status label stays status',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-label-shorthand',
        reason: 'self_nag',
        scheduleId: 'schedule-label-shorthand',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async () => {
            episodeCreated = true;
            throw new Error('should not create episode for label shorthand');
          },
          preflightEvidence: () =>
            makePreflight({
              level: 'low',
  ...
  it('skips a temporal since duration note', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Temporal since', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Temporal since',
      objective: 'A since-duration phrase is part of the status, not a rationale clause',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-temporal-since',
        reason: 'self_nag',
        scheduleId: 'schedule-temporal-since',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async () => {
            episodeCreated = true;
            throw new Error('should not create episode for a since-duration status note');
          },
          preflightEvidence: () =>
            makePreflight({
              level: 'low',
  ...
  it('skips a status note whose identifier metadata names a pending PR', async () => {
    const goal = goalRepo.create({ spaceId, title: 'URL metadata', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'URL metadata',
      objective: 'An externalUrl identifier must not read as an affirmative PR outcome',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      ...
      sourceId: null,
      summary: 'Waiting for CI',
      // The URL only identifies the pending PR; manual-note metadata is
      // provenance, not outcome content.
      metadata: { externalUrl: 'https://github.com/org/repo/pull/123' },
      createdAt: 30,
    });
    let episodeCreated = false;
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-url-metadata',
        reason: 'self_nag',
        scheduleId: 'schedule-url-metadata',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async () => {
            episodeCreated = true;
            throw new Error('should not create episode from an identifier URL in metadata');
          },
          preflightEvidence: () =>
            makePreflight({
              level: 'low',
  ...
  it('skips a status note whose colon suffix describes running status', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Running suffix', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Running suffix',
      objective: 'A linking verb alone does not make a colon suffix diagnostic',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-running-suffix',
        reason: 'self_nag',
        scheduleId: 'schedule-running-suffix',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async () => {
            episodeCreated = true;
            throw new Error('should not create episode for a running-status colon suffix');
          },
          preflightEvidence: () =>
            makePreflight({
              level: 'low',
  ...
  it('still produces an episode for a diagnosis introduced by due to', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Due to', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Due to',
      objective: 'A due-to rationale is a causal clause carrying the diagnosis',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'manual_note',
      sourceId: null,
      summary: 'Rollout is blocked due to the cache key ignoring tenant IDs',
      createdAt: 30,
    });
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-due-to',
        reason: 'self_nag',
        scheduleId: 'schedule-due-to',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async ({ evidenceIds }) => ({
            episode: evolutionRepo.createEpisode({
              scopeId: scope.id,
              title: 'Due to retrospective',
              evidenceIds,
              outcomeSummary: 'Due to outcome',
              findings: [],
            }),
            proposals: [],
            lessons: [],
  ...
  it('skips a conditional plan note', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Conditional plan', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Conditional plan',
      objective: 'An outcome under if/when/once is prospective, not achieved',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'manual_note',
      sourceId: null,
      summary: 'If tests pass, rollout is scheduled tomorrow',
      createdAt: 30,
    });
    let episodeCreated = false;
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-conditional-plan',
        reason: 'self_nag',
        scheduleId: 'schedule-conditional-plan',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async () => {
            episodeCreated = true;
            throw new Error('should not create episode for a conditional plan');
          },
          preflightEvidence: () =>
            makePreflight({
              level: 'low',
  ...
  it('skips an awaiting status note', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Awaiting', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Awaiting',
      objective: 'await/awaiting are pending-status vocabulary',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-awaiting',
        reason: 'self_nag',
        scheduleId: 'schedule-awaiting',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async () => {
            episodeCreated = true;
            throw new Error('should not create episode for an awaiting note');
          },
          preflightEvidence: () =>
            makePreflight({
              level: 'low',
  ...
  it('skips a conditional plan with coordinated conditions', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Coordinated if', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Coordinated if',
      objective: 'A conditional scopes the conditions coordinated after it',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'manual_note',
      sourceId: null,
      summary: 'If lint passes and tests pass, rollout is scheduled tomorrow',
      createdAt: 30,
    });
    let episodeCreated = false;
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-coordinated-if',
        reason: 'self_nag',
        scheduleId: 'schedule-coordinated-if',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async () => {
            episodeCreated = true;
            throw new Error('should not create episode for coordinated conditional conditions');
          },
          preflightEvidence: () =>
            makePreflight({
              level: 'low',
  ...
  it('still produces an episode for a diagnosis introduced by blocked by', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Blocked by', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Blocked by',
      objective: 'A blocked-by cause phrase with an article carries the diagnosis',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'manual_note',
      sourceId: null,
      summary: 'Rollout is blocked by a deadlock in the tenant cache',
      createdAt: 30,
    });
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-blocked-by',
        reason: 'self_nag',
        scheduleId: 'schedule-blocked-by',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async ({ evidenceIds }) => ({
            episode: evolutionRepo.createEpisode({
              scopeId: scope.id,
              title: 'Blocked by retrospective',
              evidenceIds,
              outcomeSummary: 'Blocked by outcome',
              findings: [],
            }),
            proposals: [],
            lessons: [],
  ...
  it('skips an active execution status note', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Active execution', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Active execution',
      objective: 'running / in progress are process status',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-active-execution',
        reason: 'self_nag',
        scheduleId: 'schedule-active-execution',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async () => {
            episodeCreated = true;
            throw new Error('should not create episode for an active execution note');
          },
          preflightEvidence: () =>
            makePreflight({
              level: 'low',
  ...
  it('skips when a trace capture failure diagnostic is the only due row', async () => {
    const goal = goalRepo.create({ spaceId, title: 'Trace capture error', type: 'recurring' });
    const scope = evolutionRepo.createScope({
      spaceId,
      spaceGoalId: goal.id,
      kind: 'mission',
      name: 'Trace capture error',
      objective: 'Capture-infrastructure noise is not a project outcome',
      policy: { automation: { selfNagCronExpression: '0 * * * *' } },
    });
    evolutionRepo.createEvidence({
      scopeId: scope.id,
      kind: 'session',
      sourceId: 'session-trace-capture',
      summary: 'Trace evidence capture failed',
      metadata: { traceDiagnostic: true, error: 'session already archived' },
      createdAt: 30,
    });
    let episodeCreated = false;
    ...
    const result = await handleGoalAutomationExecute(
      createAutomationJob({
        ...
        scopeId: scope.id,
        triggerKind: 'self_nag',
        triggerKey: 'schedule-trace-capture-error',
        reason: 'self_nag',
        scheduleId: 'schedule-trace-capture-error',
      }),
      {
        goalRepo,
        taskRepo,
        ...
        episodeService: {
          createFromEvidence: async () => {
            episodeCreated = true;
            throw new Error('should not create episode for capture infrastructure noise');
          },
          preflightEvidence: () =>
            makePreflight({
              level: 'low',
