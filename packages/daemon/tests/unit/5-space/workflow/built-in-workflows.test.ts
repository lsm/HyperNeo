...
import { isApprovalGate } from '../../../../src/lib/space/runtime/gate-features.ts';
...
describe('PLAN_AND_DECOMPOSE_WORKFLOW template', () => {
  ...
  test('plan review approval migration carries vote data and resets votes on revision feedback', () => {
    ...
    expect(approvalSource).toContain('Plan dispatch requires four approved plan-review votes');
    expect(approvalSource).toContain('approval_count');
...
describe('seedBuiltInWorkflows()', () => {
  ...
  test('mergeGateStructuralFieldsFromTemplate preserves existing gate features (feature mechanism retired)', () => {
    // The gate-feature mechanism (incl. the legacy codex_review_bot feature) was
    // removed in #2304; restamp no longer copies/clears template features, it
    // preserves whatever the gate already carries.
    const existingGates = [{ id: 'g1', fields: [], features: { some_other_feature: true } }];
    const templateGates = [{ id: 'g1', fields: [] }];

    const result = mergeGateStructuralFieldsFromTemplate(existingGates, templateGates);
    expect(result).toHaveLength(1);
    expect(result![0].features).toEqual({ some_other_feature: true });
  });
  ...
  test('migrated approval hook is approval-only and codex is a declarative preset hook', () => {
    const workflow = migrateWorkflowGateProgressionToHooks({
      ...PLAN_AND_DECOMPOSE_WORKFLOW,
      templateName: PLAN_AND_DECOMPOSE_WORKFLOW.name,
      templateGates: PLAN_AND_DECOMPOSE_WORKFLOW.gates ?? [],
    }).workflow;
    // Approval hook: approval-count only, no codex bash (the epic #2299 #2304
    // unification moved codex into a declarative preset hook).
    const approval = workflow.hooks?.find(
      (candidate) =>
        candidate.sourceNode === 'Plan Review' &&
        candidate.targetNode === 'Task Dispatcher' &&
        candidate.validator.kind === 'script'
    );
    const source = approval?.validator.kind === 'script' ? approval.validator.source : '';
    expect(source).toContain('Plan dispatch requires four approved plan-review votes');
    expect(source).not.toContain('gh pr view');
    expect(source).not.toContain('codex_timed_out');

    // Codex hook: a built_in reference to the `codex_review_approved` preset,
    // ordered after the approval hook so the wait starts at the handoff.
  ...
  test('re-stamp reuses renamed generated route hooks', () => {
    ...
    // Only the script approval hook is counted — the re-stamp also adds the
    // remapped `codex_review_approved` built_in hook on the renamed route.
    const approvalHooks = after.hooks?.filter(
      (hook) =>
        hook.sourceNode === 'Plan Review Renamed' &&
        hook.targetNode === 'Dispatch Renamed' &&
        hook.validator.kind === 'script'
    );
    expect(approvalHooks).toHaveLength(1);
  });
