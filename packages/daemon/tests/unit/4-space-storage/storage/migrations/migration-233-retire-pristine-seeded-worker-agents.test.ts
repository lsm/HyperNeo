...
function createDb(): {
  ...
  return {
    db,
    repo,
    idsByName: new Map(presets.map((preset) => [preset.name, `agent-${preset.handle}`])),
  };
}
...
function insertCustomNamedCoder(db: Database): void {
  db.prepare(
    `INSERT INTO space_long_horizon_agents (
       ...
       NULL, 'My own coder prompt', '{}', 'My own coder', 1, 1)`
  ).run();
}
...
describe('migration 233: retire pristine seeded worker agents', () => {
  ...
  test('ignores junk agent entries and malformed node configs', () => {
    ...
    expect(() => runMigration233(db)).not.toThrow();
    expect(remaining(repo)).toEqual([]);
    db.close();
  });
  ...
  test('keeps a pristine worker referenced by a tab-and-newline-padded m228 key', () => {
    const { db, repo, idsByName } = createDb();
    insertNodeWithAgents(
      db,
      'node-tab-key',
  ...
  test('keeps a pristine worker whose dotted id appears in an m228 key', () => {
    const { db, repo, idsByName } = createDb();
    db.prepare(`UPDATE space_long_horizon_agents SET id = 'team.worker' WHERE id = ?`).run(
      idsByName.get('Coder')
    );
    insertNodeWithAgents(
      db,
      'node-dotted-key',
      JSON.stringify({ agents: [{ agentId: '', templateKey: 'migrated.agent.team.worker' }] })
    );
