...
import {
  ...
  runMigration205,
} from '../../../../../src/storage/schema/m205-restamp-reviewer-review-modes.ts';
...
describe('migration 205 — reviewer review-modes contract restamp', () => {
  test('re-stamps a pristine Reviewer row carrying the pre-review-modes contract', () => {
    const spaceId = 'space-m205-a';
    insertSpace(db, spaceId);
    insertAgent(db, {
      id: 'agent-reviewer',
      spaceId,
      name: 'Reviewer',
      customPrompt: OLD_REVIEWER_PROMPT_PRE_REVIEW_MODES,
    ...
    expect(row.custom_prompt).toContain('Your role by review source');
  ...
  test('leaves a customized Reviewer row untouched', () => {
    ...
    const otherSpaceId = 'space-m205-c2';
  ...
  test('ignores non-Reviewer agents', () => {
    ...
    expect(row.custom_prompt).toBe(OLD_REVIEWER_PROMPT_PRE_REVIEW_MODES);
  });
});
