/**
 * Migration 181 tests — `uq_message_delivery_active_turn` (message-delivery v2).
 *
 ...
 *   - existing DB upgrade → `runMigration181` (job_queue already exists when
 *     migrations run);
...
describe('Migration 181: uq_message_delivery_active_turn', () => {
  ...
  test('existing-DB upgrade: runMigration181 creates the index once job_queue exists', () => {
    makeJobQueue(db);
  ...
  test('runMigration181 is a no-op (idempotent) when run twice', () => {
    makeJobQueue(db);
    runMigration181(db);
  ...
  test('runMigration181 is a guarded no-op before the table exists (fresh-DB migration order)', () => {
