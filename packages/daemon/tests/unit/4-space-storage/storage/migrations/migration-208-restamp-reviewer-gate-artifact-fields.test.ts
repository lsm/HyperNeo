...
import {
  OLD_REVIEWER_PROMPT_PRE_GATE_ARTIFACT_FIELDS,
  runMigration208,
} from '../../../../../src/storage/schema/m208-restamp-reviewer-gate-artifact-fields.ts';
...
describe('migration 208 — reviewer gate-artifact-fields contract restamp', () => {
  test('re-stamps a pristine Reviewer row carrying the pre-review-modes contract', () => {
    const spaceId = 'space-m208-a';
    ...
    expect(row.custom_prompt).toContain('base_oid: "<baseRefOid>"');
    expect(row.custom_prompt).toContain('the latest explicit instruction wins');
  ...
  test('leaves a Reviewer row with customized tools untouched', () => {
    ...
    expect(row.custom_prompt).toBe(OLD_REVIEWER_PROMPT_PRE_GATE_ARTIFACT_FIELDS);
    expect(row.template_name).toBeNull();
  });

  test('leaves a customized Reviewer row untouched', () => {
    const spaceId = 'space-m208-c';
    insertSpace(db, spaceId);
    insertAgent(db, {
      id: 'agent-custom',
      spaceId,
    ...
    const otherSpaceId = 'space-m208-c2';
