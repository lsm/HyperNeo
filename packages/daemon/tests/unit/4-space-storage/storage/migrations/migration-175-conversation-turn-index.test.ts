...
import { runMigration175 } from '../../../../../src/storage/schema/migrations';

/**
 * The pre-175 `sdk_messages` shape: has the columns the backfill reads
 ...
 */
function createPre175SdkMessages(db: BunDatabase): void {
  db.exec(`
...
describe('Migration 175: conversation_turn_index backfill + index', () => {
  let db: BunDatabase;
  ...
  test('is idempotent', () => {
    ...
    runMigration175(db);
    expect(() => runMigration175(db)).not.toThrow();
    expect(
      (
