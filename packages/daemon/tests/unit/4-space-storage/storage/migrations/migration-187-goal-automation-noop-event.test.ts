/**
 * Migration 187 Tests — widen space_goal_events.event_type CHECK to include
 * 'automation_noop'.
 *
 ...
 * cannot ALTER a CHECK constraint, so M187 rebuilds the table (same pattern as
...
import { runMigration187 } from '../../../../../src/storage/schema/index.ts';
...
describe('runMigration187', () => {
  ...
  test('is a safe no-op when space_goal_events does not exist', () => {
    const db = new BunDatabase(':memory:');
    expect(() => runMigration187(db)).not.toThrow();
    db.close();
  });
});
