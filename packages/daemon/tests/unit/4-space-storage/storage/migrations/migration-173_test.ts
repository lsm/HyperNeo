/**
 * Migration 173 Tests — post-approval completion resumability columns (task #868).
 *
 ...
 *   - Pre-M173 schema (columns absent) gains them after the migration.
...
import { runMigration173 } from '../../../../../src/storage/schema/m173-post-approval-completion-columns';
...
describe('Migration 173 — post-approval completion columns', () => {
  let db: BunDatabase;
  beforeEach(() => {
  ...
  test('adds the columns when absent (pre-M173 schema)', () => {
    // Minimal pre-M173 space_tasks with the M103 post-approval columns present
    // (so we are simulating a DB that ran M103 but not M173).
    db.exec(`
  ...
  test('idempotent — running twice is a no-op', () => {
    ...
    runMigration173(db); // second run must not throw / not duplicate
