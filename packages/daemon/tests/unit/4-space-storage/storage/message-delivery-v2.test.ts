...
describe('message-delivery v2 — substrate (job_queue)', () => {
  ...
  describe('activeMailboxMessageUuids — in-flight mailbox handoff guard', () => {
    function enqueueMailboxJob(
      sessionId: string,
      messageUuid: string,
      settle?: 'processing' | 'completed'
    ): void {
      repo.enqueue({
        queue: 'mailbox',
        payload: {
          id: `entry-${messageUuid}`,
      ...
      if (settle === 'processing') {
        const [job] = repo.dequeue('mailbox', 1);
        expect(job).toBeDefined();
      }
      if (settle === 'completed') {
        const [job] = repo.dequeue('mailbox', 1);
        repo.complete(job.id, { ok: true }, job.claimToken);
      }
    }

    it('lists seeded uuids of pending and processing entries addressed to the session', () => {
      expect(repo.activeMailboxMessageUuids(SESSION)).toEqual(new Set());
      enqueueMailboxJob(SESSION, 'mbox-a');
      enqueueMailboxJob(SESSION, 'mbox-b', 'processing');
      expect(repo.activeMailboxMessageUuids(SESSION)).toEqual(new Set(['mbox-a', 'mbox-b']));
    });

    it('excludes completed entries, other sessions, and agent-addressed entries', () => {
      enqueueMailboxJob(SESSION, 'mbox-done', 'completed');
      enqueueMailboxJob('sess-other', 'mbox-other');
      repo.enqueue({
        queue: 'mailbox',
        payload: {
          id: 'entry-agent',
          to: { kind: 'agent', spaceId: 'space-1', handle: 'worker' },
          origin: 'space_agent',
          messageUuid: 'mbox-agent',
          status: 'enqueued',
          deliveryMode: 'immediate',
        },
      });
      expect(repo.activeMailboxMessageUuids(SESSION)).toEqual(new Set());
      expect(repo.activeMailboxMessageUuids('sess-other')).toEqual(new Set(['mbox-other']));
    });

    it('ignores entries without a seeded messageUuid', () => {
      repo.enqueue({
        queue: 'mailbox',
        payload: {
          id: 'entry-unseeded',
  ...
  describe('mailboxEntryJobStatus — per-entry settlement lookup', () => {
    it('tracks an entry from pending through processing to completed and reports absent for unknown ids', () => {
      repo.enqueue({
        queue: 'mailbox',
        payload: { id: 'entry-x', to: { kind: 'session', sessionId: SESSION }, origin: 'chat' },
      });
      expect(repo.mailboxEntryJobStatus('entry-x')).toBe('pending');
      const [job] = repo.dequeue('mailbox', 1);
      expect(repo.mailboxEntryJobStatus('entry-x')).toBe('processing');
      repo.complete(job.id, { ok: true }, job.claimToken);
      expect(repo.mailboxEntryJobStatus('entry-x')).toBe('completed');
      expect(repo.mailboxEntryJobStatus('entry-unknown')).toBe('absent');
    });

    it('reports dead for an entry whose job dead-lettered', () => {
      repo.enqueue({
        queue: 'mailbox',
        payload: {
          id: 'entry-doomed',
      ...
      const [job] = repo.dequeue('mailbox', 1);
      repo.markDead(job.id, 'retries exhausted', job.claimToken);
      expect(repo.mailboxEntryJobStatus('entry-doomed')).toBe('dead');
    });
  });

  describe('cancelPendingMailboxEntry — bounded cancellation', () => {
    it('deletes a pending entry, keeps claimed entries running, and reports what it removed', () => {
      repo.enqueue({
        queue: 'mailbox',
        payload: {
          id: 'entry-pending',
      ...
      const [first] = repo.dequeue('mailbox', 1);
      expect(first.payload.id).toBe('entry-pending');

      expect(repo.cancelPendingMailboxEntry('entry-claimed')).toBe(true);
      expect(repo.cancelPendingMailboxEntry('entry-claimed')).toBe(false);
      expect(repo.cancelPendingMailboxEntry('entry-pending')).toBe(false);
      expect(repo.mailboxEntryJobStatus('entry-claimed')).toBe('absent');
      expect(repo.mailboxEntryJobStatus('entry-pending')).toBe('processing');
    });
  });

  describe('cancelHeldDeliveryJob — failed deferred-retry cleanup', () => {
    it('deletes only the pending held job for that session and uuid', () => {
      deliverMessage(repo, SESSION, 'held-uuid', { origin: 'chat' });
      db.prepare(
        `UPDATE job_queue SET payload = json_set(payload, '$.released', json('false'))
          WHERE json_extract(payload, '$.messageUuid') = 'held-uuid'`
      ).run();
      deliverMessage(repo, SESSION, 'released-uuid', { origin: 'chat' });
      deliverMessage(repo, 'sess-other', 'held-uuid', { origin: 'chat' });
      db.prepare(
        `UPDATE job_queue SET payload = json_set(payload, '$.released', json('false'))
          WHERE json_extract(payload, '$.messageUuid') = 'held-uuid'
            AND json_extract(payload, '$.sessionId') = 'sess-other'`
      ).run();

      expect(repo.cancelHeldDeliveryJob(SESSION, 'held-uuid')).toBe(true);
      expect(repo.cancelHeldDeliveryJob(SESSION, 'held-uuid')).toBe(false);
      expect(repo.activeDeliveryMessageUuids(SESSION)).toEqual(new Set(['released-uuid']));
      expect(repo.activeDeliveryMessageUuids('sess-other')).toEqual(new Set(['held-uuid']));
    });
  });
