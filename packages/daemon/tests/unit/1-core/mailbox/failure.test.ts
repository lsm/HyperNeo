...
function captureStructuredEvents(): { events: StructuredLogEvent[]; stop: () => void } {
...
function findMaterializeFailureEvent(
  events: StructuredLogEvent[],
  level: 'error' | 'warn'
): StructuredLogEvent | undefined {
  return events.find(
    (candidate) =>
      candidate.module === 'hyperneo:daemon:mailbox:materialize-failure' &&
...
describe('createMailboxDeadHandler', () => {
  ...
  test('conservatively no-ops when a uuid is reused with different content', async () => {
    const mailbox = createMailboxTestDb();
    const capture = captureStructuredEvents();
    ...
    const event = findMaterializeFailureEvent(capture.events, 'warn');
    expect(event?.metadata).toMatchObject({
      entryId: entry.id,
      sessionId: SESSION_ID,
      messageUuid: 'reused-uuid',
    });
    mailbox.close();
  });

  test('conservatively no-ops when the full message differs beyond content', async () => {
    const mailbox = createMailboxTestDb();
    const capture = captureStructuredEvents();
    ...
    expect(publishFailed).not.toHaveBeenCalled();
    expect(settleSkipped).not.toHaveBeenCalled();
    expect(findMaterializeFailureEvent(capture.events, 'warn')).toBeDefined();
    mailbox.close();
  });

  test('conservatively no-ops when a differing sibling hides behind a matching row', async () => {
    const mailbox = createMailboxTestDb();
    const capture = captureStructuredEvents();
    mailbox.sdkMessageRepo.saveUserMessage(
      SESSION_ID,
      { ...message, uuid: 'shared-sibling-uuid', session_id: SESSION_ID },
      'consumed'
    );
    ...
    mailbox.sdkMessageRepo.saveUserMessage(
      SESSION_ID,
      { ...other, uuid: 'shared-sibling-uuid', session_id: SESSION_ID },
      'enqueued'
    );
    const entry = makeEntry({ origin: 'chat', messageUuid: 'shared-sibling-uuid' });
    const job = claimMailboxJob(mailbox, entry);
    const publishFailed = mock(async () => {});
    const settleSkipped = mock(async () => {});
    const saveFailed = mock(() => 'should-not-happen');
    ...
    expect(rows[0].send_status).toBe('consumed');
    expect(rows[1].send_status).toBe('enqueued');
    expect(saveFailed).not.toHaveBeenCalled();
    expect(publishFailed).not.toHaveBeenCalled();
    expect(settleSkipped).not.toHaveBeenCalled();
    expect(findMaterializeFailureEvent(capture.events, 'warn')).toBeDefined();
  ...
  test('emits a structured error event instead of escaping when the chain throws', () => {
    ...
    const event = findMaterializeFailureEvent(capture.events, 'error');
    expect(event?.metadata).toMatchObject({
      entryId: entry.id,
      sessionId: SESSION_ID,
...
describe('materializeMailboxFailure', () => {
  ...
  test('targets unseeded session entries with the deterministic uuid', () => {
    ...
    expect(saveFailed).toHaveBeenCalledWith(
      SESSION_ID,
      expect.objectContaining({ uuid: deterministicMailboxUuid(entry.id), isSynthetic: true }),
      'system'
    );
    mailbox.close();
  });

  test('conservatively no-ops when a differing sibling holds the uuid', () => {
    const mailbox = createMailboxTestDb();
    const capture = captureStructuredEvents();
    const original: MailboxMessage = {
      type: 'user',
    ...
    mailbox.sdkMessageRepo.saveUserMessage(
      SESSION_ID,
      { ...original, uuid: 'conflicting-uuid', session_id: SESSION_ID },
      'enqueued'
    );
    const conflicting: MailboxMessage = {
      type: 'user',
  ...
  test('does not seed a failed row when the derived uuid belongs to different content', () => {
    const mailbox = createMailboxTestDb();
    const entry = makeEntry();
  ...
  test('skips settlement when the sibling lookup itself fails', () => {
    ...
    expect(findMaterializeFailureEvent(capture.events, 'error')).toBeDefined();
    expect(settleSkipped).not.toHaveBeenCalled();
    mailbox.close();
  });
