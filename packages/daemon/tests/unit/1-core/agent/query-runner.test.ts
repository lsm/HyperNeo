  ...
  describe('startup-timeout retry backoff and attempt cap', () => {
    ...
    it.skipIf(!sdkQueryIsMock)(
      ...
      async () => {
        ...
        // The real queue TTL sets this error NAME (message-queue.ts) — the
        // rejection handler's TTL-only gate keys on it, so the simulation
        // must match or it no-ops like a non-TTL rejection.
        const queueTtlError = () => {
