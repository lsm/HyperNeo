    ...
    try {
      ...
      if (isStartupTimeout && retryAttempt === 0 && !this.ctx.isCleaningUp()) {
        ...
        if (this.ctx.getQueryGeneration() !== queryGeneration) {
          ...
          // Terminalize ONLY the abandoned attempt's message (A) — the
          // startup-timeout path's stop('retry_pending') retained the shared
          ...
          // A — it does NOT drain the whole set (which could contain a
          // replacement message B the newer query already consumed, marking B
          // failed and blocking its first_progress/completed). Also NOT
          ...
          if (this._lastConsumedUserMessage === startupRetryMsg) {
            this._lastConsumedUserMessage = null;
          }
          return;
        }
