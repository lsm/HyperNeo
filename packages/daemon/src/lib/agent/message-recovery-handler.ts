...
export class MessageRecoveryHandler {
  ...
  recoverOrphanedConsumedMessages(): void {
    ...
    try {
      ...
      // The ledger is keyed by SDK UUID; rows without a real UUID ('unknown'
      ...
      // Delivery-lifecycle: record `failed` only for rows with a real SDK UUID
      // (the ledger is UUID-keyed; a global 'unknown' sentinel would let one
      // UUID-less orphan affect another). The sdk_messages status update above
      // uses dbId and applies regardless. See task #859 8511.
      for (const orphaned of recoverable) {
        if (!hasRealUuid(orphaned.uuid)) continue;
