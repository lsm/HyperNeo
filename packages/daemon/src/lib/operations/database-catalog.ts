...
export function createDatabaseOperationCatalog(
  ...
  overrides: Partial<TaskOperationDependencies & AgentOperationDependencies> = {}
) {
  const agentBag: AgentOperationDependencies = {
    listAgents: (spaceId) => db.getLongHorizonAgentRepo().listBySpaceId(spaceId),
    getAgent: (agentId) => db.getLongHorizonAgentRepo().getById(agentId),
  };
