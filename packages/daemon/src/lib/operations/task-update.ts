...
export function createUpdateTaskOperation(
  ...
  return defineOperation({
    name: 'task.update',
    description:
      'Edit available task metadata. Space-scoped MCP callers can edit tasks in their owning Space. Supply taskId and at least one of title, description, priority, labels or preferredWorkflowId. preferredWorkflowId selects which workflow the task runs and is refused for MCP callers. Omitted fields are preserved. Returns null for missing or unavailable targets. Does not change lifecycle or execution.',
    inputSchema: z
      .object({
        ...
        preferredWorkflowId: z.string().min(1).nullable().optional(),
      })
      .strict()
      .refine(
        (input) =>
