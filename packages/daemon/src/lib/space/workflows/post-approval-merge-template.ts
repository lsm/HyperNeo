...
export const CODER_OWNED_MERGE_INSTRUCTIONS: string = [
  ...
  '        gh pr view {{pr_url}} --json state,mergeStateStatus,autoMergeRequest --jq .',
  '      MERGED -> step 3. `--json state` alone cannot see a removed merge-queue entry or a failed merge-group',
  '      check (both leave the PR OPEN), so decide from the extra fields: mergeStateStatus BEHIND/DIRTY/BLOCKED,',
  '      or autoMergeRequest is null after the merge was enqueued (queue entry removed / merge-group check',
  '      failed), means the merge will not complete — treat it as a blocker per 2c (capture the snapshot).',
  '      Otherwise re-query roughly once a minute, up to ~10 attempts; if it is still not MERGED after that, the',
  '      merge did not complete — treat it as a blocker per 2c (this intra-turn poll has no other cap; do not',
  '      loop indefinitely).',
