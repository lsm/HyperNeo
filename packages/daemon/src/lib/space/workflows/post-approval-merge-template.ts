...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '   branch name: this built-in runs against arbitrary repos (dev/main/master/...).',
  ...
  '          data: { pr_url: "{{pr_url}}", base_branch: "$BASE",',
  ...
  '                  base_branch: "$BASE", conflicting_files: ["..."],',
  '                  reason: "merge_conflict" })',
  '      The message body MUST instruct the coder: "Rebase onto latest',
  '      `origin/$BASE`, resolve the listed conflicts, run the tests that touch',
  ...
  '5. Sync so both this isolated worktree AND the root repository track the freshly-merged $BASE.',
  '   The reviewer session is the ONLY actor that fast-forwards the separate root checkout —',
  '   do NOT skip step b, or every later task branched from a stale root inherits the stale base branch.',
  ...
  '      Guard the root before pulling. `git pull --ff-only origin "$BASE"` fast-forwards the',
  '      CURRENTLY checked-out branch, so a root not on $BASE would be moved or silently left',
  '      behind, and a local $BASE ahead of origin/$BASE prints "Already up to date" while',
  ...
  '        if [ "$(git -C "$ROOT" rev-parse --abbrev-ref HEAD)" != "$BASE" ]; then',
  '          # root checked out on a different branch — do NOT move it; warn and skip.',
  '          record a NON-result cleanup_warning artifact (root not on $BASE) and continue.',
  '        fi',
  '        git -C "$ROOT" fetch origin "$BASE"',
  '        git -C "$ROOT" pull --ff-only origin "$BASE"',
  '        if [ "$(git -C "$ROOT" rev-parse HEAD)" != "$(git -C "$ROOT" rev-parse "origin/$BASE")" ]; then',
  ...
  '          record a NON-result cleanup_warning artifact (root $BASE ahead of origin/$BASE) and continue.',
  '        fi',
