...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '      and only re-attempt `gh pr merge {{pr_url}} --squash` once both pass',
  '      again. If it still conflicts, loop once more — maximum 2 coder attempts.',
  '      Conflict handoffs reuse the Review → Coding cycle budget, so if a',
  '      handoff is rejected because the cycle cap is reached, treat that as the',
  '      end of the loop. After 2 coder attempts (or an early cycle-cap exit),',
  '      escalate to space-agent with the accumulated evidence and stop:',
