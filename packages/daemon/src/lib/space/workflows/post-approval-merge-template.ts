...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '      routed blocker is resolved, re-run',
  '      the step 1 and step 2 checks. BUT if the coder PUSHED to resolve it',
  '      (A9/A10/B3/D1/D4 — the current headRefOid differs from the one step 1',
  '      captured), do NOT merge on the stale approval: this template targets',
  '      arbitrary repos, some without auto-dismiss-stale-approvals, so mirror',
  '      conflict-loop step e — fetch the current head, inspect the FULL delta vs',
  '      the approved head, and post a FRESH review before retrying. Only retry the',
  '      merge when the head is unchanged or the new head is freshly approved. If NO',
