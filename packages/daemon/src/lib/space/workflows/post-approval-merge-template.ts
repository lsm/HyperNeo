...
export const CODER_OWNED_MERGE_INSTRUCTIONS: string = [
  ...
  '     test "$REVIEW_DECISION" = "APPROVED" || { echo "GitHub reviewDecision is $REVIEW_DECISION (REVIEW_REQUIRED or CHANGES_REQUESTED) — required approvals / code-owner rules not met, or an outstanding AUTHORIZED change request; do NOT merge." >&2; exit 1; }',
  '   REVIEW_DECISION is the aggregate per branch protection: it counts only AUTHORIZED reviews (authorAssociation OWNER/MEMBER/COLLABORATOR) and enforces required-approvals / code-owner rules. Do NOT merge on REVIEW_REQUIRED or CHANGES_REQUESTED even if a single current-head approval is visible below — GitHub may still require more approvals or a code-owner review. (This also ignores outside-contributor reviews, which have no merge authority.)',
