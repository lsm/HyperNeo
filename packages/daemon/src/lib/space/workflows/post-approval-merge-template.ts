...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '      the end of the loop.',
  '   g. If the loop ends without a successful merge (2 attempts used or cycle',
  '      cap reached), escalate to space-agent with the evidence AND call',
  '      request_human_input with the same summary, so the task is not left',
  '      stuck in post-approval — do not mark the task complete:',
  ...
  '      Then call request_human_input with the same summary.',
