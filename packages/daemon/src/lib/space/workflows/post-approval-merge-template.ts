...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '        B3 Head branch deleted (`gh pr view {{pr_url}} --json headRef`):',
  '           message the coder to restore/recreate the branch.',
  '        B4 Base branch deleted/renamed (`gh pr view {{pr_url}} --json baseRef`):',
  '           unusual — go to step h.',
  ...
  '      Category E — permission/access (insufficient merge perms, org membership,',
  '      IP allowlist) and Category F — GitHub mechanics (F1 merge queue: retry',
  '      `gh pr merge {{pr_url}} --queue`; F2 locked branch): these are not coder',
  '      work and not self-fixable — go to step h.',
