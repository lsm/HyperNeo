...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '      coder changes with that fresh URL as review_url. If the fix is sound,',
  '      post a fresh APPROVED review on the corrected head before retrying —',
  '      any conflict-fix force-push can dismiss stale approvals in',
  '      required-review repos, so re-approve on every retry, not only after a',
  '      CHANGES_REQUESTED. Only then re-attempt',
  ...
  '   warning artifact and continue — the PR is already merged, so do NOT let a',
  '   cleanup failure block the completion step. Do NOT delete `dev`.',
