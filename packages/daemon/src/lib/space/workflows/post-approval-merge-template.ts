...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '      not expose a review URL, so use the paginated REST API with the same',
  '      <host> step 2 extracts (for GitHub Enterprise), so a large review',
  '      history cannot hide the approval:',
  ...
  '      Include BOTH pr_url and the resolved review/comment URL in the data',
  '      payload so the gate opens:',
  ...
  '      conflict site was resolved soundly; if a resolution looks wrong, post',
  '      a fresh PR comment documenting the issue and request changes from the',
  '      coder, using that fresh comment URL (not the stale approval URL) as the',
  '      review_url in the step-c handoff — review-posted-gate resets each cycle.',
  '      Only then re-attempt',
