...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '      coder changes with that fresh URL as review_url — this counts as one',
  '      coder round. If the fix is sound, post a fresh APPROVED review on the',
  ...
  '      then re-attempt `gh pr merge {{pr_url}} --squash`. Every coder round',
  '      counts against the 2-attempt cap — whether it ends in a conflicting',
  '      re-merge OR a CHANGES_REQUESTED for a bad fix — so after 2 rounds',
  '      total, go to step g instead of looping again.',
