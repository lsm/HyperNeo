...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '           for `state` == MERGED before steps 4-6 — BUT if the queue entry is',
  '           removed or its merge-group check fails (the PR stays open, never',
  '           MERGED), do NOT poll forever: return to diagnosis / step h.',
  ...
  '      routed blocker is resolved, re-run the step 1 and step 2 checks, then',
  '      retry the merge. BUT if the coder PUSHED to resolve it — any coder-push',
  '      category (A3 / A9 / A10 / B3 / C1 / D1 / D4, not exhaustive) — the PR',
  '      head changed: the merger does NOT review or re-approve (NEVER approve a',
  '      PR yourself — no review authority) and does NOT merge on the stale',
  '      approval — message the coder that their push changed the reviewed head',
  '      and the PR must be re-approved before merge, then re-run steps 1 and 2',
  '      and retry only once an APPROVED review COVERS the current head — its',
  '      `commit_id` equals the current headRefOid (a stale approval on the old',
  '      head does NOT cover the new head; the merger VERIFIES this, never posts',
  '      a review). A repo that auto-dismisses stale approvals surfaces this as',
  '      A2. If NO',
