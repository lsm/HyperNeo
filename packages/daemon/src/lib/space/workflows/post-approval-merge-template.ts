...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '      coder changes with that fresh URL as review_url. If a prior',
  '      CHANGES_REQUESTED from this loop is now resolved, post a fresh APPROVED',
  '      review on the corrected head before retrying — required-review repos',
  '      block the merge until the same reviewer re-approves. Only then',
  '      re-attempt',
