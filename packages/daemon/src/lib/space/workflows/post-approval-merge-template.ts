...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '        CLEAN    -> no blocker; retry the merge (the failure was transient)',
  ...
  '        A1 Required status checks failing (`gh pr checks {{pr_url}}` shows fail',
  '           or in_progress): rerun the failed jobs',
  '           (`gh run rerun <RUN_ID> --failed`); if still failing after 2 reruns,',
  '           message the coder with the failing job, then re-run step 1/2 + retry.',
  ...
  '        A4 Code-owner review required (a CODEOWNERS-owned path changed): go to',
  '           step h to route to the code owner.',
  ...
  '           `--merge`.',
  '        A9 Required commit signatures (`git log --format=%G?` shows unsigned',
  '           commits): message the coder to sign and force-push-with-lease.',
  ...
  '        A12 Force-push / A13 branch-deletion / A14 push restrictions: not merge',
  '           blockers themselves — never pass a branch-delete flag on merge',
  '           (already the case); a force-push need is coder work.',
  '      Category B — PR state:',
  '        B1 Merge conflicts (`gh pr view {{pr_url}} --json mergeable` false):',
  '           steps a-f above.',
  ...
  '           (`gh api repos/<owner>/<repo>/pulls/<number>/reviews` shows',
  '           CHANGES_REQUESTED): message the coder to address; only the reviewer',
  '           (via step h) can dismiss/supersede it — not you.',
  ...
  '           (`gh api repos/<owner>/<repo>/commits/<SHA>/status`): identify the',
  '           external service and message the coder.',
  ...
  '      step 2 checks, then retry the merge.',
  '   h. Escalate ONLY when step g routes a human/review/operator blocker here, OR the cyclic Post-Approval →',
