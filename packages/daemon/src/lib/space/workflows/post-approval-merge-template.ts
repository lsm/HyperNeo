 ...
 *   - `{{workspace_path_sh}}`  — the single-quote-escaped shell literal of the
 *                              Space checkout's absolute path (for step 6), so
 *                              the `SPACE_WS=` assignment survives paths with
 *                              single quotes. (`{{workspace_path}}` stays
 *                              verbatim for user-defined templates.)
...
export const CODER_OWNED_MERGE_INSTRUCTIONS: string = [
  ...
  "     gh pr view {{pr_url}} --json reviews --jq '[.reviews[] | {state, commit: .commit.oid, author: .author.login, authorAssociation: .authorAssociation, body, submittedAt}]'",
  '   First, reject any effective change request: for each review in the list, it is an effective `CHANGES_REQUESTED` if its state is CHANGES_REQUESTED, OR if its state is COMMENTED AND its author is the PR author (login equals $PR_AUTHOR) AND its body carries the exact marker line "Recommendation: REQUEST_CHANGES" (the own-PR fallback for requesting changes — GitHub rejects REQUEST_CHANGES from the PR owner). Such a review is blocking unless the SAME author has since left a LATER `APPROVED` review with a LATER `submittedAt` (or the marked own-PR approval-fallback COMMENTED review — a plain COMMENTED review does NOT withdraw requested changes, and a REQUEST_CHANGES marker from the PR author is a blocking verdict, not a plain comment). If any effective change request stands, the merge is blocked — report per step 4 and wait (the author must dismiss or re-approve). Do NOT merge while a changes-requested stands, even if another reviewer approved (the deleted merge-pr gate rejected these too).',
