...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '   If it FAILS for any reason (conflict, failing check, missing approval,',
  '   permissions, merge queue, etc.) — do NOT diagnose-and-fix it yourself and',
  ...
  '      review threads), and confirm an APPROVED review now COVERS the current',
  '      head (its commit_id equals the current headRefOid; a stale approval on',
  '      the old head does NOT cover the new one — if not covered, report again',
  '      per 3b instead of merging). Only then re-attempt',
