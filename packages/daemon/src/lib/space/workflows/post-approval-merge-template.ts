...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '        A1 Required status checks: `gh pr checks {{pr_url}}`. If a check FAILED',
  '           (completed, red), rerun its failed jobs',
  ...
  '           If a check is still IN_PROGRESS / pending, do NOT rerun — wait (or use',
  '           `gh pr merge --auto`); `gh run rerun` errors on an unfinished run.',
  ...
  '           `--merge`.',
  '        A9 Required commit signatures: fetch the PR head and inspect ONLY the',
  '           base..head commits — bare `git log --format=%G?` scans the whole repo',
  '           and blames the coder for unsigned base history. Any unsigned commit',
  '           in the range -> message the coder to sign and force-push-with-lease.',
  ...
  '        C1 Changes requested (`gh pr view {{pr_url}} --json reviewDecision` is',
  '           REVIEW_REQUIRED, and the LATEST review by a reviewer is',
  '           CHANGES_REQUESTED): a later APPROVED by the same reviewer supersedes',
  '           an earlier CHANGES_REQUESTED, so use the latest review per reviewer',
  '           (or the aggregate reviewDecision) — never a stale historical entry.',
