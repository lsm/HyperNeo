...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '   branched from it inherits old `dev`), then exit. Do NOT redo steps 2–4 and do NOT save',
  '   the step-6 merge artifact (no merge happened in this session).',
  ...
  '5. Sync so both this isolated worktree AND the root repository track the freshly-merged `dev`.',
  '   The reviewer session is the ONLY actor that fast-forwards the separate root checkout —',
  '   do NOT skip step b, or every later task branched from a stale root inherits old `dev`.',
  '   a. In this isolated worktree, do NOT `git checkout dev` and do NOT switch branches — the',
  '      worktree must stay on its task branch. Just fetch:',
  '        git fetch origin dev',
  '   b. The post-approval session receives NO worktree-isolation banner (it has no',
  '      session.worktree), so derive the root path from git itself — your cwd is the task',
  "      worktree, and its shared common dir is the root repo's `.git`, whose parent is the",
  '      root checkout (this holds whether cwd is a worktree or the main repo itself):',
  '        ROOT=$(cd "$(git rev-parse --git-common-dir)/.." && pwd)',
  '      Guard the root before pulling. `git pull --ff-only origin dev` fast-forwards the',
  '      CURRENTLY checked-out branch, so a root not on `dev` would be moved or silently left',
  '      behind, and a local `dev` ahead of `origin/dev` prints "Already up to date" while',
  '      hiding stray unmerged commits that later task worktrees would inherit. Each guard',
  '      records a NON-result cleanup_warning artifact (never a "result") and continues:',
  '        if [ "$(git -C "$ROOT" rev-parse --abbrev-ref HEAD)" != "dev" ]; then',
  '          # root checked out on a different branch — do NOT move it; warn and skip.',
  '          record a NON-result cleanup_warning artifact (root not on dev) and continue.',
  '        fi',
  '        git -C "$ROOT" fetch origin dev',
  '        git -C "$ROOT" pull --ff-only origin dev',
  '        if [ "$(git -C "$ROOT" rev-parse HEAD)" != "$(git -C "$ROOT" rev-parse origin/dev)" ]; then',
  '          # pull said "Already up to date" but local dev is AHEAD of origin/dev — stray',
  '          # commits remain at HEAD; do NOT claim the root is synchronized.',
  '          record a NON-result cleanup_warning artifact (root dev ahead of origin/dev) and continue.',
  '        fi',
