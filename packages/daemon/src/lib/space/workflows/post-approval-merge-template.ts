...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '   b. The reviewer session is ALSO responsible for fast-forwarding the SEPARATE root',
  '      repository checkout — nothing else does it. The worktree-isolation banner above names',
  '      that path as the "Main repository" and explicitly permits writes when these',
  '      post-approval instructions say so:',
  '        git -C <mainRepoPath> pull --ff-only origin dev',
  '      Do NOT skip this: a stale root repo silently breaks every later task branched from',
  '      it. If the ff-only fails (local divergence), do NOT force it — record a NON-result',
  '      cleanup_warning artifact and continue (the PR is already merged; this mirrors',
  '      step 4 best-effort handling).',
