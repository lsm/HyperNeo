...
export const CODER_OWNED_MERGE_INSTRUCTIONS: string = [
  ...
  '   If state is MERGED, the merge already happened (possibly in a prior session). Record an audit artifact, perform step 4 ONLY (fast-forward the root checkout — a restart after merge must still sync it), then call mark_complete to close the task. Do NOT save the step-6 new-merge artifact (no merge happened in this session).',
