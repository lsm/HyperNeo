...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '      daemon restart/cache miss. Fetch the PR head OID and merge-tree it',
  '      against origin/dev:',
  '        HEAD_OID=$(gh pr view {{pr_url}} --json headRefOid --jq .headRefOid)',
  '        git fetch origin dev && git merge-tree --write-tree --no-messages origin/dev "$HEAD_OID"',
  ...
  '      `list_channels` for the Review → Coding route: in the Coding/Research',
  '      workflows it is guarded by review-posted-gate (requires both `pr_url`',
  '      and `review_url`, resets each cycle); in Fullstack QA it is UNGATED.',
  ...
  '      and inspect the FULL delta since the approved head — use the',
  '      approved_head_oid recorded in the step-b artifact as the diff base',
  '      (`git diff <approved_head_oid>..HEAD` or `gh pr diff`) — the conflict',
  '      resolution AND any unrelated changes in the same push (do NOT restrict',
  '      to the conflicting files only). Confirm each conflict site was resolved',
  '      soundly and that no unexpected non-conflict changes sneaked in; if',
  '      anything is wrong, post a fresh formal CHANGES_REQUESTED review (or,',
  '      for an own-PR where GitHub blocks that, a fresh PR comment) documenting',
  '      the issue and request changes from the coder, using that fresh URL as',
  '      the review_url in the step-c handoff — review-posted-gate resets each',
  '      cycle. Only then re-attempt',
  ...
  '      applied — never a hard-coded number. Record the block as a NON-result',
  '      artifact (a "result" artifact would be picked up as the task result',
  '      when the task is later marked complete), then notify space-agent:',
