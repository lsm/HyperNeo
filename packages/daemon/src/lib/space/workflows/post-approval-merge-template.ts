...
export const PR_MERGE_POST_APPROVAL_INSTRUCTIONS: string = [
  ...
  '        UNKNOWN  -> GitHub is recomputing mergeability (transient) — wait ~30s',
  '                   and re-query before diagnosing (the repo own',
  '                   pr-ready-validator retries UNKNOWN); only escalate if it',
  '                   persists.',
  '        (other)  -> OUT_OF_DATE / HAS_HOOKS: run the per-category checks below',
  '                   to pin it down (OUT_OF_DATE often tracks A10).',
  ...
  '           If it reports "no required checks" (empty set, non-zero exit) that',
  '           means A1 is SATISFIED — continue the checklist; do NOT treat that',
  '           exit as a diagnostic error that routes to step h.',
  '           If a required check FAILED (completed, red), rerun its failed jobs',
  '           (`gh run rerun <RUN_ID> --failed` — only valid on a COMPLETED run); if',
  '           still failing after 2 reruns, message the coder with the failing job.',
  '           If a required check is still IN_PROGRESS / pending, do NOT rerun',
  '           (`gh run rerun` errors on an unfinished run) and do NOT enable',
  '           auto-merge. BUT first check whether the pending run is blocked on a',
  '           deployment-environment approval (not merely running) — if so, route',
  '           via D3 to step h instead of waiting forever. Otherwise wait for the',
  '           check to finish, then RE-RUN steps 1 and 2 (the wait window can let',
  '           a reviewer request changes or open a thread) before retrying the',
  '           step-3 merge. Only proceed to steps 4-6 once',
  '           `state` is MERGED: `--auto` only',
  '           ENABLES auto-merge and returns before the merge actually happens.',
  '        A2 Required approving-review count not met AND reviewDecision is',
  '           REVIEW_REQUIRED (`gh pr view {{pr_url}} --json reviewDecision`): a',
  '           reviewer is needed — go to step h. If reviewDecision is',
  '           CHANGES_REQUESTED, skip to C1 (that is coder work). NEVER approve the',
  '           PR yourself (no review authority).',
  ...
  '        B3 Head branch deleted (`headRefName` keeps the recorded name after',
  '           deletion, so test the ref itself — `gh api --hostname <host>',
  '           repos/<owner>/<repo>/git/refs/heads/<headRefName>` 404s when gone):',
  '           message the coder to restore/recreate the branch.',
  ...
  '        C2 Pending review in progress: NOT an actionable blocker — the',
  '           EXCEPTION below says to wait for it (do NOT escalate an extra',
  '           pending review when approvals are already met).',
  '        C3 Required team review / C4 dismissed review request: go to step h',
  '           to re-engage the reviewer/team.',
  ...
  '           that aggregate includes OPTIONAL contexts, so confirm the failing',
  '           status is one the base branch actually requires before routing;',
  ...
  '           merge-queue-required base branch enqueues the PR on the attempt;',
  '           there is NO `--queue` flag. Do NOT add `--auto` (it only ENABLES',
  '           auto-merge and returns before the merge — same reason as A1); wait',
  '           for `state` == MERGED before steps 4-6.',
  ...
  '      genuinely IN_PROGRESS / pending is NOT an actionable blocker — wait for',
  '      it to finish (re-check with `gh pr checks {{pr_url}} --required --watch`',
  '      or a periodic re-query) instead of messaging or escalating. After a',
  '      routed blocker is resolved, re-run the step 1 and step 2 checks. BUT if',
  '      the coder PUSHED to resolve it — ANY category the coder handles by pushing',
  '      (A3 / A9 / A10 / B3 / C1 / D1 / D4 — not exhaustive), so the headRefOid',
  '      changed — first recover the APPROVED head = the commit_id of the',
  '      latest APPROVED review (for an own-PR, the merger own COMMENTED',
  '      approval-fallback review per conflict-loop step e — NOT any arbitrary',
  '      reviewer COMMENTED review, which is a comment, not approval)',
  '      (`gh api --hostname <host> repos/<owner>/<repo>/pulls/<number>/reviews`,',
  '      latest APPROVED `.commit_id` — step 1 captures no headRefOid',
  '      and a transient value would not survive the fresh shell of a later call,',
  '      so the review commit_id is the durable baseline). If it differs from the',
  '      current headRefOid, do NOT merge on the stale approval: this template',
  '      targets arbitrary repos, some without auto-dismiss-stale-approvals, so',
  '      mirror conflict-loop step e — fetch the current head, inspect the FULL',
  '      delta vs that approved-head commit_id, and post a FRESH review before',
  '      retrying. Only retry when the head is unchanged or freshly approved. If NO',
