...
const APPROVALS_SCRIPT = [
  ...
  'COMMENT_OK=$(jq --arg head "$HEAD_OID" \'[.[][] | select((.user.login | test("codex"; "i")) and .user.type == "Bot" and ((.body // "") | contains($head)))] | length\' <<< "$COMMENTS")',
  'REACTION_OK=$(jq \'[.[][] | select((.user.login | test("codex"; "i")) and .user.type == "Bot" and .content == "+1")] | length\' <<< "$REACTIONS")',
  'FRESH_REACTION_OK=$(jq --arg since "$WAIT_STARTED" \'[.[][] | select((.user.login | test("codex"; "i")) and .user.type == "Bot" and .content == "+1" and (.created_at // "") > $since)] | length\' <<< "$REACTIONS")',
  ...
  'NOW_EPOCH=$(date -u +%s)',
  'if [ $((NOW_EPOCH - START_EPOCH)) -lt 7200 ]; then jq -n --argjson approvals "$MERGED" --arg started "$WAIT_STARTED" --arg head "$HEAD_OID" \'{"type":"block","reason":"Plan approval requires fresh Codex bot approval for current head or 2-hour timeout from approval handoff","data":{"approvals":$approvals,"approval_count":4,"codex_wait_started_at":$started,"codex_wait_head_oid":$head}}\'; exit 0; fi',
...
const REVIEW_APPROVAL_SCRIPT = [
  ...
  'NOW_ISO=$(date -u +%Y-%m-%dT%H:%M:%SZ)',
  'if [ -z "$WAIT_STARTED" ] || [ "$WAIT_HEAD" != "$HEAD_OID" ]; then jq -n --arg started "$NOW_ISO" --arg head "$HEAD_OID" \'{"type":"block","reason":"Review approval requires fresh Codex bot approval for current head or 2-hour timeout from approval handoff","data":{"codex_wait_started_at":$started,"codex_wait_head_oid":$head}}\'; exit 0; fi',
  'WAIT_STARTED_PARSE=${WAIT_STARTED%%.*}; WAIT_STARTED_PARSE=${WAIT_STARTED_PARSE%Z}Z',
  'START_EPOCH=$(date -u -j -f "%Y-%m-%dT%H:%M:%SZ" "$WAIT_STARTED_PARSE" +%s 2>/dev/null || date -u -d "$WAIT_STARTED" +%s 2>/dev/null || echo 0)',
  'NOW_EPOCH=$(date -u +%s)',
  'if [ $((NOW_EPOCH - START_EPOCH)) -lt 7200 ]; then jq -n --arg started "$WAIT_STARTED" --arg head "$HEAD_OID" \'{"type":"block","reason":"Review approval requires fresh Codex bot approval for current head or 2-hour timeout from approval handoff","data":{"codex_wait_started_at":$started,"codex_wait_head_oid":$head}}\'; exit 0; fi',
