...
const APPROVALS_SCRIPT = [
  ...
  'if [ "$COMMENT_OK" != "0" ] || { [ "$REACTION_OK" != "0" ] && [ "$WAIT_HEAD" = "$HEAD_OID" ]; }; then jq -n --argjson approvals "$MERGED" --argjson reaction_count "$REACTION_OK" \'{"type":"allow","data":{"approvals":$approvals,"codex_approved":true,"codex_reaction_count":$reaction_count}}\'; exit 0; fi',
...
const REVIEW_APPROVAL_SCRIPT = [
  ...
  'COMMENT_OK=$(jq --arg head "$HEAD_OID" \'[.[][] | select((.user.login == "codex[bot]" or .user.login == "chatgpt-codex-connector[bot]") and ((.body // "") | contains($head)))] | length\' <<< "$COMMENTS")',
  'REACTION_OK=$(jq \'[.[][] | select((.user.login == "codex[bot]" or .user.login == "chatgpt-codex-connector[bot]") and .content == "+1")] | length\' <<< "$REACTIONS")',
  'if [ "$COMMENT_OK" != "0" ] || { [ "$REACTION_OK" != "0" ] && [ "$WAIT_HEAD" = "$HEAD_OID" ]; }; then jq -n --arg url "$PR_URL" --argjson reaction_count "$REACTION_OK" \'{"type":"allow","data":{"approved":true,"pr_url":$url,"codex_approved":true,"codex_reaction_count":$reaction_count}}\'; exit 0; fi',
  'NOW_ISO=$(date -u +%Y-%m-%dT%H:%M:%SZ)',
