  ...
  async activateNode(
    ...
    for (const agentEntry of agents) {
      ...
      if (existing) {
        ...
        // once assigned, but the clean-recovery resets — every caller of
        // `NodeExecutionRepository.resetForCleanRecovery` (space-stop parking,
        // passed rate caps, transient spawn aborts; see that helper's doc) —
        // deliberately clear it; either way spawn code detects pending status
