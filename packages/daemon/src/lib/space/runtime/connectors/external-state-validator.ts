...
export function createExternalStateValidator(
  ...
  return async (context: HookExecutorContext): Promise<WorkflowHookResult> => {
    ...
    if (config.pending && evaluatePredicate(config.pending, data)) {
      // Tag predicate-pending results so callers can tell them apart from a
      // *connector failure* that also presents as `retryable_block` (rate limit
      // / outage). A pending state ("the bot hasn't +1'd yet") is safe to time
      // out into an allow (the codex preset does exactly this with its 2h
      // safety valve); a lookup failure is not — an outage must never open the
      // gate.
      return {
        type: 'retryable_block',
