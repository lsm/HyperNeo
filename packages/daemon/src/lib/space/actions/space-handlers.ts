...
export function createSpaceAgentToolHandlers(config: SpaceAgentToolsConfig) {
  ...
  const deliverTaskWorkerMessage = taskAgentManager
    ? createDeliverTaskWorkerMessagePipeline({
        taskAgentManager,
        nodeExecutionRepo,
        ensureTargetSession,
  ...
  return {
    ...
    async send_message_to_task(args: {
      ...
      if (trimmedTarget) {
        let targetAddress: ReturnType<typeof parseAddress>;
        try {
        ...
        if (handleResolution?.kind === 'long-horizon-agent' && targetAddress.kind === 'handle') {
          ...
          return jsonResult({
            ...
            error:
              ...
              `To target the long-horizon agent explicitly, omit task_id and invoke session.message.send instead.`,
          });
        }
      ...
      const delivered = await deliverTaskWorkerMessage({
        task,
        workflowRunId: task.workflowRunId,
        resolved,
    ...
    async resolve_forge_scope(args: { goal_id?: string; task_id?: string }): Promise<ToolResult> {
      try {
        let scope: ReturnType<
          ReturnType<typeof requireEvolutionScopeService>['resolveScopeForGoal']
        >;
        if (args.goal_id) {
          requireGoalInSpace(args.goal_id);
