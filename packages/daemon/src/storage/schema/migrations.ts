...
export function runMigrations(db: BunDatabase, createBackup: () => void): void {
  ...
  // Migration 194: Widen space_goal_events.event_type CHECK to include
  // 'automation_noop' for self_nag low-evidence no-op notes (#919). Renumbered
  // 185→186→187→194: dev shipped M186 (message-delivery active-session index)
  // and M187–M193 (delivery turns, gate removal, stall detection, dead loops).
...
export function runMigration194(db: BunDatabase): void {
  ...
  const widenedSql = tableSql
    .replace(
      ...
      'CREATE TABLE space_goal_events_m194_new'
    )
    .replace(
  ...
  try {
    db.exec(widenedSql);
    db.exec(
      `INSERT INTO space_goal_events_m194_new (${quoted}) SELECT ${quoted} FROM space_goal_events`
    );
    db.exec('DROP TABLE space_goal_events');
    db.exec('ALTER TABLE space_goal_events_m194_new RENAME TO space_goal_events');
    for (const object of objects) db.exec(object.sql);
    db.exec('COMMIT');
  } catch (error) {
    db.exec('ROLLBACK');
