...
// The EXACT OLD (pre-change) Reviewer seed prompt + description. A row is an
// unmodified seed ONLY when its stored prompt/description EQUAL these values
// (and its tools equal OLD_REVIEWER_TOOLS) — a substring match is insufficient
// because a user who appended instructions to the old prompt would still contain
// any distinctive marker. Any difference (append, edit, replace) means a user
// customization and the row is left for the drift/sync UI.
...
// `gh api repos/.../reviews` command (superseded by the run-scoped GraphQL
// mutation), so a pristine row carrying it must be migrated too.
